
obj/user/echosrv.debug:     file format elf32-i386


Disassembly of section .text:

00800020 <_start>:
// starts us running when we are initially loaded into a new environment.
.text
.globl _start
_start:
	// See if we were started with arguments on the stack
	cmpl $USTACKTOP, %esp
  800020:	81 fc 00 e0 bf ee    	cmp    $0xeebfe000,%esp
	jne args_exist
  800026:	75 04                	jne    80002c <args_exist>

	// If not, push dummy argc/argv arguments.
	// This happens when we are loaded by the kernel,
	// because the kernel does not know about passing arguments.
	pushl $0
  800028:	6a 00                	push   $0x0
	pushl $0
  80002a:	6a 00                	push   $0x0

0080002c <args_exist>:

args_exist:
	call libmain
  80002c:	e8 a9 04 00 00       	call   8004da <libmain>
1:	jmp 1b
  800031:	eb fe                	jmp    800031 <args_exist+0x5>

00800033 <die>:
#define BUFFSIZE 32
#define MAXPENDING 5    // Max connection requests

static void
die(char *m)
{
  800033:	55                   	push   %ebp
  800034:	89 e5                	mov    %esp,%ebp
  800036:	83 ec 10             	sub    $0x10,%esp
	cprintf("%s\n", m);
  800039:	50                   	push   %eax
  80003a:	68 10 28 80 00       	push   $0x802810
  80003f:	e8 89 05 00 00       	call   8005cd <cprintf>
	exit();
  800044:	e8 d7 04 00 00       	call   800520 <exit>
  800049:	83 c4 10             	add    $0x10,%esp
}
  80004c:	c9                   	leave  
  80004d:	c3                   	ret    

0080004e <handle_client>:

void
handle_client(int sock)
{
  80004e:	55                   	push   %ebp
  80004f:	89 e5                	mov    %esp,%ebp
  800051:	57                   	push   %edi
  800052:	56                   	push   %esi
  800053:	53                   	push   %ebx
  800054:	83 ec 30             	sub    $0x30,%esp
  800057:	8b 75 08             	mov    0x8(%ebp),%esi
	char buffer[BUFFSIZE];
	int received = -1;
	// Receive message
	if ((received = read(sock, buffer, BUFFSIZE)) < 0)
  80005a:	6a 20                	push   $0x20
  80005c:	8d 45 c8             	lea    -0x38(%ebp),%eax
  80005f:	50                   	push   %eax
  800060:	56                   	push   %esi
  800061:	e8 6e 14 00 00       	call   8014d4 <read>
  800066:	89 c3                	mov    %eax,%ebx
  800068:	83 c4 10             	add    $0x10,%esp
  80006b:	85 c0                	test   %eax,%eax
  80006d:	79 0a                	jns    800079 <handle_client+0x2b>
		die("Failed to receive initial bytes from client");
  80006f:	b8 14 28 80 00       	mov    $0x802814,%eax
  800074:	e8 ba ff ff ff       	call   800033 <die>

	// Send bytes and check for more incoming data in loop
	while (received > 0) {
		// Send back received data
		if (write(sock, buffer, received) != received)
  800079:	8d 7d c8             	lea    -0x38(%ebp),%edi
  80007c:	eb 3b                	jmp    8000b9 <handle_client+0x6b>
  80007e:	83 ec 04             	sub    $0x4,%esp
  800081:	53                   	push   %ebx
  800082:	57                   	push   %edi
  800083:	56                   	push   %esi
  800084:	e8 21 15 00 00       	call   8015aa <write>
  800089:	83 c4 10             	add    $0x10,%esp
  80008c:	39 d8                	cmp    %ebx,%eax
  80008e:	74 0a                	je     80009a <handle_client+0x4c>
			die("Failed to send bytes to client");
  800090:	b8 40 28 80 00       	mov    $0x802840,%eax
  800095:	e8 99 ff ff ff       	call   800033 <die>

		// Check for more data
		if ((received = read(sock, buffer, BUFFSIZE)) < 0)
  80009a:	83 ec 04             	sub    $0x4,%esp
  80009d:	6a 20                	push   $0x20
  80009f:	57                   	push   %edi
  8000a0:	56                   	push   %esi
  8000a1:	e8 2e 14 00 00       	call   8014d4 <read>
  8000a6:	89 c3                	mov    %eax,%ebx
  8000a8:	83 c4 10             	add    $0x10,%esp
  8000ab:	85 c0                	test   %eax,%eax
  8000ad:	79 0a                	jns    8000b9 <handle_client+0x6b>
			die("Failed to receive additional bytes from client");
  8000af:	b8 60 28 80 00       	mov    $0x802860,%eax
  8000b4:	e8 7a ff ff ff       	call   800033 <die>
	// Receive message
	if ((received = read(sock, buffer, BUFFSIZE)) < 0)
		die("Failed to receive initial bytes from client");

	// Send bytes and check for more incoming data in loop
	while (received > 0) {
  8000b9:	85 db                	test   %ebx,%ebx
  8000bb:	7f c1                	jg     80007e <handle_client+0x30>

		// Check for more data
		if ((received = read(sock, buffer, BUFFSIZE)) < 0)
			die("Failed to receive additional bytes from client");
	}
	close(sock);
  8000bd:	83 ec 0c             	sub    $0xc,%esp
  8000c0:	56                   	push   %esi
  8000c1:	e8 ce 12 00 00       	call   801394 <close>
  8000c6:	83 c4 10             	add    $0x10,%esp
}
  8000c9:	8d 65 f4             	lea    -0xc(%ebp),%esp
  8000cc:	5b                   	pop    %ebx
  8000cd:	5e                   	pop    %esi
  8000ce:	5f                   	pop    %edi
  8000cf:	5d                   	pop    %ebp
  8000d0:	c3                   	ret    

008000d1 <umain>:

void
umain(int argc, char **argv)
{
  8000d1:	55                   	push   %ebp
  8000d2:	89 e5                	mov    %esp,%ebp
  8000d4:	57                   	push   %edi
  8000d5:	56                   	push   %esi
  8000d6:	53                   	push   %ebx
  8000d7:	83 ec 40             	sub    $0x40,%esp
	char buffer[BUFFSIZE];
	unsigned int echolen;
	int received = 0;

	// Create the TCP socket
	if ((serversock = socket(PF_INET, SOCK_STREAM, IPPROTO_TCP)) < 0)
  8000da:	6a 06                	push   $0x6
  8000dc:	6a 01                	push   $0x1
  8000de:	6a 02                	push   $0x2
  8000e0:	e8 37 1b 00 00       	call   801c1c <socket>
  8000e5:	89 c6                	mov    %eax,%esi
  8000e7:	83 c4 10             	add    $0x10,%esp
  8000ea:	85 c0                	test   %eax,%eax
  8000ec:	79 0a                	jns    8000f8 <umain+0x27>
		die("Failed to create socket");
  8000ee:	b8 c0 27 80 00       	mov    $0x8027c0,%eax
  8000f3:	e8 3b ff ff ff       	call   800033 <die>

	cprintf("opened socket\n");
  8000f8:	83 ec 0c             	sub    $0xc,%esp
  8000fb:	68 d8 27 80 00       	push   $0x8027d8
  800100:	e8 c8 04 00 00       	call   8005cd <cprintf>

	// Construct the server sockaddr_in structure
	memset(&echoserver, 0, sizeof(echoserver));       // Clear struct
  800105:	83 c4 0c             	add    $0xc,%esp
  800108:	6a 10                	push   $0x10
  80010a:	6a 00                	push   $0x0
  80010c:	8d 5d d8             	lea    -0x28(%ebp),%ebx
  80010f:	53                   	push   %ebx
  800110:	e8 84 0b 00 00       	call   800c99 <memset>
	echoserver.sin_family = AF_INET;                  // Internet/IP
  800115:	c6 45 d9 02          	movb   $0x2,-0x27(%ebp)
	echoserver.sin_addr.s_addr = htonl(INADDR_ANY);   // IP address
  800119:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
  800120:	e8 6e 01 00 00       	call   800293 <htonl>
  800125:	89 45 dc             	mov    %eax,-0x24(%ebp)
	echoserver.sin_port = htons(PORT);		  // server port
  800128:	c7 04 24 07 00 00 00 	movl   $0x7,(%esp)
  80012f:	e8 45 01 00 00       	call   800279 <htons>
  800134:	66 89 45 da          	mov    %ax,-0x26(%ebp)

	cprintf("trying to bind\n");
  800138:	c7 04 24 e7 27 80 00 	movl   $0x8027e7,(%esp)
  80013f:	e8 89 04 00 00       	call   8005cd <cprintf>

	// Bind the server socket
	if (bind(serversock, (struct sockaddr *) &echoserver,
  800144:	83 c4 0c             	add    $0xc,%esp
  800147:	6a 10                	push   $0x10
  800149:	53                   	push   %ebx
  80014a:	56                   	push   %esi
  80014b:	e8 32 1a 00 00       	call   801b82 <bind>
  800150:	83 c4 10             	add    $0x10,%esp
  800153:	85 c0                	test   %eax,%eax
  800155:	79 0a                	jns    800161 <umain+0x90>
		 sizeof(echoserver)) < 0) {
		die("Failed to bind the server socket");
  800157:	b8 90 28 80 00       	mov    $0x802890,%eax
  80015c:	e8 d2 fe ff ff       	call   800033 <die>
	}

	// Listen on the server socket
	if (listen(serversock, MAXPENDING) < 0)
  800161:	83 ec 08             	sub    $0x8,%esp
  800164:	6a 05                	push   $0x5
  800166:	56                   	push   %esi
  800167:	e8 8b 1a 00 00       	call   801bf7 <listen>
  80016c:	83 c4 10             	add    $0x10,%esp
  80016f:	85 c0                	test   %eax,%eax
  800171:	79 0a                	jns    80017d <umain+0xac>
		die("Failed to listen on server socket");
  800173:	b8 b4 28 80 00       	mov    $0x8028b4,%eax
  800178:	e8 b6 fe ff ff       	call   800033 <die>

	cprintf("bound\n");
  80017d:	83 ec 0c             	sub    $0xc,%esp
  800180:	68 f7 27 80 00       	push   $0x8027f7
  800185:	e8 43 04 00 00       	call   8005cd <cprintf>
  80018a:	83 c4 10             	add    $0x10,%esp

	// Run until canceled
	while (1) {
		unsigned int clientlen = sizeof(echoclient);
		// Wait for client connection
		if ((clientsock =
  80018d:	8d 7d c4             	lea    -0x3c(%ebp),%edi

	cprintf("bound\n");

	// Run until canceled
	while (1) {
		unsigned int clientlen = sizeof(echoclient);
  800190:	c7 45 c4 10 00 00 00 	movl   $0x10,-0x3c(%ebp)
		// Wait for client connection
		if ((clientsock =
  800197:	83 ec 04             	sub    $0x4,%esp
  80019a:	57                   	push   %edi
  80019b:	8d 45 c8             	lea    -0x38(%ebp),%eax
  80019e:	50                   	push   %eax
  80019f:	56                   	push   %esi
  8001a0:	e8 a6 19 00 00       	call   801b4b <accept>
  8001a5:	89 c3                	mov    %eax,%ebx
  8001a7:	83 c4 10             	add    $0x10,%esp
  8001aa:	85 c0                	test   %eax,%eax
  8001ac:	79 0a                	jns    8001b8 <umain+0xe7>
		     accept(serversock, (struct sockaddr *) &echoclient,
			    &clientlen)) < 0) {
			die("Failed to accept client connection");
  8001ae:	b8 d8 28 80 00       	mov    $0x8028d8,%eax
  8001b3:	e8 7b fe ff ff       	call   800033 <die>
		}
		cprintf("Client connected: %s\n", inet_ntoa(echoclient.sin_addr));
  8001b8:	83 ec 0c             	sub    $0xc,%esp
  8001bb:	ff 75 cc             	pushl  -0x34(%ebp)
  8001be:	e8 1b 00 00 00       	call   8001de <inet_ntoa>
  8001c3:	83 c4 08             	add    $0x8,%esp
  8001c6:	50                   	push   %eax
  8001c7:	68 fe 27 80 00       	push   $0x8027fe
  8001cc:	e8 fc 03 00 00       	call   8005cd <cprintf>
		handle_client(clientsock);
  8001d1:	89 1c 24             	mov    %ebx,(%esp)
  8001d4:	e8 75 fe ff ff       	call   80004e <handle_client>
	}
  8001d9:	83 c4 10             	add    $0x10,%esp
  8001dc:	eb b2                	jmp    800190 <umain+0xbf>

008001de <inet_ntoa>:
 * @return pointer to a global static (!) buffer that holds the ASCII
 *         represenation of addr
 */
char *
inet_ntoa(struct in_addr addr)
{
  8001de:	55                   	push   %ebp
  8001df:	89 e5                	mov    %esp,%ebp
  8001e1:	57                   	push   %edi
  8001e2:	56                   	push   %esi
  8001e3:	53                   	push   %ebx
  8001e4:	83 ec 14             	sub    $0x14,%esp
  static char str[16];
  u32_t s_addr = addr.s_addr;
  8001e7:	8b 45 08             	mov    0x8(%ebp),%eax
  8001ea:	89 45 f0             	mov    %eax,-0x10(%ebp)
  u8_t rem;
  u8_t n;
  u8_t i;

  rp = str;
  ap = (u8_t *)&s_addr;
  8001ed:	8d 7d f0             	lea    -0x10(%ebp),%edi
  u8_t *ap;
  u8_t rem;
  u8_t n;
  u8_t i;

  rp = str;
  8001f0:	c7 45 e0 00 40 80 00 	movl   $0x804000,-0x20(%ebp)
  8001f7:	0f b6 1f             	movzbl (%edi),%ebx
  8001fa:	b9 00 00 00 00       	mov    $0x0,%ecx
  ap = (u8_t *)&s_addr;
  for(n = 0; n < 4; n++) {
    i = 0;
    do {
      rem = *ap % (u8_t)10;
  8001ff:	0f b6 d3             	movzbl %bl,%edx
  800202:	8d 04 92             	lea    (%edx,%edx,4),%eax
  800205:	8d 04 c2             	lea    (%edx,%eax,8),%eax
  800208:	8d 04 80             	lea    (%eax,%eax,4),%eax
  80020b:	66 c1 e8 0b          	shr    $0xb,%ax
  80020f:	89 c2                	mov    %eax,%edx
  800211:	8d 04 80             	lea    (%eax,%eax,4),%eax
  800214:	01 c0                	add    %eax,%eax
  800216:	29 c3                	sub    %eax,%ebx
  800218:	89 d8                	mov    %ebx,%eax
      *ap /= (u8_t)10;
  80021a:	89 d3                	mov    %edx,%ebx
      inv[i++] = '0' + rem;
  80021c:	8d 71 01             	lea    0x1(%ecx),%esi
  80021f:	0f b6 c9             	movzbl %cl,%ecx
  800222:	83 c0 30             	add    $0x30,%eax
  800225:	88 44 0d ed          	mov    %al,-0x13(%ebp,%ecx,1)
  800229:	89 f1                	mov    %esi,%ecx
    } while(*ap);
  80022b:	84 d2                	test   %dl,%dl
  80022d:	75 d0                	jne    8001ff <inet_ntoa+0x21>
  80022f:	89 f2                	mov    %esi,%edx
  for(n = 0; n < 4; n++) {
    i = 0;
    do {
      rem = *ap % (u8_t)10;
      *ap /= (u8_t)10;
      inv[i++] = '0' + rem;
  800231:	89 f3                	mov    %esi,%ebx
  800233:	c6 07 00             	movb   $0x0,(%edi)
  800236:	8b 4d e0             	mov    -0x20(%ebp),%ecx
  800239:	eb 0d                	jmp    800248 <inet_ntoa+0x6a>
    } while(*ap);
    while(i--)
      *rp++ = inv[i];
  80023b:	0f b6 c2             	movzbl %dl,%eax
  80023e:	0f b6 44 05 ed       	movzbl -0x13(%ebp,%eax,1),%eax
  800243:	88 01                	mov    %al,(%ecx)
  800245:	83 c1 01             	add    $0x1,%ecx
    do {
      rem = *ap % (u8_t)10;
      *ap /= (u8_t)10;
      inv[i++] = '0' + rem;
    } while(*ap);
    while(i--)
  800248:	83 ea 01             	sub    $0x1,%edx
  80024b:	80 fa ff             	cmp    $0xff,%dl
  80024e:	75 eb                	jne    80023b <inet_ntoa+0x5d>
  800250:	0f b6 db             	movzbl %bl,%ebx
  800253:	03 5d e0             	add    -0x20(%ebp),%ebx
      *rp++ = inv[i];
    *rp++ = '.';
  800256:	8d 43 01             	lea    0x1(%ebx),%eax
  800259:	89 45 e0             	mov    %eax,-0x20(%ebp)
  80025c:	c6 03 2e             	movb   $0x2e,(%ebx)
    ap++;
  80025f:	83 c7 01             	add    $0x1,%edi
  u8_t n;
  u8_t i;

  rp = str;
  ap = (u8_t *)&s_addr;
  for(n = 0; n < 4; n++) {
  800262:	8d 45 f4             	lea    -0xc(%ebp),%eax
  800265:	39 c7                	cmp    %eax,%edi
  800267:	75 8e                	jne    8001f7 <inet_ntoa+0x19>
    while(i--)
      *rp++ = inv[i];
    *rp++ = '.';
    ap++;
  }
  *--rp = 0;
  800269:	c6 03 00             	movb   $0x0,(%ebx)
  return str;
}
  80026c:	b8 00 40 80 00       	mov    $0x804000,%eax
  800271:	83 c4 14             	add    $0x14,%esp
  800274:	5b                   	pop    %ebx
  800275:	5e                   	pop    %esi
  800276:	5f                   	pop    %edi
  800277:	5d                   	pop    %ebp
  800278:	c3                   	ret    

00800279 <htons>:
 * @param n u16_t in host byte order
 * @return n in network byte order
 */
u16_t
htons(u16_t n)
{
  800279:	55                   	push   %ebp
  80027a:	89 e5                	mov    %esp,%ebp
  return ((n & 0xff) << 8) | ((n & 0xff00) >> 8);
  80027c:	0f b7 45 08          	movzwl 0x8(%ebp),%eax
  800280:	66 c1 c0 08          	rol    $0x8,%ax
}
  800284:	5d                   	pop    %ebp
  800285:	c3                   	ret    

00800286 <ntohs>:
 * @param n u16_t in network byte order
 * @return n in host byte order
 */
u16_t
ntohs(u16_t n)
{
  800286:	55                   	push   %ebp
  800287:	89 e5                	mov    %esp,%ebp
 * @return n in network byte order
 */
u16_t
htons(u16_t n)
{
  return ((n & 0xff) << 8) | ((n & 0xff00) >> 8);
  800289:	0f b7 45 08          	movzwl 0x8(%ebp),%eax
  80028d:	66 c1 c0 08          	rol    $0x8,%ax
 */
u16_t
ntohs(u16_t n)
{
  return htons(n);
}
  800291:	5d                   	pop    %ebp
  800292:	c3                   	ret    

00800293 <htonl>:
 * @param n u32_t in host byte order
 * @return n in network byte order
 */
u32_t
htonl(u32_t n)
{
  800293:	55                   	push   %ebp
  800294:	89 e5                	mov    %esp,%ebp
  800296:	8b 55 08             	mov    0x8(%ebp),%edx
  return ((n & 0xff) << 24) |
    ((n & 0xff00) << 8) |
    ((n & 0xff0000UL) >> 8) |
    ((n & 0xff000000UL) >> 24);
  800299:	89 d1                	mov    %edx,%ecx
  80029b:	c1 e9 18             	shr    $0x18,%ecx
 * @return n in network byte order
 */
u32_t
htonl(u32_t n)
{
  return ((n & 0xff) << 24) |
  80029e:	89 d0                	mov    %edx,%eax
  8002a0:	c1 e0 18             	shl    $0x18,%eax
  8002a3:	09 c8                	or     %ecx,%eax
    ((n & 0xff00) << 8) |
  8002a5:	89 d1                	mov    %edx,%ecx
  8002a7:	81 e1 00 ff 00 00    	and    $0xff00,%ecx
  8002ad:	c1 e1 08             	shl    $0x8,%ecx
  8002b0:	09 c8                	or     %ecx,%eax
    ((n & 0xff0000UL) >> 8) |
  8002b2:	81 e2 00 00 ff 00    	and    $0xff0000,%edx
  8002b8:	c1 ea 08             	shr    $0x8,%edx
 * @return n in network byte order
 */
u32_t
htonl(u32_t n)
{
  return ((n & 0xff) << 24) |
  8002bb:	09 d0                	or     %edx,%eax
    ((n & 0xff00) << 8) |
    ((n & 0xff0000UL) >> 8) |
    ((n & 0xff000000UL) >> 24);
}
  8002bd:	5d                   	pop    %ebp
  8002be:	c3                   	ret    

008002bf <inet_aton>:
 * @param addr pointer to which to save the ip address in network order
 * @return 1 if cp could be converted to addr, 0 on failure
 */
int
inet_aton(const char *cp, struct in_addr *addr)
{
  8002bf:	55                   	push   %ebp
  8002c0:	89 e5                	mov    %esp,%ebp
  8002c2:	57                   	push   %edi
  8002c3:	56                   	push   %esi
  8002c4:	53                   	push   %ebx
  8002c5:	83 ec 1c             	sub    $0x1c,%esp
  8002c8:	8b 45 08             	mov    0x8(%ebp),%eax
  u32_t val;
  int base, n, c;
  u32_t parts[4];
  u32_t *pp = parts;

  c = *cp;
  8002cb:	0f be 10             	movsbl (%eax),%edx
inet_aton(const char *cp, struct in_addr *addr)
{
  u32_t val;
  int base, n, c;
  u32_t parts[4];
  u32_t *pp = parts;
  8002ce:	8d 75 e4             	lea    -0x1c(%ebp),%esi
  8002d1:	89 75 d8             	mov    %esi,-0x28(%ebp)
    /*
     * Collect number up to ``.''.
     * Values are specified as for C:
     * 0x=hex, 0=octal, 1-9=decimal.
     */
    if (!isdigit(c))
  8002d4:	8d 4a d0             	lea    -0x30(%edx),%ecx
  8002d7:	80 f9 09             	cmp    $0x9,%cl
  8002da:	0f 87 a6 01 00 00    	ja     800486 <inet_aton+0x1c7>
      return (0);
    val = 0;
    base = 10;
  8002e0:	c7 45 e0 0a 00 00 00 	movl   $0xa,-0x20(%ebp)
    if (c == '0') {
  8002e7:	83 fa 30             	cmp    $0x30,%edx
  8002ea:	75 2b                	jne    800317 <inet_aton+0x58>
      c = *++cp;
  8002ec:	0f b6 50 01          	movzbl 0x1(%eax),%edx
      if (c == 'x' || c == 'X') {
  8002f0:	89 d1                	mov    %edx,%ecx
  8002f2:	83 e1 df             	and    $0xffffffdf,%ecx
  8002f5:	80 f9 58             	cmp    $0x58,%cl
  8002f8:	74 0f                	je     800309 <inet_aton+0x4a>
    if (!isdigit(c))
      return (0);
    val = 0;
    base = 10;
    if (c == '0') {
      c = *++cp;
  8002fa:	83 c0 01             	add    $0x1,%eax
  8002fd:	0f be d2             	movsbl %dl,%edx
      if (c == 'x' || c == 'X') {
        base = 16;
        c = *++cp;
      } else
        base = 8;
  800300:	c7 45 e0 08 00 00 00 	movl   $0x8,-0x20(%ebp)
  800307:	eb 0e                	jmp    800317 <inet_aton+0x58>
    base = 10;
    if (c == '0') {
      c = *++cp;
      if (c == 'x' || c == 'X') {
        base = 16;
        c = *++cp;
  800309:	0f be 50 02          	movsbl 0x2(%eax),%edx
  80030d:	8d 40 02             	lea    0x2(%eax),%eax
    val = 0;
    base = 10;
    if (c == '0') {
      c = *++cp;
      if (c == 'x' || c == 'X') {
        base = 16;
  800310:	c7 45 e0 10 00 00 00 	movl   $0x10,-0x20(%ebp)
  800317:	83 c0 01             	add    $0x1,%eax
  80031a:	bf 00 00 00 00       	mov    $0x0,%edi
  80031f:	eb 03                	jmp    800324 <inet_aton+0x65>
  800321:	83 c0 01             	add    $0x1,%eax
  800324:	8d 58 ff             	lea    -0x1(%eax),%ebx
        c = *++cp;
      } else
        base = 8;
    }
    for (;;) {
      if (isdigit(c)) {
  800327:	89 d6                	mov    %edx,%esi
  800329:	8d 4a d0             	lea    -0x30(%edx),%ecx
  80032c:	80 f9 09             	cmp    $0x9,%cl
  80032f:	77 0d                	ja     80033e <inet_aton+0x7f>
        val = (val * base) + (int)(c - '0');
  800331:	0f af 7d e0          	imul   -0x20(%ebp),%edi
  800335:	8d 7c 3a d0          	lea    -0x30(%edx,%edi,1),%edi
        c = *++cp;
  800339:	0f be 10             	movsbl (%eax),%edx
  80033c:	eb e3                	jmp    800321 <inet_aton+0x62>
      } else if (base == 16 && isxdigit(c)) {
  80033e:	83 7d e0 10          	cmpl   $0x10,-0x20(%ebp)
  800342:	75 2e                	jne    800372 <inet_aton+0xb3>
  800344:	8d 4e 9f             	lea    -0x61(%esi),%ecx
  800347:	88 4d df             	mov    %cl,-0x21(%ebp)
  80034a:	89 d1                	mov    %edx,%ecx
  80034c:	83 e1 df             	and    $0xffffffdf,%ecx
  80034f:	83 e9 41             	sub    $0x41,%ecx
  800352:	80 f9 05             	cmp    $0x5,%cl
  800355:	77 21                	ja     800378 <inet_aton+0xb9>
        val = (val << 4) | (int)(c + 10 - (islower(c) ? 'a' : 'A'));
  800357:	c1 e7 04             	shl    $0x4,%edi
  80035a:	83 c2 0a             	add    $0xa,%edx
  80035d:	80 7d df 1a          	cmpb   $0x1a,-0x21(%ebp)
  800361:	19 c9                	sbb    %ecx,%ecx
  800363:	83 e1 20             	and    $0x20,%ecx
  800366:	83 c1 41             	add    $0x41,%ecx
  800369:	29 ca                	sub    %ecx,%edx
  80036b:	09 d7                	or     %edx,%edi
        c = *++cp;
  80036d:	0f be 10             	movsbl (%eax),%edx
  800370:	eb af                	jmp    800321 <inet_aton+0x62>
  800372:	89 d0                	mov    %edx,%eax
  800374:	89 f9                	mov    %edi,%ecx
  800376:	eb 04                	jmp    80037c <inet_aton+0xbd>
  800378:	89 d0                	mov    %edx,%eax
  80037a:	89 f9                	mov    %edi,%ecx
      } else
        break;
    }
    if (c == '.') {
  80037c:	83 f8 2e             	cmp    $0x2e,%eax
  80037f:	75 23                	jne    8003a4 <inet_aton+0xe5>
       * Internet format:
       *  a.b.c.d
       *  a.b.c   (with c treated as 16 bits)
       *  a.b (with b treated as 24 bits)
       */
      if (pp >= parts + 3)
  800381:	8b 45 d8             	mov    -0x28(%ebp),%eax
  800384:	8d 75 f0             	lea    -0x10(%ebp),%esi
  800387:	39 f0                	cmp    %esi,%eax
  800389:	0f 84 fe 00 00 00    	je     80048d <inet_aton+0x1ce>
        return (0);
      *pp++ = val;
  80038f:	83 c0 04             	add    $0x4,%eax
  800392:	89 45 d8             	mov    %eax,-0x28(%ebp)
  800395:	89 48 fc             	mov    %ecx,-0x4(%eax)
      c = *++cp;
  800398:	8d 43 01             	lea    0x1(%ebx),%eax
  80039b:	0f be 53 01          	movsbl 0x1(%ebx),%edx
    } else
      break;
  }
  80039f:	e9 30 ff ff ff       	jmp    8002d4 <inet_aton+0x15>
  8003a4:	89 f9                	mov    %edi,%ecx
  /*
   * Check for trailing characters.
   */
  if (c != '\0' && (!isprint(c) || !isspace(c)))
  8003a6:	85 d2                	test   %edx,%edx
  8003a8:	74 29                	je     8003d3 <inet_aton+0x114>
    return (0);
  8003aa:	b8 00 00 00 00       	mov    $0x0,%eax
      break;
  }
  /*
   * Check for trailing characters.
   */
  if (c != '\0' && (!isprint(c) || !isspace(c)))
  8003af:	89 f3                	mov    %esi,%ebx
  8003b1:	80 fb 1f             	cmp    $0x1f,%bl
  8003b4:	0f 86 e6 00 00 00    	jbe    8004a0 <inet_aton+0x1e1>
  8003ba:	84 d2                	test   %dl,%dl
  8003bc:	0f 88 d2 00 00 00    	js     800494 <inet_aton+0x1d5>
  8003c2:	83 fa 20             	cmp    $0x20,%edx
  8003c5:	74 0c                	je     8003d3 <inet_aton+0x114>
  8003c7:	83 ea 09             	sub    $0x9,%edx
  8003ca:	83 fa 04             	cmp    $0x4,%edx
  8003cd:	0f 87 cd 00 00 00    	ja     8004a0 <inet_aton+0x1e1>
    return (0);
  /*
   * Concoct the address according to
   * the number of parts specified.
   */
  n = pp - parts + 1;
  8003d3:	8d 45 e4             	lea    -0x1c(%ebp),%eax
  8003d6:	8b 55 d8             	mov    -0x28(%ebp),%edx
  8003d9:	29 c2                	sub    %eax,%edx
  8003db:	c1 fa 02             	sar    $0x2,%edx
  8003de:	83 c2 01             	add    $0x1,%edx
  switch (n) {
  8003e1:	83 fa 02             	cmp    $0x2,%edx
  8003e4:	74 20                	je     800406 <inet_aton+0x147>
  8003e6:	83 fa 02             	cmp    $0x2,%edx
  8003e9:	7f 0f                	jg     8003fa <inet_aton+0x13b>

  case 0:
    return (0);       /* initial nondigit */
  8003eb:	b8 00 00 00 00       	mov    $0x0,%eax
  /*
   * Concoct the address according to
   * the number of parts specified.
   */
  n = pp - parts + 1;
  switch (n) {
  8003f0:	85 d2                	test   %edx,%edx
  8003f2:	0f 84 a8 00 00 00    	je     8004a0 <inet_aton+0x1e1>
  8003f8:	eb 71                	jmp    80046b <inet_aton+0x1ac>
  8003fa:	83 fa 03             	cmp    $0x3,%edx
  8003fd:	74 24                	je     800423 <inet_aton+0x164>
  8003ff:	83 fa 04             	cmp    $0x4,%edx
  800402:	74 40                	je     800444 <inet_aton+0x185>
  800404:	eb 65                	jmp    80046b <inet_aton+0x1ac>
  case 1:             /* a -- 32 bits */
    break;

  case 2:             /* a.b -- 8.24 bits */
    if (val > 0xffffffUL)
      return (0);
  800406:	b8 00 00 00 00       	mov    $0x0,%eax

  case 1:             /* a -- 32 bits */
    break;

  case 2:             /* a.b -- 8.24 bits */
    if (val > 0xffffffUL)
  80040b:	81 f9 ff ff ff 00    	cmp    $0xffffff,%ecx
  800411:	0f 87 89 00 00 00    	ja     8004a0 <inet_aton+0x1e1>
      return (0);
    val |= parts[0] << 24;
  800417:	8b 45 e4             	mov    -0x1c(%ebp),%eax
  80041a:	c1 e0 18             	shl    $0x18,%eax
  80041d:	89 cf                	mov    %ecx,%edi
  80041f:	09 c7                	or     %eax,%edi
    break;
  800421:	eb 48                	jmp    80046b <inet_aton+0x1ac>

  case 3:             /* a.b.c -- 8.8.16 bits */
    if (val > 0xffff)
      return (0);
  800423:	b8 00 00 00 00       	mov    $0x0,%eax
      return (0);
    val |= parts[0] << 24;
    break;

  case 3:             /* a.b.c -- 8.8.16 bits */
    if (val > 0xffff)
  800428:	81 f9 ff ff 00 00    	cmp    $0xffff,%ecx
  80042e:	77 70                	ja     8004a0 <inet_aton+0x1e1>
      return (0);
    val |= (parts[0] << 24) | (parts[1] << 16);
  800430:	8b 55 e8             	mov    -0x18(%ebp),%edx
  800433:	c1 e2 10             	shl    $0x10,%edx
  800436:	8b 45 e4             	mov    -0x1c(%ebp),%eax
  800439:	c1 e0 18             	shl    $0x18,%eax
  80043c:	09 d0                	or     %edx,%eax
  80043e:	09 c8                	or     %ecx,%eax
  800440:	89 c7                	mov    %eax,%edi
    break;
  800442:	eb 27                	jmp    80046b <inet_aton+0x1ac>

  case 4:             /* a.b.c.d -- 8.8.8.8 bits */
    if (val > 0xff)
      return (0);
  800444:	b8 00 00 00 00       	mov    $0x0,%eax
      return (0);
    val |= (parts[0] << 24) | (parts[1] << 16);
    break;

  case 4:             /* a.b.c.d -- 8.8.8.8 bits */
    if (val > 0xff)
  800449:	81 f9 ff 00 00 00    	cmp    $0xff,%ecx
  80044f:	77 4f                	ja     8004a0 <inet_aton+0x1e1>
      return (0);
    val |= (parts[0] << 24) | (parts[1] << 16) | (parts[2] << 8);
  800451:	8b 55 e8             	mov    -0x18(%ebp),%edx
  800454:	c1 e2 10             	shl    $0x10,%edx
  800457:	8b 45 e4             	mov    -0x1c(%ebp),%eax
  80045a:	c1 e0 18             	shl    $0x18,%eax
  80045d:	09 c2                	or     %eax,%edx
  80045f:	8b 45 ec             	mov    -0x14(%ebp),%eax
  800462:	c1 e0 08             	shl    $0x8,%eax
  800465:	09 d0                	or     %edx,%eax
  800467:	09 c8                	or     %ecx,%eax
  800469:	89 c7                	mov    %eax,%edi
    break;
  }
  if (addr)
  80046b:	83 7d 0c 00          	cmpl   $0x0,0xc(%ebp)
  80046f:	74 2a                	je     80049b <inet_aton+0x1dc>
    addr->s_addr = htonl(val);
  800471:	57                   	push   %edi
  800472:	e8 1c fe ff ff       	call   800293 <htonl>
  800477:	83 c4 04             	add    $0x4,%esp
  80047a:	8b 75 0c             	mov    0xc(%ebp),%esi
  80047d:	89 06                	mov    %eax,(%esi)
  return (1);
  80047f:	b8 01 00 00 00       	mov    $0x1,%eax
  800484:	eb 1a                	jmp    8004a0 <inet_aton+0x1e1>
     * Collect number up to ``.''.
     * Values are specified as for C:
     * 0x=hex, 0=octal, 1-9=decimal.
     */
    if (!isdigit(c))
      return (0);
  800486:	b8 00 00 00 00       	mov    $0x0,%eax
  80048b:	eb 13                	jmp    8004a0 <inet_aton+0x1e1>
       *  a.b.c.d
       *  a.b.c   (with c treated as 16 bits)
       *  a.b (with b treated as 24 bits)
       */
      if (pp >= parts + 3)
        return (0);
  80048d:	b8 00 00 00 00       	mov    $0x0,%eax
  800492:	eb 0c                	jmp    8004a0 <inet_aton+0x1e1>
  }
  /*
   * Check for trailing characters.
   */
  if (c != '\0' && (!isprint(c) || !isspace(c)))
    return (0);
  800494:	b8 00 00 00 00       	mov    $0x0,%eax
  800499:	eb 05                	jmp    8004a0 <inet_aton+0x1e1>
    val |= (parts[0] << 24) | (parts[1] << 16) | (parts[2] << 8);
    break;
  }
  if (addr)
    addr->s_addr = htonl(val);
  return (1);
  80049b:	b8 01 00 00 00       	mov    $0x1,%eax
}
  8004a0:	8d 65 f4             	lea    -0xc(%ebp),%esp
  8004a3:	5b                   	pop    %ebx
  8004a4:	5e                   	pop    %esi
  8004a5:	5f                   	pop    %edi
  8004a6:	5d                   	pop    %ebp
  8004a7:	c3                   	ret    

008004a8 <inet_addr>:
 * @param cp IP address in ascii represenation (e.g. "127.0.0.1")
 * @return ip address in network order
 */
u32_t
inet_addr(const char *cp)
{
  8004a8:	55                   	push   %ebp
  8004a9:	89 e5                	mov    %esp,%ebp
  8004ab:	83 ec 10             	sub    $0x10,%esp
  struct in_addr val;

  if (inet_aton(cp, &val)) {
  8004ae:	8d 45 fc             	lea    -0x4(%ebp),%eax
  8004b1:	50                   	push   %eax
  8004b2:	ff 75 08             	pushl  0x8(%ebp)
  8004b5:	e8 05 fe ff ff       	call   8002bf <inet_aton>
  8004ba:	83 c4 08             	add    $0x8,%esp
    return (val.s_addr);
  8004bd:	85 c0                	test   %eax,%eax
  8004bf:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
  8004c4:	0f 45 45 fc          	cmovne -0x4(%ebp),%eax
  }
  return (INADDR_NONE);
}
  8004c8:	c9                   	leave  
  8004c9:	c3                   	ret    

008004ca <ntohl>:
 * @param n u32_t in network byte order
 * @return n in host byte order
 */
u32_t
ntohl(u32_t n)
{
  8004ca:	55                   	push   %ebp
  8004cb:	89 e5                	mov    %esp,%ebp
  return htonl(n);
  8004cd:	ff 75 08             	pushl  0x8(%ebp)
  8004d0:	e8 be fd ff ff       	call   800293 <htonl>
  8004d5:	83 c4 04             	add    $0x4,%esp
}
  8004d8:	c9                   	leave  
  8004d9:	c3                   	ret    

008004da <libmain>:
const volatile struct Env *thisenv;
const char *binaryname = "<unknown>";

void
libmain(int argc, char **argv)
{
  8004da:	55                   	push   %ebp
  8004db:	89 e5                	mov    %esp,%ebp
  8004dd:	56                   	push   %esi
  8004de:	53                   	push   %ebx
  8004df:	8b 5d 08             	mov    0x8(%ebp),%ebx
  8004e2:	8b 75 0c             	mov    0xc(%ebp),%esi
	// set thisenv to point at our Env structure in envs[].
	// LAB 3: Your code here.
	thisenv = envs+ENVX(sys_getenvid());
  8004e5:	e8 35 0a 00 00       	call   800f1f <sys_getenvid>
  8004ea:	25 ff 03 00 00       	and    $0x3ff,%eax
  8004ef:	6b c0 7c             	imul   $0x7c,%eax,%eax
  8004f2:	05 00 00 c0 ee       	add    $0xeec00000,%eax
  8004f7:	a3 18 40 80 00       	mov    %eax,0x804018

	// save the name of the program so that panic() can use it
	if (argc > 0)
  8004fc:	85 db                	test   %ebx,%ebx
  8004fe:	7e 07                	jle    800507 <libmain+0x2d>
		binaryname = argv[0];
  800500:	8b 06                	mov    (%esi),%eax
  800502:	a3 00 30 80 00       	mov    %eax,0x803000

	// call user main routine
	umain(argc, argv);
  800507:	83 ec 08             	sub    $0x8,%esp
  80050a:	56                   	push   %esi
  80050b:	53                   	push   %ebx
  80050c:	e8 c0 fb ff ff       	call   8000d1 <umain>

	// exit gracefully
	exit();
  800511:	e8 0a 00 00 00       	call   800520 <exit>
  800516:	83 c4 10             	add    $0x10,%esp
}
  800519:	8d 65 f8             	lea    -0x8(%ebp),%esp
  80051c:	5b                   	pop    %ebx
  80051d:	5e                   	pop    %esi
  80051e:	5d                   	pop    %ebp
  80051f:	c3                   	ret    

00800520 <exit>:

#include <inc/lib.h>

void
exit(void)
{
  800520:	55                   	push   %ebp
  800521:	89 e5                	mov    %esp,%ebp
  800523:	83 ec 08             	sub    $0x8,%esp
	close_all();
  800526:	e8 96 0e 00 00       	call   8013c1 <close_all>
	sys_env_destroy(0);
  80052b:	83 ec 0c             	sub    $0xc,%esp
  80052e:	6a 00                	push   $0x0
  800530:	e8 a9 09 00 00       	call   800ede <sys_env_destroy>
  800535:	83 c4 10             	add    $0x10,%esp
}
  800538:	c9                   	leave  
  800539:	c3                   	ret    

0080053a <putch>:
};


static void
putch(int ch, struct printbuf *b)
{
  80053a:	55                   	push   %ebp
  80053b:	89 e5                	mov    %esp,%ebp
  80053d:	53                   	push   %ebx
  80053e:	83 ec 04             	sub    $0x4,%esp
  800541:	8b 5d 0c             	mov    0xc(%ebp),%ebx
	b->buf[b->idx++] = ch;
  800544:	8b 13                	mov    (%ebx),%edx
  800546:	8d 42 01             	lea    0x1(%edx),%eax
  800549:	89 03                	mov    %eax,(%ebx)
  80054b:	8b 4d 08             	mov    0x8(%ebp),%ecx
  80054e:	88 4c 13 08          	mov    %cl,0x8(%ebx,%edx,1)
	if (b->idx == 256-1) {
  800552:	3d ff 00 00 00       	cmp    $0xff,%eax
  800557:	75 1a                	jne    800573 <putch+0x39>
		sys_cputs(b->buf, b->idx);
  800559:	83 ec 08             	sub    $0x8,%esp
  80055c:	68 ff 00 00 00       	push   $0xff
  800561:	8d 43 08             	lea    0x8(%ebx),%eax
  800564:	50                   	push   %eax
  800565:	e8 37 09 00 00       	call   800ea1 <sys_cputs>
		b->idx = 0;
  80056a:	c7 03 00 00 00 00    	movl   $0x0,(%ebx)
  800570:	83 c4 10             	add    $0x10,%esp
	}
	b->cnt++;
  800573:	83 43 04 01          	addl   $0x1,0x4(%ebx)
}
  800577:	8b 5d fc             	mov    -0x4(%ebp),%ebx
  80057a:	c9                   	leave  
  80057b:	c3                   	ret    

0080057c <vcprintf>:

int
vcprintf(const char *fmt, va_list ap)
{
  80057c:	55                   	push   %ebp
  80057d:	89 e5                	mov    %esp,%ebp
  80057f:	81 ec 18 01 00 00    	sub    $0x118,%esp
	struct printbuf b;

	b.idx = 0;
  800585:	c7 85 f0 fe ff ff 00 	movl   $0x0,-0x110(%ebp)
  80058c:	00 00 00 
	b.cnt = 0;
  80058f:	c7 85 f4 fe ff ff 00 	movl   $0x0,-0x10c(%ebp)
  800596:	00 00 00 
	vprintfmt((void*)putch, &b, fmt, ap);
  800599:	ff 75 0c             	pushl  0xc(%ebp)
  80059c:	ff 75 08             	pushl  0x8(%ebp)
  80059f:	8d 85 f0 fe ff ff    	lea    -0x110(%ebp),%eax
  8005a5:	50                   	push   %eax
  8005a6:	68 3a 05 80 00       	push   $0x80053a
  8005ab:	e8 4f 01 00 00       	call   8006ff <vprintfmt>
	sys_cputs(b.buf, b.idx);
  8005b0:	83 c4 08             	add    $0x8,%esp
  8005b3:	ff b5 f0 fe ff ff    	pushl  -0x110(%ebp)
  8005b9:	8d 85 f8 fe ff ff    	lea    -0x108(%ebp),%eax
  8005bf:	50                   	push   %eax
  8005c0:	e8 dc 08 00 00       	call   800ea1 <sys_cputs>

	return b.cnt;
}
  8005c5:	8b 85 f4 fe ff ff    	mov    -0x10c(%ebp),%eax
  8005cb:	c9                   	leave  
  8005cc:	c3                   	ret    

008005cd <cprintf>:

int
cprintf(const char *fmt, ...)
{
  8005cd:	55                   	push   %ebp
  8005ce:	89 e5                	mov    %esp,%ebp
  8005d0:	83 ec 10             	sub    $0x10,%esp
	va_list ap;
	int cnt;

	va_start(ap, fmt);
  8005d3:	8d 45 0c             	lea    0xc(%ebp),%eax
	cnt = vcprintf(fmt, ap);
  8005d6:	50                   	push   %eax
  8005d7:	ff 75 08             	pushl  0x8(%ebp)
  8005da:	e8 9d ff ff ff       	call   80057c <vcprintf>
	va_end(ap);

	return cnt;
}
  8005df:	c9                   	leave  
  8005e0:	c3                   	ret    

008005e1 <printnum>:
 * using specified putch function and associated pointer putdat.
 */
static void
printnum(void (*putch)(int, void*), void *putdat,
	 unsigned long long num, unsigned base, int width, int padc)
{
  8005e1:	55                   	push   %ebp
  8005e2:	89 e5                	mov    %esp,%ebp
  8005e4:	57                   	push   %edi
  8005e5:	56                   	push   %esi
  8005e6:	53                   	push   %ebx
  8005e7:	83 ec 1c             	sub    $0x1c,%esp
  8005ea:	89 c7                	mov    %eax,%edi
  8005ec:	89 d6                	mov    %edx,%esi
  8005ee:	8b 45 08             	mov    0x8(%ebp),%eax
  8005f1:	8b 55 0c             	mov    0xc(%ebp),%edx
  8005f4:	89 d1                	mov    %edx,%ecx
  8005f6:	89 45 d8             	mov    %eax,-0x28(%ebp)
  8005f9:	89 4d dc             	mov    %ecx,-0x24(%ebp)
  8005fc:	8b 45 10             	mov    0x10(%ebp),%eax
  8005ff:	8b 5d 14             	mov    0x14(%ebp),%ebx
	// first recursively print all preceding (more significant) digits
	if (num >= base) {
  800602:	89 45 e0             	mov    %eax,-0x20(%ebp)
  800605:	c7 45 e4 00 00 00 00 	movl   $0x0,-0x1c(%ebp)
  80060c:	39 4d e4             	cmp    %ecx,-0x1c(%ebp)
  80060f:	72 05                	jb     800616 <printnum+0x35>
  800611:	3b 45 d8             	cmp    -0x28(%ebp),%eax
  800614:	77 3e                	ja     800654 <printnum+0x73>
		printnum(putch, putdat, num / base, base, width - 1, padc);
  800616:	83 ec 0c             	sub    $0xc,%esp
  800619:	ff 75 18             	pushl  0x18(%ebp)
  80061c:	83 eb 01             	sub    $0x1,%ebx
  80061f:	53                   	push   %ebx
  800620:	50                   	push   %eax
  800621:	83 ec 08             	sub    $0x8,%esp
  800624:	ff 75 e4             	pushl  -0x1c(%ebp)
  800627:	ff 75 e0             	pushl  -0x20(%ebp)
  80062a:	ff 75 dc             	pushl  -0x24(%ebp)
  80062d:	ff 75 d8             	pushl  -0x28(%ebp)
  800630:	e8 bb 1e 00 00       	call   8024f0 <__udivdi3>
  800635:	83 c4 18             	add    $0x18,%esp
  800638:	52                   	push   %edx
  800639:	50                   	push   %eax
  80063a:	89 f2                	mov    %esi,%edx
  80063c:	89 f8                	mov    %edi,%eax
  80063e:	e8 9e ff ff ff       	call   8005e1 <printnum>
  800643:	83 c4 20             	add    $0x20,%esp
  800646:	eb 13                	jmp    80065b <printnum+0x7a>
	} else {
		// print any needed pad characters before first digit
		while (--width > 0)
			putch(padc, putdat);
  800648:	83 ec 08             	sub    $0x8,%esp
  80064b:	56                   	push   %esi
  80064c:	ff 75 18             	pushl  0x18(%ebp)
  80064f:	ff d7                	call   *%edi
  800651:	83 c4 10             	add    $0x10,%esp
	// first recursively print all preceding (more significant) digits
	if (num >= base) {
		printnum(putch, putdat, num / base, base, width - 1, padc);
	} else {
		// print any needed pad characters before first digit
		while (--width > 0)
  800654:	83 eb 01             	sub    $0x1,%ebx
  800657:	85 db                	test   %ebx,%ebx
  800659:	7f ed                	jg     800648 <printnum+0x67>
			putch(padc, putdat);
	}

	// then print this (the least significant) digit
	putch("0123456789abcdef"[num % base], putdat);
  80065b:	83 ec 08             	sub    $0x8,%esp
  80065e:	56                   	push   %esi
  80065f:	83 ec 04             	sub    $0x4,%esp
  800662:	ff 75 e4             	pushl  -0x1c(%ebp)
  800665:	ff 75 e0             	pushl  -0x20(%ebp)
  800668:	ff 75 dc             	pushl  -0x24(%ebp)
  80066b:	ff 75 d8             	pushl  -0x28(%ebp)
  80066e:	e8 ad 1f 00 00       	call   802620 <__umoddi3>
  800673:	83 c4 14             	add    $0x14,%esp
  800676:	0f be 80 05 29 80 00 	movsbl 0x802905(%eax),%eax
  80067d:	50                   	push   %eax
  80067e:	ff d7                	call   *%edi
  800680:	83 c4 10             	add    $0x10,%esp
}
  800683:	8d 65 f4             	lea    -0xc(%ebp),%esp
  800686:	5b                   	pop    %ebx
  800687:	5e                   	pop    %esi
  800688:	5f                   	pop    %edi
  800689:	5d                   	pop    %ebp
  80068a:	c3                   	ret    

0080068b <getuint>:

// Get an unsigned int of various possible sizes from a varargs list,
// depending on the lflag parameter.
static unsigned long long
getuint(va_list *ap, int lflag)
{
  80068b:	55                   	push   %ebp
  80068c:	89 e5                	mov    %esp,%ebp
	if (lflag >= 2)
  80068e:	83 fa 01             	cmp    $0x1,%edx
  800691:	7e 0e                	jle    8006a1 <getuint+0x16>
		return va_arg(*ap, unsigned long long);
  800693:	8b 10                	mov    (%eax),%edx
  800695:	8d 4a 08             	lea    0x8(%edx),%ecx
  800698:	89 08                	mov    %ecx,(%eax)
  80069a:	8b 02                	mov    (%edx),%eax
  80069c:	8b 52 04             	mov    0x4(%edx),%edx
  80069f:	eb 22                	jmp    8006c3 <getuint+0x38>
	else if (lflag)
  8006a1:	85 d2                	test   %edx,%edx
  8006a3:	74 10                	je     8006b5 <getuint+0x2a>
		return va_arg(*ap, unsigned long);
  8006a5:	8b 10                	mov    (%eax),%edx
  8006a7:	8d 4a 04             	lea    0x4(%edx),%ecx
  8006aa:	89 08                	mov    %ecx,(%eax)
  8006ac:	8b 02                	mov    (%edx),%eax
  8006ae:	ba 00 00 00 00       	mov    $0x0,%edx
  8006b3:	eb 0e                	jmp    8006c3 <getuint+0x38>
	else
		return va_arg(*ap, unsigned int);
  8006b5:	8b 10                	mov    (%eax),%edx
  8006b7:	8d 4a 04             	lea    0x4(%edx),%ecx
  8006ba:	89 08                	mov    %ecx,(%eax)
  8006bc:	8b 02                	mov    (%edx),%eax
  8006be:	ba 00 00 00 00       	mov    $0x0,%edx
}
  8006c3:	5d                   	pop    %ebp
  8006c4:	c3                   	ret    

008006c5 <sprintputch>:
	int cnt;
};

static void
sprintputch(int ch, struct sprintbuf *b)
{
  8006c5:	55                   	push   %ebp
  8006c6:	89 e5                	mov    %esp,%ebp
  8006c8:	8b 45 0c             	mov    0xc(%ebp),%eax
	b->cnt++;
  8006cb:	83 40 08 01          	addl   $0x1,0x8(%eax)
	if (b->buf < b->ebuf)
  8006cf:	8b 10                	mov    (%eax),%edx
  8006d1:	3b 50 04             	cmp    0x4(%eax),%edx
  8006d4:	73 0a                	jae    8006e0 <sprintputch+0x1b>
		*b->buf++ = ch;
  8006d6:	8d 4a 01             	lea    0x1(%edx),%ecx
  8006d9:	89 08                	mov    %ecx,(%eax)
  8006db:	8b 45 08             	mov    0x8(%ebp),%eax
  8006de:	88 02                	mov    %al,(%edx)
}
  8006e0:	5d                   	pop    %ebp
  8006e1:	c3                   	ret    

008006e2 <printfmt>:
	}
}

void
printfmt(void (*putch)(int, void*), void *putdat, const char *fmt, ...)
{
  8006e2:	55                   	push   %ebp
  8006e3:	89 e5                	mov    %esp,%ebp
  8006e5:	83 ec 08             	sub    $0x8,%esp
	va_list ap;

	va_start(ap, fmt);
  8006e8:	8d 45 14             	lea    0x14(%ebp),%eax
	vprintfmt(putch, putdat, fmt, ap);
  8006eb:	50                   	push   %eax
  8006ec:	ff 75 10             	pushl  0x10(%ebp)
  8006ef:	ff 75 0c             	pushl  0xc(%ebp)
  8006f2:	ff 75 08             	pushl  0x8(%ebp)
  8006f5:	e8 05 00 00 00       	call   8006ff <vprintfmt>
	va_end(ap);
  8006fa:	83 c4 10             	add    $0x10,%esp
}
  8006fd:	c9                   	leave  
  8006fe:	c3                   	ret    

008006ff <vprintfmt>:
// Main function to format and print a string.
void printfmt(void (*putch)(int, void*), void *putdat, const char *fmt, ...);

void
vprintfmt(void (*putch)(int, void*), void *putdat, const char *fmt, va_list ap)
{
  8006ff:	55                   	push   %ebp
  800700:	89 e5                	mov    %esp,%ebp
  800702:	57                   	push   %edi
  800703:	56                   	push   %esi
  800704:	53                   	push   %ebx
  800705:	83 ec 2c             	sub    $0x2c,%esp
  800708:	8b 75 08             	mov    0x8(%ebp),%esi
  80070b:	8b 5d 0c             	mov    0xc(%ebp),%ebx
  80070e:	8b 7d 10             	mov    0x10(%ebp),%edi
  800711:	eb 12                	jmp    800725 <vprintfmt+0x26>
	int base, lflag, width, precision, altflag;
	char padc;

	while (1) {
		while ((ch = *(unsigned char *) fmt++) != '%') {
			if (ch == '\0')
  800713:	85 c0                	test   %eax,%eax
  800715:	0f 84 90 03 00 00    	je     800aab <vprintfmt+0x3ac>
				return;
			putch(ch, putdat);
  80071b:	83 ec 08             	sub    $0x8,%esp
  80071e:	53                   	push   %ebx
  80071f:	50                   	push   %eax
  800720:	ff d6                	call   *%esi
  800722:	83 c4 10             	add    $0x10,%esp
	unsigned long long num;
	int base, lflag, width, precision, altflag;
	char padc;

	while (1) {
		while ((ch = *(unsigned char *) fmt++) != '%') {
  800725:	83 c7 01             	add    $0x1,%edi
  800728:	0f b6 47 ff          	movzbl -0x1(%edi),%eax
  80072c:	83 f8 25             	cmp    $0x25,%eax
  80072f:	75 e2                	jne    800713 <vprintfmt+0x14>
  800731:	c6 45 d4 20          	movb   $0x20,-0x2c(%ebp)
  800735:	c7 45 d8 00 00 00 00 	movl   $0x0,-0x28(%ebp)
  80073c:	c7 45 d0 ff ff ff ff 	movl   $0xffffffff,-0x30(%ebp)
  800743:	c7 45 e0 ff ff ff ff 	movl   $0xffffffff,-0x20(%ebp)
  80074a:	ba 00 00 00 00       	mov    $0x0,%edx
  80074f:	eb 07                	jmp    800758 <vprintfmt+0x59>
		width = -1;
		precision = -1;
		lflag = 0;
		altflag = 0;
	reswitch:
		switch (ch = *(unsigned char *) fmt++) {
  800751:	8b 7d e4             	mov    -0x1c(%ebp),%edi

		// flag to pad on the right
		case '-':
			padc = '-';
  800754:	c6 45 d4 2d          	movb   $0x2d,-0x2c(%ebp)
		width = -1;
		precision = -1;
		lflag = 0;
		altflag = 0;
	reswitch:
		switch (ch = *(unsigned char *) fmt++) {
  800758:	8d 47 01             	lea    0x1(%edi),%eax
  80075b:	89 45 e4             	mov    %eax,-0x1c(%ebp)
  80075e:	0f b6 07             	movzbl (%edi),%eax
  800761:	0f b6 c8             	movzbl %al,%ecx
  800764:	83 e8 23             	sub    $0x23,%eax
  800767:	3c 55                	cmp    $0x55,%al
  800769:	0f 87 21 03 00 00    	ja     800a90 <vprintfmt+0x391>
  80076f:	0f b6 c0             	movzbl %al,%eax
  800772:	ff 24 85 40 2a 80 00 	jmp    *0x802a40(,%eax,4)
  800779:	8b 7d e4             	mov    -0x1c(%ebp),%edi
			padc = '-';
			goto reswitch;

		// flag to pad with 0's instead of spaces
		case '0':
			padc = '0';
  80077c:	c6 45 d4 30          	movb   $0x30,-0x2c(%ebp)
  800780:	eb d6                	jmp    800758 <vprintfmt+0x59>
		width = -1;
		precision = -1;
		lflag = 0;
		altflag = 0;
	reswitch:
		switch (ch = *(unsigned char *) fmt++) {
  800782:	8b 7d e4             	mov    -0x1c(%ebp),%edi
  800785:	b8 00 00 00 00       	mov    $0x0,%eax
  80078a:	89 55 e4             	mov    %edx,-0x1c(%ebp)
		case '6':
		case '7':
		case '8':
		case '9':
			for (precision = 0; ; ++fmt) {
				precision = precision * 10 + ch - '0';
  80078d:	8d 04 80             	lea    (%eax,%eax,4),%eax
  800790:	8d 44 41 d0          	lea    -0x30(%ecx,%eax,2),%eax
				ch = *fmt;
  800794:	0f be 0f             	movsbl (%edi),%ecx
				if (ch < '0' || ch > '9')
  800797:	8d 51 d0             	lea    -0x30(%ecx),%edx
  80079a:	83 fa 09             	cmp    $0x9,%edx
  80079d:	77 39                	ja     8007d8 <vprintfmt+0xd9>
		case '5':
		case '6':
		case '7':
		case '8':
		case '9':
			for (precision = 0; ; ++fmt) {
  80079f:	83 c7 01             	add    $0x1,%edi
				precision = precision * 10 + ch - '0';
				ch = *fmt;
				if (ch < '0' || ch > '9')
					break;
			}
  8007a2:	eb e9                	jmp    80078d <vprintfmt+0x8e>
			goto process_precision;

		case '*':
			precision = va_arg(ap, int);
  8007a4:	8b 45 14             	mov    0x14(%ebp),%eax
  8007a7:	8d 48 04             	lea    0x4(%eax),%ecx
  8007aa:	89 4d 14             	mov    %ecx,0x14(%ebp)
  8007ad:	8b 00                	mov    (%eax),%eax
  8007af:	89 45 d0             	mov    %eax,-0x30(%ebp)
		width = -1;
		precision = -1;
		lflag = 0;
		altflag = 0;
	reswitch:
		switch (ch = *(unsigned char *) fmt++) {
  8007b2:	8b 7d e4             	mov    -0x1c(%ebp),%edi
			}
			goto process_precision;

		case '*':
			precision = va_arg(ap, int);
			goto process_precision;
  8007b5:	eb 27                	jmp    8007de <vprintfmt+0xdf>
  8007b7:	8b 45 e0             	mov    -0x20(%ebp),%eax
  8007ba:	85 c0                	test   %eax,%eax
  8007bc:	b9 00 00 00 00       	mov    $0x0,%ecx
  8007c1:	0f 49 c8             	cmovns %eax,%ecx
  8007c4:	89 4d e0             	mov    %ecx,-0x20(%ebp)
		width = -1;
		precision = -1;
		lflag = 0;
		altflag = 0;
	reswitch:
		switch (ch = *(unsigned char *) fmt++) {
  8007c7:	8b 7d e4             	mov    -0x1c(%ebp),%edi
  8007ca:	eb 8c                	jmp    800758 <vprintfmt+0x59>
  8007cc:	8b 7d e4             	mov    -0x1c(%ebp),%edi
			if (width < 0)
				width = 0;
			goto reswitch;

		case '#':
			altflag = 1;
  8007cf:	c7 45 d8 01 00 00 00 	movl   $0x1,-0x28(%ebp)
			goto reswitch;
  8007d6:	eb 80                	jmp    800758 <vprintfmt+0x59>
  8007d8:	8b 55 e4             	mov    -0x1c(%ebp),%edx
  8007db:	89 45 d0             	mov    %eax,-0x30(%ebp)

		process_precision:
			if (width < 0)
  8007de:	83 7d e0 00          	cmpl   $0x0,-0x20(%ebp)
  8007e2:	0f 89 70 ff ff ff    	jns    800758 <vprintfmt+0x59>
				width = precision, precision = -1;
  8007e8:	8b 45 d0             	mov    -0x30(%ebp),%eax
  8007eb:	89 45 e0             	mov    %eax,-0x20(%ebp)
  8007ee:	c7 45 d0 ff ff ff ff 	movl   $0xffffffff,-0x30(%ebp)
  8007f5:	e9 5e ff ff ff       	jmp    800758 <vprintfmt+0x59>
			goto reswitch;

		// long flag (doubled for long long)
		case 'l':
			lflag++;
  8007fa:	83 c2 01             	add    $0x1,%edx
		width = -1;
		precision = -1;
		lflag = 0;
		altflag = 0;
	reswitch:
		switch (ch = *(unsigned char *) fmt++) {
  8007fd:	8b 7d e4             	mov    -0x1c(%ebp),%edi
			goto reswitch;

		// long flag (doubled for long long)
		case 'l':
			lflag++;
			goto reswitch;
  800800:	e9 53 ff ff ff       	jmp    800758 <vprintfmt+0x59>

		// character
		case 'c':
			putch(va_arg(ap, int), putdat);
  800805:	8b 45 14             	mov    0x14(%ebp),%eax
  800808:	8d 50 04             	lea    0x4(%eax),%edx
  80080b:	89 55 14             	mov    %edx,0x14(%ebp)
  80080e:	83 ec 08             	sub    $0x8,%esp
  800811:	53                   	push   %ebx
  800812:	ff 30                	pushl  (%eax)
  800814:	ff d6                	call   *%esi
			break;
  800816:	83 c4 10             	add    $0x10,%esp
		width = -1;
		precision = -1;
		lflag = 0;
		altflag = 0;
	reswitch:
		switch (ch = *(unsigned char *) fmt++) {
  800819:	8b 7d e4             	mov    -0x1c(%ebp),%edi
			goto reswitch;

		// character
		case 'c':
			putch(va_arg(ap, int), putdat);
			break;
  80081c:	e9 04 ff ff ff       	jmp    800725 <vprintfmt+0x26>

		// error message
		case 'e':
			err = va_arg(ap, int);
  800821:	8b 45 14             	mov    0x14(%ebp),%eax
  800824:	8d 50 04             	lea    0x4(%eax),%edx
  800827:	89 55 14             	mov    %edx,0x14(%ebp)
  80082a:	8b 00                	mov    (%eax),%eax
  80082c:	99                   	cltd   
  80082d:	31 d0                	xor    %edx,%eax
  80082f:	29 d0                	sub    %edx,%eax
			if (err < 0)
				err = -err;
			if (err >= MAXERROR || (p = error_string[err]) == NULL)
  800831:	83 f8 0f             	cmp    $0xf,%eax
  800834:	7f 0b                	jg     800841 <vprintfmt+0x142>
  800836:	8b 14 85 c0 2b 80 00 	mov    0x802bc0(,%eax,4),%edx
  80083d:	85 d2                	test   %edx,%edx
  80083f:	75 18                	jne    800859 <vprintfmt+0x15a>
				printfmt(putch, putdat, "error %d", err);
  800841:	50                   	push   %eax
  800842:	68 1d 29 80 00       	push   $0x80291d
  800847:	53                   	push   %ebx
  800848:	56                   	push   %esi
  800849:	e8 94 fe ff ff       	call   8006e2 <printfmt>
  80084e:	83 c4 10             	add    $0x10,%esp
		width = -1;
		precision = -1;
		lflag = 0;
		altflag = 0;
	reswitch:
		switch (ch = *(unsigned char *) fmt++) {
  800851:	8b 7d e4             	mov    -0x1c(%ebp),%edi
		case 'e':
			err = va_arg(ap, int);
			if (err < 0)
				err = -err;
			if (err >= MAXERROR || (p = error_string[err]) == NULL)
				printfmt(putch, putdat, "error %d", err);
  800854:	e9 cc fe ff ff       	jmp    800725 <vprintfmt+0x26>
			else
				printfmt(putch, putdat, "%s", p);
  800859:	52                   	push   %edx
  80085a:	68 f5 2c 80 00       	push   $0x802cf5
  80085f:	53                   	push   %ebx
  800860:	56                   	push   %esi
  800861:	e8 7c fe ff ff       	call   8006e2 <printfmt>
  800866:	83 c4 10             	add    $0x10,%esp
		width = -1;
		precision = -1;
		lflag = 0;
		altflag = 0;
	reswitch:
		switch (ch = *(unsigned char *) fmt++) {
  800869:	8b 7d e4             	mov    -0x1c(%ebp),%edi
  80086c:	e9 b4 fe ff ff       	jmp    800725 <vprintfmt+0x26>
  800871:	8b 4d d0             	mov    -0x30(%ebp),%ecx
  800874:	8b 45 e0             	mov    -0x20(%ebp),%eax
  800877:	89 45 cc             	mov    %eax,-0x34(%ebp)
				printfmt(putch, putdat, "%s", p);
			break;

		// string
		case 's':
			if ((p = va_arg(ap, char *)) == NULL)
  80087a:	8b 45 14             	mov    0x14(%ebp),%eax
  80087d:	8d 50 04             	lea    0x4(%eax),%edx
  800880:	89 55 14             	mov    %edx,0x14(%ebp)
  800883:	8b 38                	mov    (%eax),%edi
				p = "(null)";
  800885:	85 ff                	test   %edi,%edi
  800887:	ba 16 29 80 00       	mov    $0x802916,%edx
  80088c:	0f 44 fa             	cmove  %edx,%edi
			if (width > 0 && padc != '-')
  80088f:	80 7d d4 2d          	cmpb   $0x2d,-0x2c(%ebp)
  800893:	0f 84 92 00 00 00    	je     80092b <vprintfmt+0x22c>
  800899:	83 7d cc 00          	cmpl   $0x0,-0x34(%ebp)
  80089d:	0f 8e 96 00 00 00    	jle    800939 <vprintfmt+0x23a>
				for (width -= strnlen(p, precision); width > 0; width--)
  8008a3:	83 ec 08             	sub    $0x8,%esp
  8008a6:	51                   	push   %ecx
  8008a7:	57                   	push   %edi
  8008a8:	e8 86 02 00 00       	call   800b33 <strnlen>
  8008ad:	8b 4d cc             	mov    -0x34(%ebp),%ecx
  8008b0:	29 c1                	sub    %eax,%ecx
  8008b2:	89 4d cc             	mov    %ecx,-0x34(%ebp)
  8008b5:	83 c4 10             	add    $0x10,%esp
					putch(padc, putdat);
  8008b8:	0f be 45 d4          	movsbl -0x2c(%ebp),%eax
  8008bc:	89 45 e0             	mov    %eax,-0x20(%ebp)
  8008bf:	89 7d d4             	mov    %edi,-0x2c(%ebp)
  8008c2:	89 cf                	mov    %ecx,%edi
		// string
		case 's':
			if ((p = va_arg(ap, char *)) == NULL)
				p = "(null)";
			if (width > 0 && padc != '-')
				for (width -= strnlen(p, precision); width > 0; width--)
  8008c4:	eb 0f                	jmp    8008d5 <vprintfmt+0x1d6>
					putch(padc, putdat);
  8008c6:	83 ec 08             	sub    $0x8,%esp
  8008c9:	53                   	push   %ebx
  8008ca:	ff 75 e0             	pushl  -0x20(%ebp)
  8008cd:	ff d6                	call   *%esi
		// string
		case 's':
			if ((p = va_arg(ap, char *)) == NULL)
				p = "(null)";
			if (width > 0 && padc != '-')
				for (width -= strnlen(p, precision); width > 0; width--)
  8008cf:	83 ef 01             	sub    $0x1,%edi
  8008d2:	83 c4 10             	add    $0x10,%esp
  8008d5:	85 ff                	test   %edi,%edi
  8008d7:	7f ed                	jg     8008c6 <vprintfmt+0x1c7>
  8008d9:	8b 7d d4             	mov    -0x2c(%ebp),%edi
  8008dc:	8b 4d cc             	mov    -0x34(%ebp),%ecx
  8008df:	85 c9                	test   %ecx,%ecx
  8008e1:	b8 00 00 00 00       	mov    $0x0,%eax
  8008e6:	0f 49 c1             	cmovns %ecx,%eax
  8008e9:	29 c1                	sub    %eax,%ecx
  8008eb:	89 75 08             	mov    %esi,0x8(%ebp)
  8008ee:	8b 75 d0             	mov    -0x30(%ebp),%esi
  8008f1:	89 5d 0c             	mov    %ebx,0xc(%ebp)
  8008f4:	89 cb                	mov    %ecx,%ebx
  8008f6:	eb 4d                	jmp    800945 <vprintfmt+0x246>
					putch(padc, putdat);
			for (; (ch = *p++) != '\0' && (precision < 0 || --precision >= 0); width--)
				if (altflag && (ch < ' ' || ch > '~'))
  8008f8:	83 7d d8 00          	cmpl   $0x0,-0x28(%ebp)
  8008fc:	74 1b                	je     800919 <vprintfmt+0x21a>
  8008fe:	0f be c0             	movsbl %al,%eax
  800901:	83 e8 20             	sub    $0x20,%eax
  800904:	83 f8 5e             	cmp    $0x5e,%eax
  800907:	76 10                	jbe    800919 <vprintfmt+0x21a>
					putch('?', putdat);
  800909:	83 ec 08             	sub    $0x8,%esp
  80090c:	ff 75 0c             	pushl  0xc(%ebp)
  80090f:	6a 3f                	push   $0x3f
  800911:	ff 55 08             	call   *0x8(%ebp)
  800914:	83 c4 10             	add    $0x10,%esp
  800917:	eb 0d                	jmp    800926 <vprintfmt+0x227>
				else
					putch(ch, putdat);
  800919:	83 ec 08             	sub    $0x8,%esp
  80091c:	ff 75 0c             	pushl  0xc(%ebp)
  80091f:	52                   	push   %edx
  800920:	ff 55 08             	call   *0x8(%ebp)
  800923:	83 c4 10             	add    $0x10,%esp
			if ((p = va_arg(ap, char *)) == NULL)
				p = "(null)";
			if (width > 0 && padc != '-')
				for (width -= strnlen(p, precision); width > 0; width--)
					putch(padc, putdat);
			for (; (ch = *p++) != '\0' && (precision < 0 || --precision >= 0); width--)
  800926:	83 eb 01             	sub    $0x1,%ebx
  800929:	eb 1a                	jmp    800945 <vprintfmt+0x246>
  80092b:	89 75 08             	mov    %esi,0x8(%ebp)
  80092e:	8b 75 d0             	mov    -0x30(%ebp),%esi
  800931:	89 5d 0c             	mov    %ebx,0xc(%ebp)
  800934:	8b 5d e0             	mov    -0x20(%ebp),%ebx
  800937:	eb 0c                	jmp    800945 <vprintfmt+0x246>
  800939:	89 75 08             	mov    %esi,0x8(%ebp)
  80093c:	8b 75 d0             	mov    -0x30(%ebp),%esi
  80093f:	89 5d 0c             	mov    %ebx,0xc(%ebp)
  800942:	8b 5d e0             	mov    -0x20(%ebp),%ebx
  800945:	83 c7 01             	add    $0x1,%edi
  800948:	0f b6 47 ff          	movzbl -0x1(%edi),%eax
  80094c:	0f be d0             	movsbl %al,%edx
  80094f:	85 d2                	test   %edx,%edx
  800951:	74 23                	je     800976 <vprintfmt+0x277>
  800953:	85 f6                	test   %esi,%esi
  800955:	78 a1                	js     8008f8 <vprintfmt+0x1f9>
  800957:	83 ee 01             	sub    $0x1,%esi
  80095a:	79 9c                	jns    8008f8 <vprintfmt+0x1f9>
  80095c:	89 df                	mov    %ebx,%edi
  80095e:	8b 75 08             	mov    0x8(%ebp),%esi
  800961:	8b 5d 0c             	mov    0xc(%ebp),%ebx
  800964:	eb 18                	jmp    80097e <vprintfmt+0x27f>
				if (altflag && (ch < ' ' || ch > '~'))
					putch('?', putdat);
				else
					putch(ch, putdat);
			for (; width > 0; width--)
				putch(' ', putdat);
  800966:	83 ec 08             	sub    $0x8,%esp
  800969:	53                   	push   %ebx
  80096a:	6a 20                	push   $0x20
  80096c:	ff d6                	call   *%esi
			for (; (ch = *p++) != '\0' && (precision < 0 || --precision >= 0); width--)
				if (altflag && (ch < ' ' || ch > '~'))
					putch('?', putdat);
				else
					putch(ch, putdat);
			for (; width > 0; width--)
  80096e:	83 ef 01             	sub    $0x1,%edi
  800971:	83 c4 10             	add    $0x10,%esp
  800974:	eb 08                	jmp    80097e <vprintfmt+0x27f>
  800976:	89 df                	mov    %ebx,%edi
  800978:	8b 75 08             	mov    0x8(%ebp),%esi
  80097b:	8b 5d 0c             	mov    0xc(%ebp),%ebx
  80097e:	85 ff                	test   %edi,%edi
  800980:	7f e4                	jg     800966 <vprintfmt+0x267>
		width = -1;
		precision = -1;
		lflag = 0;
		altflag = 0;
	reswitch:
		switch (ch = *(unsigned char *) fmt++) {
  800982:	8b 7d e4             	mov    -0x1c(%ebp),%edi
  800985:	e9 9b fd ff ff       	jmp    800725 <vprintfmt+0x26>
// Same as getuint but signed - can't use getuint
// because of sign extension
static long long
getint(va_list *ap, int lflag)
{
	if (lflag >= 2)
  80098a:	83 fa 01             	cmp    $0x1,%edx
  80098d:	7e 16                	jle    8009a5 <vprintfmt+0x2a6>
		return va_arg(*ap, long long);
  80098f:	8b 45 14             	mov    0x14(%ebp),%eax
  800992:	8d 50 08             	lea    0x8(%eax),%edx
  800995:	89 55 14             	mov    %edx,0x14(%ebp)
  800998:	8b 50 04             	mov    0x4(%eax),%edx
  80099b:	8b 00                	mov    (%eax),%eax
  80099d:	89 45 d8             	mov    %eax,-0x28(%ebp)
  8009a0:	89 55 dc             	mov    %edx,-0x24(%ebp)
  8009a3:	eb 32                	jmp    8009d7 <vprintfmt+0x2d8>
	else if (lflag)
  8009a5:	85 d2                	test   %edx,%edx
  8009a7:	74 18                	je     8009c1 <vprintfmt+0x2c2>
		return va_arg(*ap, long);
  8009a9:	8b 45 14             	mov    0x14(%ebp),%eax
  8009ac:	8d 50 04             	lea    0x4(%eax),%edx
  8009af:	89 55 14             	mov    %edx,0x14(%ebp)
  8009b2:	8b 00                	mov    (%eax),%eax
  8009b4:	89 45 d8             	mov    %eax,-0x28(%ebp)
  8009b7:	89 c1                	mov    %eax,%ecx
  8009b9:	c1 f9 1f             	sar    $0x1f,%ecx
  8009bc:	89 4d dc             	mov    %ecx,-0x24(%ebp)
  8009bf:	eb 16                	jmp    8009d7 <vprintfmt+0x2d8>
	else
		return va_arg(*ap, int);
  8009c1:	8b 45 14             	mov    0x14(%ebp),%eax
  8009c4:	8d 50 04             	lea    0x4(%eax),%edx
  8009c7:	89 55 14             	mov    %edx,0x14(%ebp)
  8009ca:	8b 00                	mov    (%eax),%eax
  8009cc:	89 45 d8             	mov    %eax,-0x28(%ebp)
  8009cf:	89 c1                	mov    %eax,%ecx
  8009d1:	c1 f9 1f             	sar    $0x1f,%ecx
  8009d4:	89 4d dc             	mov    %ecx,-0x24(%ebp)
				putch(' ', putdat);
			break;

		// (signed) decimal
		case 'd':
			num = getint(&ap, lflag);
  8009d7:	8b 45 d8             	mov    -0x28(%ebp),%eax
  8009da:	8b 55 dc             	mov    -0x24(%ebp),%edx
			if ((long long) num < 0) {
				putch('-', putdat);
				num = -(long long) num;
			}
			base = 10;
  8009dd:	b9 0a 00 00 00       	mov    $0xa,%ecx
			break;

		// (signed) decimal
		case 'd':
			num = getint(&ap, lflag);
			if ((long long) num < 0) {
  8009e2:	83 7d dc 00          	cmpl   $0x0,-0x24(%ebp)
  8009e6:	79 74                	jns    800a5c <vprintfmt+0x35d>
				putch('-', putdat);
  8009e8:	83 ec 08             	sub    $0x8,%esp
  8009eb:	53                   	push   %ebx
  8009ec:	6a 2d                	push   $0x2d
  8009ee:	ff d6                	call   *%esi
				num = -(long long) num;
  8009f0:	8b 45 d8             	mov    -0x28(%ebp),%eax
  8009f3:	8b 55 dc             	mov    -0x24(%ebp),%edx
  8009f6:	f7 d8                	neg    %eax
  8009f8:	83 d2 00             	adc    $0x0,%edx
  8009fb:	f7 da                	neg    %edx
  8009fd:	83 c4 10             	add    $0x10,%esp
			}
			base = 10;
  800a00:	b9 0a 00 00 00       	mov    $0xa,%ecx
  800a05:	eb 55                	jmp    800a5c <vprintfmt+0x35d>
			goto number;

		// unsigned decimal
		case 'u':
			num = getuint(&ap, lflag);
  800a07:	8d 45 14             	lea    0x14(%ebp),%eax
  800a0a:	e8 7c fc ff ff       	call   80068b <getuint>
			base = 10;
  800a0f:	b9 0a 00 00 00       	mov    $0xa,%ecx
			goto number;
  800a14:	eb 46                	jmp    800a5c <vprintfmt+0x35d>

		// (unsigned) octal
		case 'o':
			// Replace this with your code.
			num = getuint(&ap, lflag);
  800a16:	8d 45 14             	lea    0x14(%ebp),%eax
  800a19:	e8 6d fc ff ff       	call   80068b <getuint>
                        base = 8;
  800a1e:	b9 08 00 00 00       	mov    $0x8,%ecx
                        goto number;
  800a23:	eb 37                	jmp    800a5c <vprintfmt+0x35d>

		// pointer
		case 'p':
			putch('0', putdat);
  800a25:	83 ec 08             	sub    $0x8,%esp
  800a28:	53                   	push   %ebx
  800a29:	6a 30                	push   $0x30
  800a2b:	ff d6                	call   *%esi
			putch('x', putdat);
  800a2d:	83 c4 08             	add    $0x8,%esp
  800a30:	53                   	push   %ebx
  800a31:	6a 78                	push   $0x78
  800a33:	ff d6                	call   *%esi
			num = (unsigned long long)
				(uintptr_t) va_arg(ap, void *);
  800a35:	8b 45 14             	mov    0x14(%ebp),%eax
  800a38:	8d 50 04             	lea    0x4(%eax),%edx
  800a3b:	89 55 14             	mov    %edx,0x14(%ebp)

		// pointer
		case 'p':
			putch('0', putdat);
			putch('x', putdat);
			num = (unsigned long long)
  800a3e:	8b 00                	mov    (%eax),%eax
  800a40:	ba 00 00 00 00       	mov    $0x0,%edx
				(uintptr_t) va_arg(ap, void *);
			base = 16;
			goto number;
  800a45:	83 c4 10             	add    $0x10,%esp
		case 'p':
			putch('0', putdat);
			putch('x', putdat);
			num = (unsigned long long)
				(uintptr_t) va_arg(ap, void *);
			base = 16;
  800a48:	b9 10 00 00 00       	mov    $0x10,%ecx
			goto number;
  800a4d:	eb 0d                	jmp    800a5c <vprintfmt+0x35d>

		// (unsigned) hexadecimal
		case 'x':
			num = getuint(&ap, lflag);
  800a4f:	8d 45 14             	lea    0x14(%ebp),%eax
  800a52:	e8 34 fc ff ff       	call   80068b <getuint>
			base = 16;
  800a57:	b9 10 00 00 00       	mov    $0x10,%ecx
		number:
			printnum(putch, putdat, num, base, width, padc);
  800a5c:	83 ec 0c             	sub    $0xc,%esp
  800a5f:	0f be 7d d4          	movsbl -0x2c(%ebp),%edi
  800a63:	57                   	push   %edi
  800a64:	ff 75 e0             	pushl  -0x20(%ebp)
  800a67:	51                   	push   %ecx
  800a68:	52                   	push   %edx
  800a69:	50                   	push   %eax
  800a6a:	89 da                	mov    %ebx,%edx
  800a6c:	89 f0                	mov    %esi,%eax
  800a6e:	e8 6e fb ff ff       	call   8005e1 <printnum>
			break;
  800a73:	83 c4 20             	add    $0x20,%esp
  800a76:	8b 7d e4             	mov    -0x1c(%ebp),%edi
  800a79:	e9 a7 fc ff ff       	jmp    800725 <vprintfmt+0x26>

		// escaped '%' character
		case '%':
			putch(ch, putdat);
  800a7e:	83 ec 08             	sub    $0x8,%esp
  800a81:	53                   	push   %ebx
  800a82:	51                   	push   %ecx
  800a83:	ff d6                	call   *%esi
			break;
  800a85:	83 c4 10             	add    $0x10,%esp
		width = -1;
		precision = -1;
		lflag = 0;
		altflag = 0;
	reswitch:
		switch (ch = *(unsigned char *) fmt++) {
  800a88:	8b 7d e4             	mov    -0x1c(%ebp),%edi
			break;

		// escaped '%' character
		case '%':
			putch(ch, putdat);
			break;
  800a8b:	e9 95 fc ff ff       	jmp    800725 <vprintfmt+0x26>

		// unrecognized escape sequence - just print it literally
		default:
			putch('%', putdat);
  800a90:	83 ec 08             	sub    $0x8,%esp
  800a93:	53                   	push   %ebx
  800a94:	6a 25                	push   $0x25
  800a96:	ff d6                	call   *%esi
			for (fmt--; fmt[-1] != '%'; fmt--)
  800a98:	83 c4 10             	add    $0x10,%esp
  800a9b:	eb 03                	jmp    800aa0 <vprintfmt+0x3a1>
  800a9d:	83 ef 01             	sub    $0x1,%edi
  800aa0:	80 7f ff 25          	cmpb   $0x25,-0x1(%edi)
  800aa4:	75 f7                	jne    800a9d <vprintfmt+0x39e>
  800aa6:	e9 7a fc ff ff       	jmp    800725 <vprintfmt+0x26>
				/* do nothing */;
			break;
		}
	}
}
  800aab:	8d 65 f4             	lea    -0xc(%ebp),%esp
  800aae:	5b                   	pop    %ebx
  800aaf:	5e                   	pop    %esi
  800ab0:	5f                   	pop    %edi
  800ab1:	5d                   	pop    %ebp
  800ab2:	c3                   	ret    

00800ab3 <vsnprintf>:
		*b->buf++ = ch;
}

int
vsnprintf(char *buf, int n, const char *fmt, va_list ap)
{
  800ab3:	55                   	push   %ebp
  800ab4:	89 e5                	mov    %esp,%ebp
  800ab6:	83 ec 18             	sub    $0x18,%esp
  800ab9:	8b 45 08             	mov    0x8(%ebp),%eax
  800abc:	8b 55 0c             	mov    0xc(%ebp),%edx
	struct sprintbuf b = {buf, buf+n-1, 0};
  800abf:	89 45 ec             	mov    %eax,-0x14(%ebp)
  800ac2:	8d 4c 10 ff          	lea    -0x1(%eax,%edx,1),%ecx
  800ac6:	89 4d f0             	mov    %ecx,-0x10(%ebp)
  800ac9:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)

	if (buf == NULL || n < 1)
  800ad0:	85 c0                	test   %eax,%eax
  800ad2:	74 26                	je     800afa <vsnprintf+0x47>
  800ad4:	85 d2                	test   %edx,%edx
  800ad6:	7e 22                	jle    800afa <vsnprintf+0x47>
		return -E_INVAL;

	// print the string to the buffer
	vprintfmt((void*)sprintputch, &b, fmt, ap);
  800ad8:	ff 75 14             	pushl  0x14(%ebp)
  800adb:	ff 75 10             	pushl  0x10(%ebp)
  800ade:	8d 45 ec             	lea    -0x14(%ebp),%eax
  800ae1:	50                   	push   %eax
  800ae2:	68 c5 06 80 00       	push   $0x8006c5
  800ae7:	e8 13 fc ff ff       	call   8006ff <vprintfmt>

	// null terminate the buffer
	*b.buf = '\0';
  800aec:	8b 45 ec             	mov    -0x14(%ebp),%eax
  800aef:	c6 00 00             	movb   $0x0,(%eax)

	return b.cnt;
  800af2:	8b 45 f4             	mov    -0xc(%ebp),%eax
  800af5:	83 c4 10             	add    $0x10,%esp
  800af8:	eb 05                	jmp    800aff <vsnprintf+0x4c>
vsnprintf(char *buf, int n, const char *fmt, va_list ap)
{
	struct sprintbuf b = {buf, buf+n-1, 0};

	if (buf == NULL || n < 1)
		return -E_INVAL;
  800afa:	b8 fd ff ff ff       	mov    $0xfffffffd,%eax

	// null terminate the buffer
	*b.buf = '\0';

	return b.cnt;
}
  800aff:	c9                   	leave  
  800b00:	c3                   	ret    

00800b01 <snprintf>:

int
snprintf(char *buf, int n, const char *fmt, ...)
{
  800b01:	55                   	push   %ebp
  800b02:	89 e5                	mov    %esp,%ebp
  800b04:	83 ec 08             	sub    $0x8,%esp
	va_list ap;
	int rc;

	va_start(ap, fmt);
  800b07:	8d 45 14             	lea    0x14(%ebp),%eax
	rc = vsnprintf(buf, n, fmt, ap);
  800b0a:	50                   	push   %eax
  800b0b:	ff 75 10             	pushl  0x10(%ebp)
  800b0e:	ff 75 0c             	pushl  0xc(%ebp)
  800b11:	ff 75 08             	pushl  0x8(%ebp)
  800b14:	e8 9a ff ff ff       	call   800ab3 <vsnprintf>
	va_end(ap);

	return rc;
}
  800b19:	c9                   	leave  
  800b1a:	c3                   	ret    

00800b1b <strlen>:
// Primespipe runs 3x faster this way.
#define ASM 1

int
strlen(const char *s)
{
  800b1b:	55                   	push   %ebp
  800b1c:	89 e5                	mov    %esp,%ebp
  800b1e:	8b 55 08             	mov    0x8(%ebp),%edx
	int n;

	for (n = 0; *s != '\0'; s++)
  800b21:	b8 00 00 00 00       	mov    $0x0,%eax
  800b26:	eb 03                	jmp    800b2b <strlen+0x10>
		n++;
  800b28:	83 c0 01             	add    $0x1,%eax
int
strlen(const char *s)
{
	int n;

	for (n = 0; *s != '\0'; s++)
  800b2b:	80 3c 02 00          	cmpb   $0x0,(%edx,%eax,1)
  800b2f:	75 f7                	jne    800b28 <strlen+0xd>
		n++;
	return n;
}
  800b31:	5d                   	pop    %ebp
  800b32:	c3                   	ret    

00800b33 <strnlen>:

int
strnlen(const char *s, size_t size)
{
  800b33:	55                   	push   %ebp
  800b34:	89 e5                	mov    %esp,%ebp
  800b36:	8b 4d 08             	mov    0x8(%ebp),%ecx
  800b39:	8b 45 0c             	mov    0xc(%ebp),%eax
	int n;

	for (n = 0; size > 0 && *s != '\0'; s++, size--)
  800b3c:	ba 00 00 00 00       	mov    $0x0,%edx
  800b41:	eb 03                	jmp    800b46 <strnlen+0x13>
		n++;
  800b43:	83 c2 01             	add    $0x1,%edx
int
strnlen(const char *s, size_t size)
{
	int n;

	for (n = 0; size > 0 && *s != '\0'; s++, size--)
  800b46:	39 c2                	cmp    %eax,%edx
  800b48:	74 08                	je     800b52 <strnlen+0x1f>
  800b4a:	80 3c 11 00          	cmpb   $0x0,(%ecx,%edx,1)
  800b4e:	75 f3                	jne    800b43 <strnlen+0x10>
  800b50:	89 d0                	mov    %edx,%eax
		n++;
	return n;
}
  800b52:	5d                   	pop    %ebp
  800b53:	c3                   	ret    

00800b54 <strcpy>:

char *
strcpy(char *dst, const char *src)
{
  800b54:	55                   	push   %ebp
  800b55:	89 e5                	mov    %esp,%ebp
  800b57:	53                   	push   %ebx
  800b58:	8b 45 08             	mov    0x8(%ebp),%eax
  800b5b:	8b 4d 0c             	mov    0xc(%ebp),%ecx
	char *ret;

	ret = dst;
	while ((*dst++ = *src++) != '\0')
  800b5e:	89 c2                	mov    %eax,%edx
  800b60:	83 c2 01             	add    $0x1,%edx
  800b63:	83 c1 01             	add    $0x1,%ecx
  800b66:	0f b6 59 ff          	movzbl -0x1(%ecx),%ebx
  800b6a:	88 5a ff             	mov    %bl,-0x1(%edx)
  800b6d:	84 db                	test   %bl,%bl
  800b6f:	75 ef                	jne    800b60 <strcpy+0xc>
		/* do nothing */;
	return ret;
}
  800b71:	5b                   	pop    %ebx
  800b72:	5d                   	pop    %ebp
  800b73:	c3                   	ret    

00800b74 <strcat>:

char *
strcat(char *dst, const char *src)
{
  800b74:	55                   	push   %ebp
  800b75:	89 e5                	mov    %esp,%ebp
  800b77:	53                   	push   %ebx
  800b78:	8b 5d 08             	mov    0x8(%ebp),%ebx
	int len = strlen(dst);
  800b7b:	53                   	push   %ebx
  800b7c:	e8 9a ff ff ff       	call   800b1b <strlen>
  800b81:	83 c4 04             	add    $0x4,%esp
	strcpy(dst + len, src);
  800b84:	ff 75 0c             	pushl  0xc(%ebp)
  800b87:	01 d8                	add    %ebx,%eax
  800b89:	50                   	push   %eax
  800b8a:	e8 c5 ff ff ff       	call   800b54 <strcpy>
	return dst;
}
  800b8f:	89 d8                	mov    %ebx,%eax
  800b91:	8b 5d fc             	mov    -0x4(%ebp),%ebx
  800b94:	c9                   	leave  
  800b95:	c3                   	ret    

00800b96 <strncpy>:

char *
strncpy(char *dst, const char *src, size_t size) {
  800b96:	55                   	push   %ebp
  800b97:	89 e5                	mov    %esp,%ebp
  800b99:	56                   	push   %esi
  800b9a:	53                   	push   %ebx
  800b9b:	8b 75 08             	mov    0x8(%ebp),%esi
  800b9e:	8b 4d 0c             	mov    0xc(%ebp),%ecx
  800ba1:	89 f3                	mov    %esi,%ebx
  800ba3:	03 5d 10             	add    0x10(%ebp),%ebx
	size_t i;
	char *ret;

	ret = dst;
	for (i = 0; i < size; i++) {
  800ba6:	89 f2                	mov    %esi,%edx
  800ba8:	eb 0f                	jmp    800bb9 <strncpy+0x23>
		*dst++ = *src;
  800baa:	83 c2 01             	add    $0x1,%edx
  800bad:	0f b6 01             	movzbl (%ecx),%eax
  800bb0:	88 42 ff             	mov    %al,-0x1(%edx)
		// If strlen(src) < size, null-pad 'dst' out to 'size' chars
		if (*src != '\0')
			src++;
  800bb3:	80 39 01             	cmpb   $0x1,(%ecx)
  800bb6:	83 d9 ff             	sbb    $0xffffffff,%ecx
strncpy(char *dst, const char *src, size_t size) {
	size_t i;
	char *ret;

	ret = dst;
	for (i = 0; i < size; i++) {
  800bb9:	39 da                	cmp    %ebx,%edx
  800bbb:	75 ed                	jne    800baa <strncpy+0x14>
		// If strlen(src) < size, null-pad 'dst' out to 'size' chars
		if (*src != '\0')
			src++;
	}
	return ret;
}
  800bbd:	89 f0                	mov    %esi,%eax
  800bbf:	5b                   	pop    %ebx
  800bc0:	5e                   	pop    %esi
  800bc1:	5d                   	pop    %ebp
  800bc2:	c3                   	ret    

00800bc3 <strlcpy>:

size_t
strlcpy(char *dst, const char *src, size_t size)
{
  800bc3:	55                   	push   %ebp
  800bc4:	89 e5                	mov    %esp,%ebp
  800bc6:	56                   	push   %esi
  800bc7:	53                   	push   %ebx
  800bc8:	8b 75 08             	mov    0x8(%ebp),%esi
  800bcb:	8b 4d 0c             	mov    0xc(%ebp),%ecx
  800bce:	8b 55 10             	mov    0x10(%ebp),%edx
  800bd1:	89 f0                	mov    %esi,%eax
	char *dst_in;

	dst_in = dst;
	if (size > 0) {
  800bd3:	85 d2                	test   %edx,%edx
  800bd5:	74 21                	je     800bf8 <strlcpy+0x35>
  800bd7:	8d 44 16 ff          	lea    -0x1(%esi,%edx,1),%eax
  800bdb:	89 f2                	mov    %esi,%edx
  800bdd:	eb 09                	jmp    800be8 <strlcpy+0x25>
		while (--size > 0 && *src != '\0')
			*dst++ = *src++;
  800bdf:	83 c2 01             	add    $0x1,%edx
  800be2:	83 c1 01             	add    $0x1,%ecx
  800be5:	88 5a ff             	mov    %bl,-0x1(%edx)
{
	char *dst_in;

	dst_in = dst;
	if (size > 0) {
		while (--size > 0 && *src != '\0')
  800be8:	39 c2                	cmp    %eax,%edx
  800bea:	74 09                	je     800bf5 <strlcpy+0x32>
  800bec:	0f b6 19             	movzbl (%ecx),%ebx
  800bef:	84 db                	test   %bl,%bl
  800bf1:	75 ec                	jne    800bdf <strlcpy+0x1c>
  800bf3:	89 d0                	mov    %edx,%eax
			*dst++ = *src++;
		*dst = '\0';
  800bf5:	c6 00 00             	movb   $0x0,(%eax)
	}
	return dst - dst_in;
  800bf8:	29 f0                	sub    %esi,%eax
}
  800bfa:	5b                   	pop    %ebx
  800bfb:	5e                   	pop    %esi
  800bfc:	5d                   	pop    %ebp
  800bfd:	c3                   	ret    

00800bfe <strcmp>:

int
strcmp(const char *p, const char *q)
{
  800bfe:	55                   	push   %ebp
  800bff:	89 e5                	mov    %esp,%ebp
  800c01:	8b 4d 08             	mov    0x8(%ebp),%ecx
  800c04:	8b 55 0c             	mov    0xc(%ebp),%edx
	while (*p && *p == *q)
  800c07:	eb 06                	jmp    800c0f <strcmp+0x11>
		p++, q++;
  800c09:	83 c1 01             	add    $0x1,%ecx
  800c0c:	83 c2 01             	add    $0x1,%edx
}

int
strcmp(const char *p, const char *q)
{
	while (*p && *p == *q)
  800c0f:	0f b6 01             	movzbl (%ecx),%eax
  800c12:	84 c0                	test   %al,%al
  800c14:	74 04                	je     800c1a <strcmp+0x1c>
  800c16:	3a 02                	cmp    (%edx),%al
  800c18:	74 ef                	je     800c09 <strcmp+0xb>
		p++, q++;
	return (int) ((unsigned char) *p - (unsigned char) *q);
  800c1a:	0f b6 c0             	movzbl %al,%eax
  800c1d:	0f b6 12             	movzbl (%edx),%edx
  800c20:	29 d0                	sub    %edx,%eax
}
  800c22:	5d                   	pop    %ebp
  800c23:	c3                   	ret    

00800c24 <strncmp>:

int
strncmp(const char *p, const char *q, size_t n)
{
  800c24:	55                   	push   %ebp
  800c25:	89 e5                	mov    %esp,%ebp
  800c27:	53                   	push   %ebx
  800c28:	8b 45 08             	mov    0x8(%ebp),%eax
  800c2b:	8b 55 0c             	mov    0xc(%ebp),%edx
  800c2e:	89 c3                	mov    %eax,%ebx
  800c30:	03 5d 10             	add    0x10(%ebp),%ebx
	while (n > 0 && *p && *p == *q)
  800c33:	eb 06                	jmp    800c3b <strncmp+0x17>
		n--, p++, q++;
  800c35:	83 c0 01             	add    $0x1,%eax
  800c38:	83 c2 01             	add    $0x1,%edx
}

int
strncmp(const char *p, const char *q, size_t n)
{
	while (n > 0 && *p && *p == *q)
  800c3b:	39 d8                	cmp    %ebx,%eax
  800c3d:	74 15                	je     800c54 <strncmp+0x30>
  800c3f:	0f b6 08             	movzbl (%eax),%ecx
  800c42:	84 c9                	test   %cl,%cl
  800c44:	74 04                	je     800c4a <strncmp+0x26>
  800c46:	3a 0a                	cmp    (%edx),%cl
  800c48:	74 eb                	je     800c35 <strncmp+0x11>
		n--, p++, q++;
	if (n == 0)
		return 0;
	else
		return (int) ((unsigned char) *p - (unsigned char) *q);
  800c4a:	0f b6 00             	movzbl (%eax),%eax
  800c4d:	0f b6 12             	movzbl (%edx),%edx
  800c50:	29 d0                	sub    %edx,%eax
  800c52:	eb 05                	jmp    800c59 <strncmp+0x35>
strncmp(const char *p, const char *q, size_t n)
{
	while (n > 0 && *p && *p == *q)
		n--, p++, q++;
	if (n == 0)
		return 0;
  800c54:	b8 00 00 00 00       	mov    $0x0,%eax
	else
		return (int) ((unsigned char) *p - (unsigned char) *q);
}
  800c59:	5b                   	pop    %ebx
  800c5a:	5d                   	pop    %ebp
  800c5b:	c3                   	ret    

00800c5c <strchr>:

// Return a pointer to the first occurrence of 'c' in 's',
// or a null pointer if the string has no 'c'.
char *
strchr(const char *s, char c)
{
  800c5c:	55                   	push   %ebp
  800c5d:	89 e5                	mov    %esp,%ebp
  800c5f:	8b 45 08             	mov    0x8(%ebp),%eax
  800c62:	0f b6 4d 0c          	movzbl 0xc(%ebp),%ecx
	for (; *s; s++)
  800c66:	eb 07                	jmp    800c6f <strchr+0x13>
		if (*s == c)
  800c68:	38 ca                	cmp    %cl,%dl
  800c6a:	74 0f                	je     800c7b <strchr+0x1f>
// Return a pointer to the first occurrence of 'c' in 's',
// or a null pointer if the string has no 'c'.
char *
strchr(const char *s, char c)
{
	for (; *s; s++)
  800c6c:	83 c0 01             	add    $0x1,%eax
  800c6f:	0f b6 10             	movzbl (%eax),%edx
  800c72:	84 d2                	test   %dl,%dl
  800c74:	75 f2                	jne    800c68 <strchr+0xc>
		if (*s == c)
			return (char *) s;
	return 0;
  800c76:	b8 00 00 00 00       	mov    $0x0,%eax
}
  800c7b:	5d                   	pop    %ebp
  800c7c:	c3                   	ret    

00800c7d <strfind>:

// Return a pointer to the first occurrence of 'c' in 's',
// or a pointer to the string-ending null character if the string has no 'c'.
char *
strfind(const char *s, char c)
{
  800c7d:	55                   	push   %ebp
  800c7e:	89 e5                	mov    %esp,%ebp
  800c80:	8b 45 08             	mov    0x8(%ebp),%eax
  800c83:	0f b6 4d 0c          	movzbl 0xc(%ebp),%ecx
	for (; *s; s++)
  800c87:	eb 03                	jmp    800c8c <strfind+0xf>
  800c89:	83 c0 01             	add    $0x1,%eax
  800c8c:	0f b6 10             	movzbl (%eax),%edx
		if (*s == c)
  800c8f:	84 d2                	test   %dl,%dl
  800c91:	74 04                	je     800c97 <strfind+0x1a>
  800c93:	38 ca                	cmp    %cl,%dl
  800c95:	75 f2                	jne    800c89 <strfind+0xc>
			break;
	return (char *) s;
}
  800c97:	5d                   	pop    %ebp
  800c98:	c3                   	ret    

00800c99 <memset>:

#if ASM
void *
memset(void *v, int c, size_t n)
{
  800c99:	55                   	push   %ebp
  800c9a:	89 e5                	mov    %esp,%ebp
  800c9c:	57                   	push   %edi
  800c9d:	56                   	push   %esi
  800c9e:	53                   	push   %ebx
  800c9f:	8b 7d 08             	mov    0x8(%ebp),%edi
  800ca2:	8b 4d 10             	mov    0x10(%ebp),%ecx
	char *p;

	if (n == 0)
  800ca5:	85 c9                	test   %ecx,%ecx
  800ca7:	74 36                	je     800cdf <memset+0x46>
		return v;
	if ((int)v%4 == 0 && n%4 == 0) {
  800ca9:	f7 c7 03 00 00 00    	test   $0x3,%edi
  800caf:	75 28                	jne    800cd9 <memset+0x40>
  800cb1:	f6 c1 03             	test   $0x3,%cl
  800cb4:	75 23                	jne    800cd9 <memset+0x40>
		c &= 0xFF;
  800cb6:	0f b6 55 0c          	movzbl 0xc(%ebp),%edx
		c = (c<<24)|(c<<16)|(c<<8)|c;
  800cba:	89 d3                	mov    %edx,%ebx
  800cbc:	c1 e3 08             	shl    $0x8,%ebx
  800cbf:	89 d6                	mov    %edx,%esi
  800cc1:	c1 e6 18             	shl    $0x18,%esi
  800cc4:	89 d0                	mov    %edx,%eax
  800cc6:	c1 e0 10             	shl    $0x10,%eax
  800cc9:	09 f0                	or     %esi,%eax
  800ccb:	09 c2                	or     %eax,%edx
  800ccd:	89 d0                	mov    %edx,%eax
  800ccf:	09 d8                	or     %ebx,%eax
		asm volatile("cld; rep stosl\n"
			:: "D" (v), "a" (c), "c" (n/4)
  800cd1:	c1 e9 02             	shr    $0x2,%ecx
	if (n == 0)
		return v;
	if ((int)v%4 == 0 && n%4 == 0) {
		c &= 0xFF;
		c = (c<<24)|(c<<16)|(c<<8)|c;
		asm volatile("cld; rep stosl\n"
  800cd4:	fc                   	cld    
  800cd5:	f3 ab                	rep stos %eax,%es:(%edi)
  800cd7:	eb 06                	jmp    800cdf <memset+0x46>
			:: "D" (v), "a" (c), "c" (n/4)
			: "cc", "memory");
	} else
		asm volatile("cld; rep stosb\n"
  800cd9:	8b 45 0c             	mov    0xc(%ebp),%eax
  800cdc:	fc                   	cld    
  800cdd:	f3 aa                	rep stos %al,%es:(%edi)
			:: "D" (v), "a" (c), "c" (n)
			: "cc", "memory");
	return v;
}
  800cdf:	89 f8                	mov    %edi,%eax
  800ce1:	5b                   	pop    %ebx
  800ce2:	5e                   	pop    %esi
  800ce3:	5f                   	pop    %edi
  800ce4:	5d                   	pop    %ebp
  800ce5:	c3                   	ret    

00800ce6 <memmove>:

void *
memmove(void *dst, const void *src, size_t n)
{
  800ce6:	55                   	push   %ebp
  800ce7:	89 e5                	mov    %esp,%ebp
  800ce9:	57                   	push   %edi
  800cea:	56                   	push   %esi
  800ceb:	8b 45 08             	mov    0x8(%ebp),%eax
  800cee:	8b 75 0c             	mov    0xc(%ebp),%esi
  800cf1:	8b 4d 10             	mov    0x10(%ebp),%ecx
	const char *s;
	char *d;

	s = src;
	d = dst;
	if (s < d && s + n > d) {
  800cf4:	39 c6                	cmp    %eax,%esi
  800cf6:	73 35                	jae    800d2d <memmove+0x47>
  800cf8:	8d 14 0e             	lea    (%esi,%ecx,1),%edx
  800cfb:	39 d0                	cmp    %edx,%eax
  800cfd:	73 2e                	jae    800d2d <memmove+0x47>
		s += n;
		d += n;
  800cff:	8d 3c 08             	lea    (%eax,%ecx,1),%edi
  800d02:	89 d6                	mov    %edx,%esi
  800d04:	09 fe                	or     %edi,%esi
		if ((int)s%4 == 0 && (int)d%4 == 0 && n%4 == 0)
  800d06:	f7 c6 03 00 00 00    	test   $0x3,%esi
  800d0c:	75 13                	jne    800d21 <memmove+0x3b>
  800d0e:	f6 c1 03             	test   $0x3,%cl
  800d11:	75 0e                	jne    800d21 <memmove+0x3b>
			asm volatile("std; rep movsl\n"
				:: "D" (d-4), "S" (s-4), "c" (n/4) : "cc", "memory");
  800d13:	83 ef 04             	sub    $0x4,%edi
  800d16:	8d 72 fc             	lea    -0x4(%edx),%esi
  800d19:	c1 e9 02             	shr    $0x2,%ecx
	d = dst;
	if (s < d && s + n > d) {
		s += n;
		d += n;
		if ((int)s%4 == 0 && (int)d%4 == 0 && n%4 == 0)
			asm volatile("std; rep movsl\n"
  800d1c:	fd                   	std    
  800d1d:	f3 a5                	rep movsl %ds:(%esi),%es:(%edi)
  800d1f:	eb 09                	jmp    800d2a <memmove+0x44>
				:: "D" (d-4), "S" (s-4), "c" (n/4) : "cc", "memory");
		else
			asm volatile("std; rep movsb\n"
				:: "D" (d-1), "S" (s-1), "c" (n) : "cc", "memory");
  800d21:	83 ef 01             	sub    $0x1,%edi
  800d24:	8d 72 ff             	lea    -0x1(%edx),%esi
		d += n;
		if ((int)s%4 == 0 && (int)d%4 == 0 && n%4 == 0)
			asm volatile("std; rep movsl\n"
				:: "D" (d-4), "S" (s-4), "c" (n/4) : "cc", "memory");
		else
			asm volatile("std; rep movsb\n"
  800d27:	fd                   	std    
  800d28:	f3 a4                	rep movsb %ds:(%esi),%es:(%edi)
				:: "D" (d-1), "S" (s-1), "c" (n) : "cc", "memory");
		// Some versions of GCC rely on DF being clear
		asm volatile("cld" ::: "cc");
  800d2a:	fc                   	cld    
  800d2b:	eb 1d                	jmp    800d4a <memmove+0x64>
  800d2d:	89 f2                	mov    %esi,%edx
  800d2f:	09 c2                	or     %eax,%edx
	} else {
		if ((int)s%4 == 0 && (int)d%4 == 0 && n%4 == 0)
  800d31:	f6 c2 03             	test   $0x3,%dl
  800d34:	75 0f                	jne    800d45 <memmove+0x5f>
  800d36:	f6 c1 03             	test   $0x3,%cl
  800d39:	75 0a                	jne    800d45 <memmove+0x5f>
			asm volatile("cld; rep movsl\n"
				:: "D" (d), "S" (s), "c" (n/4) : "cc", "memory");
  800d3b:	c1 e9 02             	shr    $0x2,%ecx
				:: "D" (d-1), "S" (s-1), "c" (n) : "cc", "memory");
		// Some versions of GCC rely on DF being clear
		asm volatile("cld" ::: "cc");
	} else {
		if ((int)s%4 == 0 && (int)d%4 == 0 && n%4 == 0)
			asm volatile("cld; rep movsl\n"
  800d3e:	89 c7                	mov    %eax,%edi
  800d40:	fc                   	cld    
  800d41:	f3 a5                	rep movsl %ds:(%esi),%es:(%edi)
  800d43:	eb 05                	jmp    800d4a <memmove+0x64>
				:: "D" (d), "S" (s), "c" (n/4) : "cc", "memory");
		else
			asm volatile("cld; rep movsb\n"
  800d45:	89 c7                	mov    %eax,%edi
  800d47:	fc                   	cld    
  800d48:	f3 a4                	rep movsb %ds:(%esi),%es:(%edi)
				:: "D" (d), "S" (s), "c" (n) : "cc", "memory");
	}
	return dst;
}
  800d4a:	5e                   	pop    %esi
  800d4b:	5f                   	pop    %edi
  800d4c:	5d                   	pop    %ebp
  800d4d:	c3                   	ret    

00800d4e <memcpy>:
}
#endif

void *
memcpy(void *dst, const void *src, size_t n)
{
  800d4e:	55                   	push   %ebp
  800d4f:	89 e5                	mov    %esp,%ebp
	return memmove(dst, src, n);
  800d51:	ff 75 10             	pushl  0x10(%ebp)
  800d54:	ff 75 0c             	pushl  0xc(%ebp)
  800d57:	ff 75 08             	pushl  0x8(%ebp)
  800d5a:	e8 87 ff ff ff       	call   800ce6 <memmove>
}
  800d5f:	c9                   	leave  
  800d60:	c3                   	ret    

00800d61 <memcmp>:

int
memcmp(const void *v1, const void *v2, size_t n)
{
  800d61:	55                   	push   %ebp
  800d62:	89 e5                	mov    %esp,%ebp
  800d64:	56                   	push   %esi
  800d65:	53                   	push   %ebx
  800d66:	8b 45 08             	mov    0x8(%ebp),%eax
  800d69:	8b 55 0c             	mov    0xc(%ebp),%edx
  800d6c:	89 c6                	mov    %eax,%esi
  800d6e:	03 75 10             	add    0x10(%ebp),%esi
	const uint8_t *s1 = (const uint8_t *) v1;
	const uint8_t *s2 = (const uint8_t *) v2;

	while (n-- > 0) {
  800d71:	eb 1a                	jmp    800d8d <memcmp+0x2c>
		if (*s1 != *s2)
  800d73:	0f b6 08             	movzbl (%eax),%ecx
  800d76:	0f b6 1a             	movzbl (%edx),%ebx
  800d79:	38 d9                	cmp    %bl,%cl
  800d7b:	74 0a                	je     800d87 <memcmp+0x26>
			return (int) *s1 - (int) *s2;
  800d7d:	0f b6 c1             	movzbl %cl,%eax
  800d80:	0f b6 db             	movzbl %bl,%ebx
  800d83:	29 d8                	sub    %ebx,%eax
  800d85:	eb 0f                	jmp    800d96 <memcmp+0x35>
		s1++, s2++;
  800d87:	83 c0 01             	add    $0x1,%eax
  800d8a:	83 c2 01             	add    $0x1,%edx
memcmp(const void *v1, const void *v2, size_t n)
{
	const uint8_t *s1 = (const uint8_t *) v1;
	const uint8_t *s2 = (const uint8_t *) v2;

	while (n-- > 0) {
  800d8d:	39 f0                	cmp    %esi,%eax
  800d8f:	75 e2                	jne    800d73 <memcmp+0x12>
		if (*s1 != *s2)
			return (int) *s1 - (int) *s2;
		s1++, s2++;
	}

	return 0;
  800d91:	b8 00 00 00 00       	mov    $0x0,%eax
}
  800d96:	5b                   	pop    %ebx
  800d97:	5e                   	pop    %esi
  800d98:	5d                   	pop    %ebp
  800d99:	c3                   	ret    

00800d9a <memfind>:

void *
memfind(const void *s, int c, size_t n)
{
  800d9a:	55                   	push   %ebp
  800d9b:	89 e5                	mov    %esp,%ebp
  800d9d:	8b 45 08             	mov    0x8(%ebp),%eax
  800da0:	8b 4d 0c             	mov    0xc(%ebp),%ecx
	const void *ends = (const char *) s + n;
  800da3:	89 c2                	mov    %eax,%edx
  800da5:	03 55 10             	add    0x10(%ebp),%edx
	for (; s < ends; s++)
  800da8:	eb 07                	jmp    800db1 <memfind+0x17>
		if (*(const unsigned char *) s == (unsigned char) c)
  800daa:	38 08                	cmp    %cl,(%eax)
  800dac:	74 07                	je     800db5 <memfind+0x1b>

void *
memfind(const void *s, int c, size_t n)
{
	const void *ends = (const char *) s + n;
	for (; s < ends; s++)
  800dae:	83 c0 01             	add    $0x1,%eax
  800db1:	39 d0                	cmp    %edx,%eax
  800db3:	72 f5                	jb     800daa <memfind+0x10>
		if (*(const unsigned char *) s == (unsigned char) c)
			break;
	return (void *) s;
}
  800db5:	5d                   	pop    %ebp
  800db6:	c3                   	ret    

00800db7 <strtol>:

long
strtol(const char *s, char **endptr, int base)
{
  800db7:	55                   	push   %ebp
  800db8:	89 e5                	mov    %esp,%ebp
  800dba:	57                   	push   %edi
  800dbb:	56                   	push   %esi
  800dbc:	53                   	push   %ebx
  800dbd:	8b 4d 08             	mov    0x8(%ebp),%ecx
  800dc0:	8b 5d 10             	mov    0x10(%ebp),%ebx
	int neg = 0;
	long val = 0;

	// gobble initial whitespace
	while (*s == ' ' || *s == '\t')
  800dc3:	eb 03                	jmp    800dc8 <strtol+0x11>
		s++;
  800dc5:	83 c1 01             	add    $0x1,%ecx
{
	int neg = 0;
	long val = 0;

	// gobble initial whitespace
	while (*s == ' ' || *s == '\t')
  800dc8:	0f b6 01             	movzbl (%ecx),%eax
  800dcb:	3c 09                	cmp    $0x9,%al
  800dcd:	74 f6                	je     800dc5 <strtol+0xe>
  800dcf:	3c 20                	cmp    $0x20,%al
  800dd1:	74 f2                	je     800dc5 <strtol+0xe>
		s++;

	// plus/minus sign
	if (*s == '+')
  800dd3:	3c 2b                	cmp    $0x2b,%al
  800dd5:	75 0a                	jne    800de1 <strtol+0x2a>
		s++;
  800dd7:	83 c1 01             	add    $0x1,%ecx
}

long
strtol(const char *s, char **endptr, int base)
{
	int neg = 0;
  800dda:	bf 00 00 00 00       	mov    $0x0,%edi
  800ddf:	eb 10                	jmp    800df1 <strtol+0x3a>
  800de1:	bf 00 00 00 00       	mov    $0x0,%edi
		s++;

	// plus/minus sign
	if (*s == '+')
		s++;
	else if (*s == '-')
  800de6:	3c 2d                	cmp    $0x2d,%al
  800de8:	75 07                	jne    800df1 <strtol+0x3a>
		s++, neg = 1;
  800dea:	8d 49 01             	lea    0x1(%ecx),%ecx
  800ded:	66 bf 01 00          	mov    $0x1,%di

	// hex or octal base prefix
	if ((base == 0 || base == 16) && (s[0] == '0' && s[1] == 'x'))
  800df1:	85 db                	test   %ebx,%ebx
  800df3:	0f 94 c0             	sete   %al
  800df6:	f7 c3 ef ff ff ff    	test   $0xffffffef,%ebx
  800dfc:	75 19                	jne    800e17 <strtol+0x60>
  800dfe:	80 39 30             	cmpb   $0x30,(%ecx)
  800e01:	75 14                	jne    800e17 <strtol+0x60>
  800e03:	80 79 01 78          	cmpb   $0x78,0x1(%ecx)
  800e07:	0f 85 82 00 00 00    	jne    800e8f <strtol+0xd8>
		s += 2, base = 16;
  800e0d:	83 c1 02             	add    $0x2,%ecx
  800e10:	bb 10 00 00 00       	mov    $0x10,%ebx
  800e15:	eb 16                	jmp    800e2d <strtol+0x76>
	else if (base == 0 && s[0] == '0')
  800e17:	84 c0                	test   %al,%al
  800e19:	74 12                	je     800e2d <strtol+0x76>
		s++, base = 8;
	else if (base == 0)
		base = 10;
  800e1b:	bb 0a 00 00 00       	mov    $0xa,%ebx
		s++, neg = 1;

	// hex or octal base prefix
	if ((base == 0 || base == 16) && (s[0] == '0' && s[1] == 'x'))
		s += 2, base = 16;
	else if (base == 0 && s[0] == '0')
  800e20:	80 39 30             	cmpb   $0x30,(%ecx)
  800e23:	75 08                	jne    800e2d <strtol+0x76>
		s++, base = 8;
  800e25:	83 c1 01             	add    $0x1,%ecx
  800e28:	bb 08 00 00 00       	mov    $0x8,%ebx
	else if (base == 0)
		base = 10;
  800e2d:	b8 00 00 00 00       	mov    $0x0,%eax
  800e32:	89 5d 10             	mov    %ebx,0x10(%ebp)

	// digits
	while (1) {
		int dig;

		if (*s >= '0' && *s <= '9')
  800e35:	0f b6 11             	movzbl (%ecx),%edx
  800e38:	8d 72 d0             	lea    -0x30(%edx),%esi
  800e3b:	89 f3                	mov    %esi,%ebx
  800e3d:	80 fb 09             	cmp    $0x9,%bl
  800e40:	77 08                	ja     800e4a <strtol+0x93>
			dig = *s - '0';
  800e42:	0f be d2             	movsbl %dl,%edx
  800e45:	83 ea 30             	sub    $0x30,%edx
  800e48:	eb 22                	jmp    800e6c <strtol+0xb5>
		else if (*s >= 'a' && *s <= 'z')
  800e4a:	8d 72 9f             	lea    -0x61(%edx),%esi
  800e4d:	89 f3                	mov    %esi,%ebx
  800e4f:	80 fb 19             	cmp    $0x19,%bl
  800e52:	77 08                	ja     800e5c <strtol+0xa5>
			dig = *s - 'a' + 10;
  800e54:	0f be d2             	movsbl %dl,%edx
  800e57:	83 ea 57             	sub    $0x57,%edx
  800e5a:	eb 10                	jmp    800e6c <strtol+0xb5>
		else if (*s >= 'A' && *s <= 'Z')
  800e5c:	8d 72 bf             	lea    -0x41(%edx),%esi
  800e5f:	89 f3                	mov    %esi,%ebx
  800e61:	80 fb 19             	cmp    $0x19,%bl
  800e64:	77 16                	ja     800e7c <strtol+0xc5>
			dig = *s - 'A' + 10;
  800e66:	0f be d2             	movsbl %dl,%edx
  800e69:	83 ea 37             	sub    $0x37,%edx
		else
			break;
		if (dig >= base)
  800e6c:	3b 55 10             	cmp    0x10(%ebp),%edx
  800e6f:	7d 0f                	jge    800e80 <strtol+0xc9>
			break;
		s++, val = (val * base) + dig;
  800e71:	83 c1 01             	add    $0x1,%ecx
  800e74:	0f af 45 10          	imul   0x10(%ebp),%eax
  800e78:	01 d0                	add    %edx,%eax
		// we don't properly detect overflow!
	}
  800e7a:	eb b9                	jmp    800e35 <strtol+0x7e>
  800e7c:	89 c2                	mov    %eax,%edx
  800e7e:	eb 02                	jmp    800e82 <strtol+0xcb>
  800e80:	89 c2                	mov    %eax,%edx

	if (endptr)
  800e82:	83 7d 0c 00          	cmpl   $0x0,0xc(%ebp)
  800e86:	74 0d                	je     800e95 <strtol+0xde>
		*endptr = (char *) s;
  800e88:	8b 75 0c             	mov    0xc(%ebp),%esi
  800e8b:	89 0e                	mov    %ecx,(%esi)
  800e8d:	eb 06                	jmp    800e95 <strtol+0xde>
		s++, neg = 1;

	// hex or octal base prefix
	if ((base == 0 || base == 16) && (s[0] == '0' && s[1] == 'x'))
		s += 2, base = 16;
	else if (base == 0 && s[0] == '0')
  800e8f:	84 c0                	test   %al,%al
  800e91:	75 92                	jne    800e25 <strtol+0x6e>
  800e93:	eb 98                	jmp    800e2d <strtol+0x76>
		// we don't properly detect overflow!
	}

	if (endptr)
		*endptr = (char *) s;
	return (neg ? -val : val);
  800e95:	f7 da                	neg    %edx
  800e97:	85 ff                	test   %edi,%edi
  800e99:	0f 45 c2             	cmovne %edx,%eax
}
  800e9c:	5b                   	pop    %ebx
  800e9d:	5e                   	pop    %esi
  800e9e:	5f                   	pop    %edi
  800e9f:	5d                   	pop    %ebp
  800ea0:	c3                   	ret    

00800ea1 <sys_cputs>:
	return ret;
}

void
sys_cputs(const char *s, size_t len)
{    
  800ea1:	55                   	push   %ebp
  800ea2:	89 e5                	mov    %esp,%ebp
  800ea4:	57                   	push   %edi
  800ea5:	56                   	push   %esi
  800ea6:	53                   	push   %ebx
	// return value.
	//
	// The last clause tells the assembler that this can
	// potentially change the condition codes and arbitrary
	// memory locations.
	asm volatile("int %1\n"
  800ea7:	b8 00 00 00 00       	mov    $0x0,%eax
  800eac:	8b 4d 0c             	mov    0xc(%ebp),%ecx
  800eaf:	8b 55 08             	mov    0x8(%ebp),%edx
  800eb2:	89 c3                	mov    %eax,%ebx
  800eb4:	89 c7                	mov    %eax,%edi
  800eb6:	89 c6                	mov    %eax,%esi
  800eb8:	cd 30                	int    $0x30

void
sys_cputs(const char *s, size_t len)
{    
	syscall(SYS_cputs, 0, (uint32_t)s, len, 0, 0, 0);
}
  800eba:	5b                   	pop    %ebx
  800ebb:	5e                   	pop    %esi
  800ebc:	5f                   	pop    %edi
  800ebd:	5d                   	pop    %ebp
  800ebe:	c3                   	ret    

00800ebf <sys_cgetc>:

int
sys_cgetc(void)
{
  800ebf:	55                   	push   %ebp
  800ec0:	89 e5                	mov    %esp,%ebp
  800ec2:	57                   	push   %edi
  800ec3:	56                   	push   %esi
  800ec4:	53                   	push   %ebx
	// return value.
	//
	// The last clause tells the assembler that this can
	// potentially change the condition codes and arbitrary
	// memory locations.
	asm volatile("int %1\n"
  800ec5:	ba 00 00 00 00       	mov    $0x0,%edx
  800eca:	b8 01 00 00 00       	mov    $0x1,%eax
  800ecf:	89 d1                	mov    %edx,%ecx
  800ed1:	89 d3                	mov    %edx,%ebx
  800ed3:	89 d7                	mov    %edx,%edi
  800ed5:	89 d6                	mov    %edx,%esi
  800ed7:	cd 30                	int    $0x30

int
sys_cgetc(void)
{
	return syscall(SYS_cgetc, 0, 0, 0, 0, 0, 0);
}
  800ed9:	5b                   	pop    %ebx
  800eda:	5e                   	pop    %esi
  800edb:	5f                   	pop    %edi
  800edc:	5d                   	pop    %ebp
  800edd:	c3                   	ret    

00800ede <sys_env_destroy>:

int
sys_env_destroy(envid_t envid)
{
  800ede:	55                   	push   %ebp
  800edf:	89 e5                	mov    %esp,%ebp
  800ee1:	57                   	push   %edi
  800ee2:	56                   	push   %esi
  800ee3:	53                   	push   %ebx
  800ee4:	83 ec 0c             	sub    $0xc,%esp
	// return value.
	//
	// The last clause tells the assembler that this can
	// potentially change the condition codes and arbitrary
	// memory locations.
	asm volatile("int %1\n"
  800ee7:	b9 00 00 00 00       	mov    $0x0,%ecx
  800eec:	b8 03 00 00 00       	mov    $0x3,%eax
  800ef1:	8b 55 08             	mov    0x8(%ebp),%edx
  800ef4:	89 cb                	mov    %ecx,%ebx
  800ef6:	89 cf                	mov    %ecx,%edi
  800ef8:	89 ce                	mov    %ecx,%esi
  800efa:	cd 30                	int    $0x30
		  "b" (a3),
		  "D" (a4),
		  "S" (a5)
		: "cc", "memory");

	if(check && ret > 0)
  800efc:	85 c0                	test   %eax,%eax
  800efe:	7e 17                	jle    800f17 <sys_env_destroy+0x39>
		panic("syscall %d returned %d (> 0)", num, ret);
  800f00:	83 ec 0c             	sub    $0xc,%esp
  800f03:	50                   	push   %eax
  800f04:	6a 03                	push   $0x3
  800f06:	68 1f 2c 80 00       	push   $0x802c1f
  800f0b:	6a 22                	push   $0x22
  800f0d:	68 3c 2c 80 00       	push   $0x802c3c
  800f12:	e8 5b 14 00 00       	call   802372 <_panic>

int
sys_env_destroy(envid_t envid)
{
	return syscall(SYS_env_destroy, 1, envid, 0, 0, 0, 0);
}
  800f17:	8d 65 f4             	lea    -0xc(%ebp),%esp
  800f1a:	5b                   	pop    %ebx
  800f1b:	5e                   	pop    %esi
  800f1c:	5f                   	pop    %edi
  800f1d:	5d                   	pop    %ebp
  800f1e:	c3                   	ret    

00800f1f <sys_getenvid>:

envid_t
sys_getenvid(void)
{                 
  800f1f:	55                   	push   %ebp
  800f20:	89 e5                	mov    %esp,%ebp
  800f22:	57                   	push   %edi
  800f23:	56                   	push   %esi
  800f24:	53                   	push   %ebx
	// return value.
	//
	// The last clause tells the assembler that this can
	// potentially change the condition codes and arbitrary
	// memory locations.
	asm volatile("int %1\n"
  800f25:	ba 00 00 00 00       	mov    $0x0,%edx
  800f2a:	b8 02 00 00 00       	mov    $0x2,%eax
  800f2f:	89 d1                	mov    %edx,%ecx
  800f31:	89 d3                	mov    %edx,%ebx
  800f33:	89 d7                	mov    %edx,%edi
  800f35:	89 d6                	mov    %edx,%esi
  800f37:	cd 30                	int    $0x30

envid_t
sys_getenvid(void)
{                 
	 return syscall(SYS_getenvid, 0, 0, 0, 0, 0, 0);
}
  800f39:	5b                   	pop    %ebx
  800f3a:	5e                   	pop    %esi
  800f3b:	5f                   	pop    %edi
  800f3c:	5d                   	pop    %ebp
  800f3d:	c3                   	ret    

00800f3e <sys_yield>:

void
sys_yield(void)
{      
  800f3e:	55                   	push   %ebp
  800f3f:	89 e5                	mov    %esp,%ebp
  800f41:	57                   	push   %edi
  800f42:	56                   	push   %esi
  800f43:	53                   	push   %ebx
	// return value.
	//
	// The last clause tells the assembler that this can
	// potentially change the condition codes and arbitrary
	// memory locations.
	asm volatile("int %1\n"
  800f44:	ba 00 00 00 00       	mov    $0x0,%edx
  800f49:	b8 0b 00 00 00       	mov    $0xb,%eax
  800f4e:	89 d1                	mov    %edx,%ecx
  800f50:	89 d3                	mov    %edx,%ebx
  800f52:	89 d7                	mov    %edx,%edi
  800f54:	89 d6                	mov    %edx,%esi
  800f56:	cd 30                	int    $0x30

void
sys_yield(void)
{      
	syscall(SYS_yield, 0, 0, 0, 0, 0, 0);
}
  800f58:	5b                   	pop    %ebx
  800f59:	5e                   	pop    %esi
  800f5a:	5f                   	pop    %edi
  800f5b:	5d                   	pop    %ebp
  800f5c:	c3                   	ret    

00800f5d <sys_page_alloc>:

int
sys_page_alloc(envid_t envid, void *va, int perm)
{
  800f5d:	55                   	push   %ebp
  800f5e:	89 e5                	mov    %esp,%ebp
  800f60:	57                   	push   %edi
  800f61:	56                   	push   %esi
  800f62:	53                   	push   %ebx
  800f63:	83 ec 0c             	sub    $0xc,%esp
	// return value.
	//
	// The last clause tells the assembler that this can
	// potentially change the condition codes and arbitrary
	// memory locations.
	asm volatile("int %1\n"
  800f66:	be 00 00 00 00       	mov    $0x0,%esi
  800f6b:	b8 04 00 00 00       	mov    $0x4,%eax
  800f70:	8b 4d 0c             	mov    0xc(%ebp),%ecx
  800f73:	8b 55 08             	mov    0x8(%ebp),%edx
  800f76:	8b 5d 10             	mov    0x10(%ebp),%ebx
  800f79:	89 f7                	mov    %esi,%edi
  800f7b:	cd 30                	int    $0x30
		  "b" (a3),
		  "D" (a4),
		  "S" (a5)
		: "cc", "memory");

	if(check && ret > 0)
  800f7d:	85 c0                	test   %eax,%eax
  800f7f:	7e 17                	jle    800f98 <sys_page_alloc+0x3b>
		panic("syscall %d returned %d (> 0)", num, ret);
  800f81:	83 ec 0c             	sub    $0xc,%esp
  800f84:	50                   	push   %eax
  800f85:	6a 04                	push   $0x4
  800f87:	68 1f 2c 80 00       	push   $0x802c1f
  800f8c:	6a 22                	push   $0x22
  800f8e:	68 3c 2c 80 00       	push   $0x802c3c
  800f93:	e8 da 13 00 00       	call   802372 <_panic>

int
sys_page_alloc(envid_t envid, void *va, int perm)
{
	return syscall(SYS_page_alloc, 1, envid, (uint32_t) va, perm, 0, 0);
}
  800f98:	8d 65 f4             	lea    -0xc(%ebp),%esp
  800f9b:	5b                   	pop    %ebx
  800f9c:	5e                   	pop    %esi
  800f9d:	5f                   	pop    %edi
  800f9e:	5d                   	pop    %ebp
  800f9f:	c3                   	ret    

00800fa0 <sys_page_map>:

int
sys_page_map(envid_t srcenv, void *srcva, envid_t dstenv, void *dstva, int perm)
{
  800fa0:	55                   	push   %ebp
  800fa1:	89 e5                	mov    %esp,%ebp
  800fa3:	57                   	push   %edi
  800fa4:	56                   	push   %esi
  800fa5:	53                   	push   %ebx
  800fa6:	83 ec 0c             	sub    $0xc,%esp
	// return value.
	//
	// The last clause tells the assembler that this can
	// potentially change the condition codes and arbitrary
	// memory locations.
	asm volatile("int %1\n"
  800fa9:	b8 05 00 00 00       	mov    $0x5,%eax
  800fae:	8b 4d 0c             	mov    0xc(%ebp),%ecx
  800fb1:	8b 55 08             	mov    0x8(%ebp),%edx
  800fb4:	8b 5d 10             	mov    0x10(%ebp),%ebx
  800fb7:	8b 7d 14             	mov    0x14(%ebp),%edi
  800fba:	8b 75 18             	mov    0x18(%ebp),%esi
  800fbd:	cd 30                	int    $0x30
		  "b" (a3),
		  "D" (a4),
		  "S" (a5)
		: "cc", "memory");

	if(check && ret > 0)
  800fbf:	85 c0                	test   %eax,%eax
  800fc1:	7e 17                	jle    800fda <sys_page_map+0x3a>
		panic("syscall %d returned %d (> 0)", num, ret);
  800fc3:	83 ec 0c             	sub    $0xc,%esp
  800fc6:	50                   	push   %eax
  800fc7:	6a 05                	push   $0x5
  800fc9:	68 1f 2c 80 00       	push   $0x802c1f
  800fce:	6a 22                	push   $0x22
  800fd0:	68 3c 2c 80 00       	push   $0x802c3c
  800fd5:	e8 98 13 00 00       	call   802372 <_panic>

int
sys_page_map(envid_t srcenv, void *srcva, envid_t dstenv, void *dstva, int perm)
{
	return syscall(SYS_page_map, 1, srcenv, (uint32_t) srcva, dstenv, (uint32_t) dstva, perm);
}
  800fda:	8d 65 f4             	lea    -0xc(%ebp),%esp
  800fdd:	5b                   	pop    %ebx
  800fde:	5e                   	pop    %esi
  800fdf:	5f                   	pop    %edi
  800fe0:	5d                   	pop    %ebp
  800fe1:	c3                   	ret    

00800fe2 <sys_page_unmap>:

int
sys_page_unmap(envid_t envid, void *va)
{
  800fe2:	55                   	push   %ebp
  800fe3:	89 e5                	mov    %esp,%ebp
  800fe5:	57                   	push   %edi
  800fe6:	56                   	push   %esi
  800fe7:	53                   	push   %ebx
  800fe8:	83 ec 0c             	sub    $0xc,%esp
	// return value.
	//
	// The last clause tells the assembler that this can
	// potentially change the condition codes and arbitrary
	// memory locations.
	asm volatile("int %1\n"
  800feb:	bb 00 00 00 00       	mov    $0x0,%ebx
  800ff0:	b8 06 00 00 00       	mov    $0x6,%eax
  800ff5:	8b 4d 0c             	mov    0xc(%ebp),%ecx
  800ff8:	8b 55 08             	mov    0x8(%ebp),%edx
  800ffb:	89 df                	mov    %ebx,%edi
  800ffd:	89 de                	mov    %ebx,%esi
  800fff:	cd 30                	int    $0x30
		  "b" (a3),
		  "D" (a4),
		  "S" (a5)
		: "cc", "memory");

	if(check && ret > 0)
  801001:	85 c0                	test   %eax,%eax
  801003:	7e 17                	jle    80101c <sys_page_unmap+0x3a>
		panic("syscall %d returned %d (> 0)", num, ret);
  801005:	83 ec 0c             	sub    $0xc,%esp
  801008:	50                   	push   %eax
  801009:	6a 06                	push   $0x6
  80100b:	68 1f 2c 80 00       	push   $0x802c1f
  801010:	6a 22                	push   $0x22
  801012:	68 3c 2c 80 00       	push   $0x802c3c
  801017:	e8 56 13 00 00       	call   802372 <_panic>

int
sys_page_unmap(envid_t envid, void *va)
{
	return syscall(SYS_page_unmap, 1, envid, (uint32_t) va, 0, 0, 0);
}
  80101c:	8d 65 f4             	lea    -0xc(%ebp),%esp
  80101f:	5b                   	pop    %ebx
  801020:	5e                   	pop    %esi
  801021:	5f                   	pop    %edi
  801022:	5d                   	pop    %ebp
  801023:	c3                   	ret    

00801024 <sys_env_set_status>:

// sys_exofork is inlined in lib.h

int
sys_env_set_status(envid_t envid, int status)
{
  801024:	55                   	push   %ebp
  801025:	89 e5                	mov    %esp,%ebp
  801027:	57                   	push   %edi
  801028:	56                   	push   %esi
  801029:	53                   	push   %ebx
  80102a:	83 ec 0c             	sub    $0xc,%esp
	// return value.
	//
	// The last clause tells the assembler that this can
	// potentially change the condition codes and arbitrary
	// memory locations.
	asm volatile("int %1\n"
  80102d:	bb 00 00 00 00       	mov    $0x0,%ebx
  801032:	b8 08 00 00 00       	mov    $0x8,%eax
  801037:	8b 4d 0c             	mov    0xc(%ebp),%ecx
  80103a:	8b 55 08             	mov    0x8(%ebp),%edx
  80103d:	89 df                	mov    %ebx,%edi
  80103f:	89 de                	mov    %ebx,%esi
  801041:	cd 30                	int    $0x30
		  "b" (a3),
		  "D" (a4),
		  "S" (a5)
		: "cc", "memory");

	if(check && ret > 0)
  801043:	85 c0                	test   %eax,%eax
  801045:	7e 17                	jle    80105e <sys_env_set_status+0x3a>
		panic("syscall %d returned %d (> 0)", num, ret);
  801047:	83 ec 0c             	sub    $0xc,%esp
  80104a:	50                   	push   %eax
  80104b:	6a 08                	push   $0x8
  80104d:	68 1f 2c 80 00       	push   $0x802c1f
  801052:	6a 22                	push   $0x22
  801054:	68 3c 2c 80 00       	push   $0x802c3c
  801059:	e8 14 13 00 00       	call   802372 <_panic>
sys_env_set_status(envid_t envid, int status)
{

	return syscall(SYS_env_set_status, 1, envid, status, 0, 0, 0);
             
}
  80105e:	8d 65 f4             	lea    -0xc(%ebp),%esp
  801061:	5b                   	pop    %ebx
  801062:	5e                   	pop    %esi
  801063:	5f                   	pop    %edi
  801064:	5d                   	pop    %ebp
  801065:	c3                   	ret    

00801066 <sys_env_set_trapframe>:

int
sys_env_set_trapframe(envid_t envid, struct Trapframe *tf)
{
  801066:	55                   	push   %ebp
  801067:	89 e5                	mov    %esp,%ebp
  801069:	57                   	push   %edi
  80106a:	56                   	push   %esi
  80106b:	53                   	push   %ebx
  80106c:	83 ec 0c             	sub    $0xc,%esp
	// return value.
	//
	// The last clause tells the assembler that this can
	// potentially change the condition codes and arbitrary
	// memory locations.
	asm volatile("int %1\n"
  80106f:	bb 00 00 00 00       	mov    $0x0,%ebx
  801074:	b8 09 00 00 00       	mov    $0x9,%eax
  801079:	8b 4d 0c             	mov    0xc(%ebp),%ecx
  80107c:	8b 55 08             	mov    0x8(%ebp),%edx
  80107f:	89 df                	mov    %ebx,%edi
  801081:	89 de                	mov    %ebx,%esi
  801083:	cd 30                	int    $0x30
		  "b" (a3),
		  "D" (a4),
		  "S" (a5)
		: "cc", "memory");

	if(check && ret > 0)
  801085:	85 c0                	test   %eax,%eax
  801087:	7e 17                	jle    8010a0 <sys_env_set_trapframe+0x3a>
		panic("syscall %d returned %d (> 0)", num, ret);
  801089:	83 ec 0c             	sub    $0xc,%esp
  80108c:	50                   	push   %eax
  80108d:	6a 09                	push   $0x9
  80108f:	68 1f 2c 80 00       	push   $0x802c1f
  801094:	6a 22                	push   $0x22
  801096:	68 3c 2c 80 00       	push   $0x802c3c
  80109b:	e8 d2 12 00 00       	call   802372 <_panic>

int
sys_env_set_trapframe(envid_t envid, struct Trapframe *tf)
{
	return syscall(SYS_env_set_trapframe, 1, envid, (uint32_t) tf, 0, 0, 0);
}
  8010a0:	8d 65 f4             	lea    -0xc(%ebp),%esp
  8010a3:	5b                   	pop    %ebx
  8010a4:	5e                   	pop    %esi
  8010a5:	5f                   	pop    %edi
  8010a6:	5d                   	pop    %ebp
  8010a7:	c3                   	ret    

008010a8 <sys_env_set_pgfault_upcall>:

int
sys_env_set_pgfault_upcall(envid_t envid, void *upcall)
{
  8010a8:	55                   	push   %ebp
  8010a9:	89 e5                	mov    %esp,%ebp
  8010ab:	57                   	push   %edi
  8010ac:	56                   	push   %esi
  8010ad:	53                   	push   %ebx
  8010ae:	83 ec 0c             	sub    $0xc,%esp
	// return value.
	//
	// The last clause tells the assembler that this can
	// potentially change the condition codes and arbitrary
	// memory locations.
	asm volatile("int %1\n"
  8010b1:	bb 00 00 00 00       	mov    $0x0,%ebx
  8010b6:	b8 0a 00 00 00       	mov    $0xa,%eax
  8010bb:	8b 4d 0c             	mov    0xc(%ebp),%ecx
  8010be:	8b 55 08             	mov    0x8(%ebp),%edx
  8010c1:	89 df                	mov    %ebx,%edi
  8010c3:	89 de                	mov    %ebx,%esi
  8010c5:	cd 30                	int    $0x30
		  "b" (a3),
		  "D" (a4),
		  "S" (a5)
		: "cc", "memory");

	if(check && ret > 0)
  8010c7:	85 c0                	test   %eax,%eax
  8010c9:	7e 17                	jle    8010e2 <sys_env_set_pgfault_upcall+0x3a>
		panic("syscall %d returned %d (> 0)", num, ret);
  8010cb:	83 ec 0c             	sub    $0xc,%esp
  8010ce:	50                   	push   %eax
  8010cf:	6a 0a                	push   $0xa
  8010d1:	68 1f 2c 80 00       	push   $0x802c1f
  8010d6:	6a 22                	push   $0x22
  8010d8:	68 3c 2c 80 00       	push   $0x802c3c
  8010dd:	e8 90 12 00 00       	call   802372 <_panic>

int
sys_env_set_pgfault_upcall(envid_t envid, void *upcall)
{
	return syscall(SYS_env_set_pgfault_upcall, 1, envid, (uint32_t) upcall, 0, 0, 0);
}
  8010e2:	8d 65 f4             	lea    -0xc(%ebp),%esp
  8010e5:	5b                   	pop    %ebx
  8010e6:	5e                   	pop    %esi
  8010e7:	5f                   	pop    %edi
  8010e8:	5d                   	pop    %ebp
  8010e9:	c3                   	ret    

008010ea <sys_ipc_try_send>:

int
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, int perm)
{
  8010ea:	55                   	push   %ebp
  8010eb:	89 e5                	mov    %esp,%ebp
  8010ed:	57                   	push   %edi
  8010ee:	56                   	push   %esi
  8010ef:	53                   	push   %ebx
	// return value.
	//
	// The last clause tells the assembler that this can
	// potentially change the condition codes and arbitrary
	// memory locations.
	asm volatile("int %1\n"
  8010f0:	be 00 00 00 00       	mov    $0x0,%esi
  8010f5:	b8 0c 00 00 00       	mov    $0xc,%eax
  8010fa:	8b 4d 0c             	mov    0xc(%ebp),%ecx
  8010fd:	8b 55 08             	mov    0x8(%ebp),%edx
  801100:	8b 5d 10             	mov    0x10(%ebp),%ebx
  801103:	8b 7d 14             	mov    0x14(%ebp),%edi
  801106:	cd 30                	int    $0x30

int
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, int perm)
{
	return syscall(SYS_ipc_try_send, 0, envid, value, (uint32_t) srcva, perm, 0);
}
  801108:	5b                   	pop    %ebx
  801109:	5e                   	pop    %esi
  80110a:	5f                   	pop    %edi
  80110b:	5d                   	pop    %ebp
  80110c:	c3                   	ret    

0080110d <sys_ipc_recv>:

int
sys_ipc_recv(void *dstva)
{  
  80110d:	55                   	push   %ebp
  80110e:	89 e5                	mov    %esp,%ebp
  801110:	57                   	push   %edi
  801111:	56                   	push   %esi
  801112:	53                   	push   %ebx
  801113:	83 ec 0c             	sub    $0xc,%esp
	// return value.
	//
	// The last clause tells the assembler that this can
	// potentially change the condition codes and arbitrary
	// memory locations.
	asm volatile("int %1\n"
  801116:	b9 00 00 00 00       	mov    $0x0,%ecx
  80111b:	b8 0d 00 00 00       	mov    $0xd,%eax
  801120:	8b 55 08             	mov    0x8(%ebp),%edx
  801123:	89 cb                	mov    %ecx,%ebx
  801125:	89 cf                	mov    %ecx,%edi
  801127:	89 ce                	mov    %ecx,%esi
  801129:	cd 30                	int    $0x30
		  "b" (a3),
		  "D" (a4),
		  "S" (a5)
		: "cc", "memory");

	if(check && ret > 0)
  80112b:	85 c0                	test   %eax,%eax
  80112d:	7e 17                	jle    801146 <sys_ipc_recv+0x39>
		panic("syscall %d returned %d (> 0)", num, ret);
  80112f:	83 ec 0c             	sub    $0xc,%esp
  801132:	50                   	push   %eax
  801133:	6a 0d                	push   $0xd
  801135:	68 1f 2c 80 00       	push   $0x802c1f
  80113a:	6a 22                	push   $0x22
  80113c:	68 3c 2c 80 00       	push   $0x802c3c
  801141:	e8 2c 12 00 00       	call   802372 <_panic>

int
sys_ipc_recv(void *dstva)
{  
	return syscall(SYS_ipc_recv, 1, (uint32_t)dstva, 0, 0, 0, 0);
}
  801146:	8d 65 f4             	lea    -0xc(%ebp),%esp
  801149:	5b                   	pop    %ebx
  80114a:	5e                   	pop    %esi
  80114b:	5f                   	pop    %edi
  80114c:	5d                   	pop    %ebp
  80114d:	c3                   	ret    

0080114e <sys_time_msec>:

unsigned int
sys_time_msec(void)
{       
  80114e:	55                   	push   %ebp
  80114f:	89 e5                	mov    %esp,%ebp
  801151:	57                   	push   %edi
  801152:	56                   	push   %esi
  801153:	53                   	push   %ebx
	// return value.
	//
	// The last clause tells the assembler that this can
	// potentially change the condition codes and arbitrary
	// memory locations.
	asm volatile("int %1\n"
  801154:	ba 00 00 00 00       	mov    $0x0,%edx
  801159:	b8 0e 00 00 00       	mov    $0xe,%eax
  80115e:	89 d1                	mov    %edx,%ecx
  801160:	89 d3                	mov    %edx,%ebx
  801162:	89 d7                	mov    %edx,%edi
  801164:	89 d6                	mov    %edx,%esi
  801166:	cd 30                	int    $0x30
sys_time_msec(void)
{       
         
	return (unsigned int) syscall(SYS_time_msec, 0, 0, 0, 0, 0, 0);
        
}
  801168:	5b                   	pop    %ebx
  801169:	5e                   	pop    %esi
  80116a:	5f                   	pop    %edi
  80116b:	5d                   	pop    %ebp
  80116c:	c3                   	ret    

0080116d <sys_transmit>:

int
sys_transmit(void *addr)
{
  80116d:	55                   	push   %ebp
  80116e:	89 e5                	mov    %esp,%ebp
  801170:	57                   	push   %edi
  801171:	56                   	push   %esi
  801172:	53                   	push   %ebx
  801173:	83 ec 0c             	sub    $0xc,%esp
	// return value.
	//
	// The last clause tells the assembler that this can
	// potentially change the condition codes and arbitrary
	// memory locations.
	asm volatile("int %1\n"
  801176:	b9 00 00 00 00       	mov    $0x0,%ecx
  80117b:	b8 0f 00 00 00       	mov    $0xf,%eax
  801180:	8b 55 08             	mov    0x8(%ebp),%edx
  801183:	89 cb                	mov    %ecx,%ebx
  801185:	89 cf                	mov    %ecx,%edi
  801187:	89 ce                	mov    %ecx,%esi
  801189:	cd 30                	int    $0x30
		  "b" (a3),
		  "D" (a4),
		  "S" (a5)
		: "cc", "memory");

	if(check && ret > 0)
  80118b:	85 c0                	test   %eax,%eax
  80118d:	7e 17                	jle    8011a6 <sys_transmit+0x39>
		panic("syscall %d returned %d (> 0)", num, ret);
  80118f:	83 ec 0c             	sub    $0xc,%esp
  801192:	50                   	push   %eax
  801193:	6a 0f                	push   $0xf
  801195:	68 1f 2c 80 00       	push   $0x802c1f
  80119a:	6a 22                	push   $0x22
  80119c:	68 3c 2c 80 00       	push   $0x802c3c
  8011a1:	e8 cc 11 00 00       	call   802372 <_panic>

int
sys_transmit(void *addr)
{
        return syscall(SYS_transmit, 1, (uint32_t)addr, 0, 0, 0, 0);
}
  8011a6:	8d 65 f4             	lea    -0xc(%ebp),%esp
  8011a9:	5b                   	pop    %ebx
  8011aa:	5e                   	pop    %esi
  8011ab:	5f                   	pop    %edi
  8011ac:	5d                   	pop    %ebp
  8011ad:	c3                   	ret    

008011ae <sys_recv>:

int
sys_recv(void *addr)
{
  8011ae:	55                   	push   %ebp
  8011af:	89 e5                	mov    %esp,%ebp
  8011b1:	57                   	push   %edi
  8011b2:	56                   	push   %esi
  8011b3:	53                   	push   %ebx
  8011b4:	83 ec 0c             	sub    $0xc,%esp
	// return value.
	//
	// The last clause tells the assembler that this can
	// potentially change the condition codes and arbitrary
	// memory locations.
	asm volatile("int %1\n"
  8011b7:	b9 00 00 00 00       	mov    $0x0,%ecx
  8011bc:	b8 10 00 00 00       	mov    $0x10,%eax
  8011c1:	8b 55 08             	mov    0x8(%ebp),%edx
  8011c4:	89 cb                	mov    %ecx,%ebx
  8011c6:	89 cf                	mov    %ecx,%edi
  8011c8:	89 ce                	mov    %ecx,%esi
  8011ca:	cd 30                	int    $0x30
		  "b" (a3),
		  "D" (a4),
		  "S" (a5)
		: "cc", "memory");

	if(check && ret > 0)
  8011cc:	85 c0                	test   %eax,%eax
  8011ce:	7e 17                	jle    8011e7 <sys_recv+0x39>
		panic("syscall %d returned %d (> 0)", num, ret);
  8011d0:	83 ec 0c             	sub    $0xc,%esp
  8011d3:	50                   	push   %eax
  8011d4:	6a 10                	push   $0x10
  8011d6:	68 1f 2c 80 00       	push   $0x802c1f
  8011db:	6a 22                	push   $0x22
  8011dd:	68 3c 2c 80 00       	push   $0x802c3c
  8011e2:	e8 8b 11 00 00       	call   802372 <_panic>

int
sys_recv(void *addr)
{
        return syscall(SYS_recv, 1, (uint32_t)addr, 0, 0, 0, 0);
}
  8011e7:	8d 65 f4             	lea    -0xc(%ebp),%esp
  8011ea:	5b                   	pop    %ebx
  8011eb:	5e                   	pop    %esi
  8011ec:	5f                   	pop    %edi
  8011ed:	5d                   	pop    %ebp
  8011ee:	c3                   	ret    

008011ef <fd2num>:
// File descriptor manipulators
// --------------------------------------------------------------

int
fd2num(struct Fd *fd)
{
  8011ef:	55                   	push   %ebp
  8011f0:	89 e5                	mov    %esp,%ebp
	return ((uintptr_t) fd - FDTABLE) / PGSIZE;
  8011f2:	8b 45 08             	mov    0x8(%ebp),%eax
  8011f5:	05 00 00 00 30       	add    $0x30000000,%eax
  8011fa:	c1 e8 0c             	shr    $0xc,%eax
}
  8011fd:	5d                   	pop    %ebp
  8011fe:	c3                   	ret    

008011ff <fd2data>:

char*
fd2data(struct Fd *fd)
{
  8011ff:	55                   	push   %ebp
  801200:	89 e5                	mov    %esp,%ebp
// --------------------------------------------------------------

int
fd2num(struct Fd *fd)
{
	return ((uintptr_t) fd - FDTABLE) / PGSIZE;
  801202:	8b 45 08             	mov    0x8(%ebp),%eax
  801205:	05 00 00 00 30       	add    $0x30000000,%eax
}

char*
fd2data(struct Fd *fd)
{
	return INDEX2DATA(fd2num(fd));
  80120a:	25 00 f0 ff ff       	and    $0xfffff000,%eax
  80120f:	2d 00 00 fe 2f       	sub    $0x2ffe0000,%eax
}
  801214:	5d                   	pop    %ebp
  801215:	c3                   	ret    

00801216 <fd_alloc>:
// Returns 0 on success, < 0 on error.  Errors are:
//	-E_MAX_FD: no more file descriptors
// On error, *fd_store is set to 0.
int
fd_alloc(struct Fd **fd_store)
{
  801216:	55                   	push   %ebp
  801217:	89 e5                	mov    %esp,%ebp
  801219:	8b 4d 08             	mov    0x8(%ebp),%ecx
  80121c:	b8 00 00 00 d0       	mov    $0xd0000000,%eax
	int i;
	struct Fd *fd;

	for (i = 0; i < MAXFD; i++) {
		fd = INDEX2FD(i);
		if ((uvpd[PDX(fd)] & PTE_P) == 0 || (uvpt[PGNUM(fd)] & PTE_P) == 0) {
  801221:	89 c2                	mov    %eax,%edx
  801223:	c1 ea 16             	shr    $0x16,%edx
  801226:	8b 14 95 00 d0 7b ef 	mov    -0x10843000(,%edx,4),%edx
  80122d:	f6 c2 01             	test   $0x1,%dl
  801230:	74 11                	je     801243 <fd_alloc+0x2d>
  801232:	89 c2                	mov    %eax,%edx
  801234:	c1 ea 0c             	shr    $0xc,%edx
  801237:	8b 14 95 00 00 40 ef 	mov    -0x10c00000(,%edx,4),%edx
  80123e:	f6 c2 01             	test   $0x1,%dl
  801241:	75 09                	jne    80124c <fd_alloc+0x36>
			*fd_store = fd;
  801243:	89 01                	mov    %eax,(%ecx)
			return 0;
  801245:	b8 00 00 00 00       	mov    $0x0,%eax
  80124a:	eb 17                	jmp    801263 <fd_alloc+0x4d>
  80124c:	05 00 10 00 00       	add    $0x1000,%eax
fd_alloc(struct Fd **fd_store)
{
	int i;
	struct Fd *fd;

	for (i = 0; i < MAXFD; i++) {
  801251:	3d 00 00 02 d0       	cmp    $0xd0020000,%eax
  801256:	75 c9                	jne    801221 <fd_alloc+0xb>
		if ((uvpd[PDX(fd)] & PTE_P) == 0 || (uvpt[PGNUM(fd)] & PTE_P) == 0) {
			*fd_store = fd;
			return 0;
		}
	}
	*fd_store = 0;
  801258:	c7 01 00 00 00 00    	movl   $0x0,(%ecx)
	return -E_MAX_OPEN;
  80125e:	b8 f6 ff ff ff       	mov    $0xfffffff6,%eax
}
  801263:	5d                   	pop    %ebp
  801264:	c3                   	ret    

00801265 <fd_lookup>:
// Returns 0 on success (the page is in range and mapped), < 0 on error.
// Errors are:
//	-E_INVAL: fdnum was either not in range or not mapped.
int
fd_lookup(int fdnum, struct Fd **fd_store)
{
  801265:	55                   	push   %ebp
  801266:	89 e5                	mov    %esp,%ebp
  801268:	8b 45 08             	mov    0x8(%ebp),%eax
	struct Fd *fd;

	if (fdnum < 0 || fdnum >= MAXFD) {
  80126b:	83 f8 1f             	cmp    $0x1f,%eax
  80126e:	77 36                	ja     8012a6 <fd_lookup+0x41>
		if (debug)
			cprintf("[%08x] bad fd %d\n", thisenv->env_id, fdnum);
		return -E_INVAL;
	}
	fd = INDEX2FD(fdnum);
  801270:	c1 e0 0c             	shl    $0xc,%eax
  801273:	2d 00 00 00 30       	sub    $0x30000000,%eax
	if (!(uvpd[PDX(fd)] & PTE_P) || !(uvpt[PGNUM(fd)] & PTE_P)) {
  801278:	89 c2                	mov    %eax,%edx
  80127a:	c1 ea 16             	shr    $0x16,%edx
  80127d:	8b 14 95 00 d0 7b ef 	mov    -0x10843000(,%edx,4),%edx
  801284:	f6 c2 01             	test   $0x1,%dl
  801287:	74 24                	je     8012ad <fd_lookup+0x48>
  801289:	89 c2                	mov    %eax,%edx
  80128b:	c1 ea 0c             	shr    $0xc,%edx
  80128e:	8b 14 95 00 00 40 ef 	mov    -0x10c00000(,%edx,4),%edx
  801295:	f6 c2 01             	test   $0x1,%dl
  801298:	74 1a                	je     8012b4 <fd_lookup+0x4f>
		if (debug)
			cprintf("[%08x] closed fd %d\n", thisenv->env_id, fdnum);
		return -E_INVAL;
	}
	*fd_store = fd;
  80129a:	8b 55 0c             	mov    0xc(%ebp),%edx
  80129d:	89 02                	mov    %eax,(%edx)
	return 0;
  80129f:	b8 00 00 00 00       	mov    $0x0,%eax
  8012a4:	eb 13                	jmp    8012b9 <fd_lookup+0x54>
	struct Fd *fd;

	if (fdnum < 0 || fdnum >= MAXFD) {
		if (debug)
			cprintf("[%08x] bad fd %d\n", thisenv->env_id, fdnum);
		return -E_INVAL;
  8012a6:	b8 fd ff ff ff       	mov    $0xfffffffd,%eax
  8012ab:	eb 0c                	jmp    8012b9 <fd_lookup+0x54>
	}
	fd = INDEX2FD(fdnum);
	if (!(uvpd[PDX(fd)] & PTE_P) || !(uvpt[PGNUM(fd)] & PTE_P)) {
		if (debug)
			cprintf("[%08x] closed fd %d\n", thisenv->env_id, fdnum);
		return -E_INVAL;
  8012ad:	b8 fd ff ff ff       	mov    $0xfffffffd,%eax
  8012b2:	eb 05                	jmp    8012b9 <fd_lookup+0x54>
  8012b4:	b8 fd ff ff ff       	mov    $0xfffffffd,%eax
	}
	*fd_store = fd;
	return 0;
}
  8012b9:	5d                   	pop    %ebp
  8012ba:	c3                   	ret    

008012bb <dev_lookup>:
	0
};

int
dev_lookup(int dev_id, struct Dev **dev)
{
  8012bb:	55                   	push   %ebp
  8012bc:	89 e5                	mov    %esp,%ebp
  8012be:	83 ec 08             	sub    $0x8,%esp
  8012c1:	8b 4d 08             	mov    0x8(%ebp),%ecx
	int i;
	for (i = 0; devtab[i]; i++)
  8012c4:	ba 00 00 00 00       	mov    $0x0,%edx
  8012c9:	eb 13                	jmp    8012de <dev_lookup+0x23>
		if (devtab[i]->dev_id == dev_id) {
  8012cb:	39 08                	cmp    %ecx,(%eax)
  8012cd:	75 0c                	jne    8012db <dev_lookup+0x20>
			*dev = devtab[i];
  8012cf:	8b 4d 0c             	mov    0xc(%ebp),%ecx
  8012d2:	89 01                	mov    %eax,(%ecx)
			return 0;
  8012d4:	b8 00 00 00 00       	mov    $0x0,%eax
  8012d9:	eb 36                	jmp    801311 <dev_lookup+0x56>

int
dev_lookup(int dev_id, struct Dev **dev)
{
	int i;
	for (i = 0; devtab[i]; i++)
  8012db:	83 c2 01             	add    $0x1,%edx
  8012de:	8b 04 95 c8 2c 80 00 	mov    0x802cc8(,%edx,4),%eax
  8012e5:	85 c0                	test   %eax,%eax
  8012e7:	75 e2                	jne    8012cb <dev_lookup+0x10>
		if (devtab[i]->dev_id == dev_id) {
			*dev = devtab[i];
			return 0;
		}
	cprintf("[%08x] unknown device type %d\n", thisenv->env_id, dev_id);
  8012e9:	a1 18 40 80 00       	mov    0x804018,%eax
  8012ee:	8b 40 48             	mov    0x48(%eax),%eax
  8012f1:	83 ec 04             	sub    $0x4,%esp
  8012f4:	51                   	push   %ecx
  8012f5:	50                   	push   %eax
  8012f6:	68 4c 2c 80 00       	push   $0x802c4c
  8012fb:	e8 cd f2 ff ff       	call   8005cd <cprintf>
	*dev = 0;
  801300:	8b 45 0c             	mov    0xc(%ebp),%eax
  801303:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
	return -E_INVAL;
  801309:	83 c4 10             	add    $0x10,%esp
  80130c:	b8 fd ff ff ff       	mov    $0xfffffffd,%eax
}
  801311:	c9                   	leave  
  801312:	c3                   	ret    

00801313 <fd_close>:
// If 'must_exist' is 1, then fd_close returns -E_INVAL when passed a
// closed or nonexistent file descriptor.
// Returns 0 on success, < 0 on error.
int
fd_close(struct Fd *fd, bool must_exist)
{
  801313:	55                   	push   %ebp
  801314:	89 e5                	mov    %esp,%ebp
  801316:	56                   	push   %esi
  801317:	53                   	push   %ebx
  801318:	83 ec 10             	sub    $0x10,%esp
  80131b:	8b 75 08             	mov    0x8(%ebp),%esi
  80131e:	8b 5d 0c             	mov    0xc(%ebp),%ebx
	struct Fd *fd2;
	struct Dev *dev;
	int r;
	if ((r = fd_lookup(fd2num(fd), &fd2)) < 0
  801321:	8d 45 f4             	lea    -0xc(%ebp),%eax
  801324:	50                   	push   %eax
// --------------------------------------------------------------

int
fd2num(struct Fd *fd)
{
	return ((uintptr_t) fd - FDTABLE) / PGSIZE;
  801325:	8d 86 00 00 00 30    	lea    0x30000000(%esi),%eax
  80132b:	c1 e8 0c             	shr    $0xc,%eax
fd_close(struct Fd *fd, bool must_exist)
{
	struct Fd *fd2;
	struct Dev *dev;
	int r;
	if ((r = fd_lookup(fd2num(fd), &fd2)) < 0
  80132e:	50                   	push   %eax
  80132f:	e8 31 ff ff ff       	call   801265 <fd_lookup>
  801334:	83 c4 08             	add    $0x8,%esp
  801337:	85 c0                	test   %eax,%eax
  801339:	78 05                	js     801340 <fd_close+0x2d>
	    || fd != fd2)
  80133b:	3b 75 f4             	cmp    -0xc(%ebp),%esi
  80133e:	74 0c                	je     80134c <fd_close+0x39>
		return (must_exist ? r : 0);
  801340:	84 db                	test   %bl,%bl
  801342:	ba 00 00 00 00       	mov    $0x0,%edx
  801347:	0f 44 c2             	cmove  %edx,%eax
  80134a:	eb 41                	jmp    80138d <fd_close+0x7a>
	if ((r = dev_lookup(fd->fd_dev_id, &dev)) >= 0) {
  80134c:	83 ec 08             	sub    $0x8,%esp
  80134f:	8d 45 f0             	lea    -0x10(%ebp),%eax
  801352:	50                   	push   %eax
  801353:	ff 36                	pushl  (%esi)
  801355:	e8 61 ff ff ff       	call   8012bb <dev_lookup>
  80135a:	89 c3                	mov    %eax,%ebx
  80135c:	83 c4 10             	add    $0x10,%esp
  80135f:	85 c0                	test   %eax,%eax
  801361:	78 1a                	js     80137d <fd_close+0x6a>
		if (dev->dev_close)
  801363:	8b 45 f0             	mov    -0x10(%ebp),%eax
  801366:	8b 40 10             	mov    0x10(%eax),%eax
			r = (*dev->dev_close)(fd);
		else
			r = 0;
  801369:	bb 00 00 00 00       	mov    $0x0,%ebx
	int r;
	if ((r = fd_lookup(fd2num(fd), &fd2)) < 0
	    || fd != fd2)
		return (must_exist ? r : 0);
	if ((r = dev_lookup(fd->fd_dev_id, &dev)) >= 0) {
		if (dev->dev_close)
  80136e:	85 c0                	test   %eax,%eax
  801370:	74 0b                	je     80137d <fd_close+0x6a>
			r = (*dev->dev_close)(fd);
  801372:	83 ec 0c             	sub    $0xc,%esp
  801375:	56                   	push   %esi
  801376:	ff d0                	call   *%eax
  801378:	89 c3                	mov    %eax,%ebx
  80137a:	83 c4 10             	add    $0x10,%esp
		else
			r = 0;
	}
	// Make sure fd is unmapped.  Might be a no-op if
	// (*dev->dev_close)(fd) already unmapped it.
	(void) sys_page_unmap(0, fd);
  80137d:	83 ec 08             	sub    $0x8,%esp
  801380:	56                   	push   %esi
  801381:	6a 00                	push   $0x0
  801383:	e8 5a fc ff ff       	call   800fe2 <sys_page_unmap>
	return r;
  801388:	83 c4 10             	add    $0x10,%esp
  80138b:	89 d8                	mov    %ebx,%eax
}
  80138d:	8d 65 f8             	lea    -0x8(%ebp),%esp
  801390:	5b                   	pop    %ebx
  801391:	5e                   	pop    %esi
  801392:	5d                   	pop    %ebp
  801393:	c3                   	ret    

00801394 <close>:
	return -E_INVAL;
}

int
close(int fdnum)
{
  801394:	55                   	push   %ebp
  801395:	89 e5                	mov    %esp,%ebp
  801397:	83 ec 18             	sub    $0x18,%esp
	struct Fd *fd;
	int r;

	if ((r = fd_lookup(fdnum, &fd)) < 0)
  80139a:	8d 45 f4             	lea    -0xc(%ebp),%eax
  80139d:	50                   	push   %eax
  80139e:	ff 75 08             	pushl  0x8(%ebp)
  8013a1:	e8 bf fe ff ff       	call   801265 <fd_lookup>
  8013a6:	89 c2                	mov    %eax,%edx
  8013a8:	83 c4 08             	add    $0x8,%esp
  8013ab:	85 d2                	test   %edx,%edx
  8013ad:	78 10                	js     8013bf <close+0x2b>
		return r;
	else
		return fd_close(fd, 1);
  8013af:	83 ec 08             	sub    $0x8,%esp
  8013b2:	6a 01                	push   $0x1
  8013b4:	ff 75 f4             	pushl  -0xc(%ebp)
  8013b7:	e8 57 ff ff ff       	call   801313 <fd_close>
  8013bc:	83 c4 10             	add    $0x10,%esp
}
  8013bf:	c9                   	leave  
  8013c0:	c3                   	ret    

008013c1 <close_all>:

void
close_all(void)
{
  8013c1:	55                   	push   %ebp
  8013c2:	89 e5                	mov    %esp,%ebp
  8013c4:	53                   	push   %ebx
  8013c5:	83 ec 04             	sub    $0x4,%esp
	int i;
	for (i = 0; i < MAXFD; i++)
  8013c8:	bb 00 00 00 00       	mov    $0x0,%ebx
		close(i);
  8013cd:	83 ec 0c             	sub    $0xc,%esp
  8013d0:	53                   	push   %ebx
  8013d1:	e8 be ff ff ff       	call   801394 <close>

void
close_all(void)
{
	int i;
	for (i = 0; i < MAXFD; i++)
  8013d6:	83 c3 01             	add    $0x1,%ebx
  8013d9:	83 c4 10             	add    $0x10,%esp
  8013dc:	83 fb 20             	cmp    $0x20,%ebx
  8013df:	75 ec                	jne    8013cd <close_all+0xc>
		close(i);
}
  8013e1:	8b 5d fc             	mov    -0x4(%ebp),%ebx
  8013e4:	c9                   	leave  
  8013e5:	c3                   	ret    

008013e6 <dup>:
// file and the file offset of the other.
// Closes any previously open file descriptor at 'newfdnum'.
// This is implemented using virtual memory tricks (of course!).
int
dup(int oldfdnum, int newfdnum)
{
  8013e6:	55                   	push   %ebp
  8013e7:	89 e5                	mov    %esp,%ebp
  8013e9:	57                   	push   %edi
  8013ea:	56                   	push   %esi
  8013eb:	53                   	push   %ebx
  8013ec:	83 ec 2c             	sub    $0x2c,%esp
  8013ef:	8b 75 0c             	mov    0xc(%ebp),%esi
	int r;
	char *ova, *nva;
	pte_t pte;
	struct Fd *oldfd, *newfd;

	if ((r = fd_lookup(oldfdnum, &oldfd)) < 0)
  8013f2:	8d 45 e4             	lea    -0x1c(%ebp),%eax
  8013f5:	50                   	push   %eax
  8013f6:	ff 75 08             	pushl  0x8(%ebp)
  8013f9:	e8 67 fe ff ff       	call   801265 <fd_lookup>
  8013fe:	89 c2                	mov    %eax,%edx
  801400:	83 c4 08             	add    $0x8,%esp
  801403:	85 d2                	test   %edx,%edx
  801405:	0f 88 c1 00 00 00    	js     8014cc <dup+0xe6>
		return r;
	close(newfdnum);
  80140b:	83 ec 0c             	sub    $0xc,%esp
  80140e:	56                   	push   %esi
  80140f:	e8 80 ff ff ff       	call   801394 <close>

	newfd = INDEX2FD(newfdnum);
  801414:	89 f3                	mov    %esi,%ebx
  801416:	c1 e3 0c             	shl    $0xc,%ebx
  801419:	81 eb 00 00 00 30    	sub    $0x30000000,%ebx
	ova = fd2data(oldfd);
  80141f:	83 c4 04             	add    $0x4,%esp
  801422:	ff 75 e4             	pushl  -0x1c(%ebp)
  801425:	e8 d5 fd ff ff       	call   8011ff <fd2data>
  80142a:	89 c7                	mov    %eax,%edi
	nva = fd2data(newfd);
  80142c:	89 1c 24             	mov    %ebx,(%esp)
  80142f:	e8 cb fd ff ff       	call   8011ff <fd2data>
  801434:	83 c4 10             	add    $0x10,%esp
  801437:	89 45 d4             	mov    %eax,-0x2c(%ebp)

	if ((uvpd[PDX(ova)] & PTE_P) && (uvpt[PGNUM(ova)] & PTE_P))
  80143a:	89 f8                	mov    %edi,%eax
  80143c:	c1 e8 16             	shr    $0x16,%eax
  80143f:	8b 04 85 00 d0 7b ef 	mov    -0x10843000(,%eax,4),%eax
  801446:	a8 01                	test   $0x1,%al
  801448:	74 37                	je     801481 <dup+0x9b>
  80144a:	89 f8                	mov    %edi,%eax
  80144c:	c1 e8 0c             	shr    $0xc,%eax
  80144f:	8b 14 85 00 00 40 ef 	mov    -0x10c00000(,%eax,4),%edx
  801456:	f6 c2 01             	test   $0x1,%dl
  801459:	74 26                	je     801481 <dup+0x9b>
		if ((r = sys_page_map(0, ova, 0, nva, uvpt[PGNUM(ova)] & PTE_SYSCALL)) < 0)
  80145b:	8b 04 85 00 00 40 ef 	mov    -0x10c00000(,%eax,4),%eax
  801462:	83 ec 0c             	sub    $0xc,%esp
  801465:	25 07 0e 00 00       	and    $0xe07,%eax
  80146a:	50                   	push   %eax
  80146b:	ff 75 d4             	pushl  -0x2c(%ebp)
  80146e:	6a 00                	push   $0x0
  801470:	57                   	push   %edi
  801471:	6a 00                	push   $0x0
  801473:	e8 28 fb ff ff       	call   800fa0 <sys_page_map>
  801478:	89 c7                	mov    %eax,%edi
  80147a:	83 c4 20             	add    $0x20,%esp
  80147d:	85 c0                	test   %eax,%eax
  80147f:	78 2e                	js     8014af <dup+0xc9>
			goto err;
	if ((r = sys_page_map(0, oldfd, 0, newfd, uvpt[PGNUM(oldfd)] & PTE_SYSCALL)) < 0)
  801481:	8b 55 e4             	mov    -0x1c(%ebp),%edx
  801484:	89 d0                	mov    %edx,%eax
  801486:	c1 e8 0c             	shr    $0xc,%eax
  801489:	8b 04 85 00 00 40 ef 	mov    -0x10c00000(,%eax,4),%eax
  801490:	83 ec 0c             	sub    $0xc,%esp
  801493:	25 07 0e 00 00       	and    $0xe07,%eax
  801498:	50                   	push   %eax
  801499:	53                   	push   %ebx
  80149a:	6a 00                	push   $0x0
  80149c:	52                   	push   %edx
  80149d:	6a 00                	push   $0x0
  80149f:	e8 fc fa ff ff       	call   800fa0 <sys_page_map>
  8014a4:	89 c7                	mov    %eax,%edi
  8014a6:	83 c4 20             	add    $0x20,%esp
		goto err;

	return newfdnum;
  8014a9:	89 f0                	mov    %esi,%eax
	nva = fd2data(newfd);

	if ((uvpd[PDX(ova)] & PTE_P) && (uvpt[PGNUM(ova)] & PTE_P))
		if ((r = sys_page_map(0, ova, 0, nva, uvpt[PGNUM(ova)] & PTE_SYSCALL)) < 0)
			goto err;
	if ((r = sys_page_map(0, oldfd, 0, newfd, uvpt[PGNUM(oldfd)] & PTE_SYSCALL)) < 0)
  8014ab:	85 ff                	test   %edi,%edi
  8014ad:	79 1d                	jns    8014cc <dup+0xe6>
		goto err;

	return newfdnum;

err:
	sys_page_unmap(0, newfd);
  8014af:	83 ec 08             	sub    $0x8,%esp
  8014b2:	53                   	push   %ebx
  8014b3:	6a 00                	push   $0x0
  8014b5:	e8 28 fb ff ff       	call   800fe2 <sys_page_unmap>
	sys_page_unmap(0, nva);
  8014ba:	83 c4 08             	add    $0x8,%esp
  8014bd:	ff 75 d4             	pushl  -0x2c(%ebp)
  8014c0:	6a 00                	push   $0x0
  8014c2:	e8 1b fb ff ff       	call   800fe2 <sys_page_unmap>
	return r;
  8014c7:	83 c4 10             	add    $0x10,%esp
  8014ca:	89 f8                	mov    %edi,%eax
}
  8014cc:	8d 65 f4             	lea    -0xc(%ebp),%esp
  8014cf:	5b                   	pop    %ebx
  8014d0:	5e                   	pop    %esi
  8014d1:	5f                   	pop    %edi
  8014d2:	5d                   	pop    %ebp
  8014d3:	c3                   	ret    

008014d4 <read>:

ssize_t
read(int fdnum, void *buf, size_t n)
{
  8014d4:	55                   	push   %ebp
  8014d5:	89 e5                	mov    %esp,%ebp
  8014d7:	53                   	push   %ebx
  8014d8:	83 ec 14             	sub    $0x14,%esp
  8014db:	8b 5d 08             	mov    0x8(%ebp),%ebx
	int r;
	struct Dev *dev;
	struct Fd *fd;

	if ((r = fd_lookup(fdnum, &fd)) < 0
  8014de:	8d 45 f0             	lea    -0x10(%ebp),%eax
  8014e1:	50                   	push   %eax
  8014e2:	53                   	push   %ebx
  8014e3:	e8 7d fd ff ff       	call   801265 <fd_lookup>
  8014e8:	83 c4 08             	add    $0x8,%esp
  8014eb:	89 c2                	mov    %eax,%edx
  8014ed:	85 c0                	test   %eax,%eax
  8014ef:	78 6d                	js     80155e <read+0x8a>
	    || (r = dev_lookup(fd->fd_dev_id, &dev)) < 0)
  8014f1:	83 ec 08             	sub    $0x8,%esp
  8014f4:	8d 45 f4             	lea    -0xc(%ebp),%eax
  8014f7:	50                   	push   %eax
  8014f8:	8b 45 f0             	mov    -0x10(%ebp),%eax
  8014fb:	ff 30                	pushl  (%eax)
  8014fd:	e8 b9 fd ff ff       	call   8012bb <dev_lookup>
  801502:	83 c4 10             	add    $0x10,%esp
  801505:	85 c0                	test   %eax,%eax
  801507:	78 4c                	js     801555 <read+0x81>
		return r;
	if ((fd->fd_omode & O_ACCMODE) == O_WRONLY) {
  801509:	8b 55 f0             	mov    -0x10(%ebp),%edx
  80150c:	8b 42 08             	mov    0x8(%edx),%eax
  80150f:	83 e0 03             	and    $0x3,%eax
  801512:	83 f8 01             	cmp    $0x1,%eax
  801515:	75 21                	jne    801538 <read+0x64>
		cprintf("[%08x] read %d -- bad mode\n", thisenv->env_id, fdnum);
  801517:	a1 18 40 80 00       	mov    0x804018,%eax
  80151c:	8b 40 48             	mov    0x48(%eax),%eax
  80151f:	83 ec 04             	sub    $0x4,%esp
  801522:	53                   	push   %ebx
  801523:	50                   	push   %eax
  801524:	68 8d 2c 80 00       	push   $0x802c8d
  801529:	e8 9f f0 ff ff       	call   8005cd <cprintf>
		return -E_INVAL;
  80152e:	83 c4 10             	add    $0x10,%esp
  801531:	ba fd ff ff ff       	mov    $0xfffffffd,%edx
  801536:	eb 26                	jmp    80155e <read+0x8a>
	}
	if (!dev->dev_read)
  801538:	8b 45 f4             	mov    -0xc(%ebp),%eax
  80153b:	8b 40 08             	mov    0x8(%eax),%eax
  80153e:	85 c0                	test   %eax,%eax
  801540:	74 17                	je     801559 <read+0x85>
		return -E_NOT_SUPP;
	return (*dev->dev_read)(fd, buf, n);
  801542:	83 ec 04             	sub    $0x4,%esp
  801545:	ff 75 10             	pushl  0x10(%ebp)
  801548:	ff 75 0c             	pushl  0xc(%ebp)
  80154b:	52                   	push   %edx
  80154c:	ff d0                	call   *%eax
  80154e:	89 c2                	mov    %eax,%edx
  801550:	83 c4 10             	add    $0x10,%esp
  801553:	eb 09                	jmp    80155e <read+0x8a>
	int r;
	struct Dev *dev;
	struct Fd *fd;

	if ((r = fd_lookup(fdnum, &fd)) < 0
	    || (r = dev_lookup(fd->fd_dev_id, &dev)) < 0)
  801555:	89 c2                	mov    %eax,%edx
  801557:	eb 05                	jmp    80155e <read+0x8a>
	if ((fd->fd_omode & O_ACCMODE) == O_WRONLY) {
		cprintf("[%08x] read %d -- bad mode\n", thisenv->env_id, fdnum);
		return -E_INVAL;
	}
	if (!dev->dev_read)
		return -E_NOT_SUPP;
  801559:	ba f1 ff ff ff       	mov    $0xfffffff1,%edx
	return (*dev->dev_read)(fd, buf, n);
}
  80155e:	89 d0                	mov    %edx,%eax
  801560:	8b 5d fc             	mov    -0x4(%ebp),%ebx
  801563:	c9                   	leave  
  801564:	c3                   	ret    

00801565 <readn>:

ssize_t
readn(int fdnum, void *buf, size_t n)
{
  801565:	55                   	push   %ebp
  801566:	89 e5                	mov    %esp,%ebp
  801568:	57                   	push   %edi
  801569:	56                   	push   %esi
  80156a:	53                   	push   %ebx
  80156b:	83 ec 0c             	sub    $0xc,%esp
  80156e:	8b 7d 08             	mov    0x8(%ebp),%edi
  801571:	8b 75 10             	mov    0x10(%ebp),%esi
	int m, tot;

	for (tot = 0; tot < n; tot += m) {
  801574:	bb 00 00 00 00       	mov    $0x0,%ebx
  801579:	eb 21                	jmp    80159c <readn+0x37>
		m = read(fdnum, (char*)buf + tot, n - tot);
  80157b:	83 ec 04             	sub    $0x4,%esp
  80157e:	89 f0                	mov    %esi,%eax
  801580:	29 d8                	sub    %ebx,%eax
  801582:	50                   	push   %eax
  801583:	89 d8                	mov    %ebx,%eax
  801585:	03 45 0c             	add    0xc(%ebp),%eax
  801588:	50                   	push   %eax
  801589:	57                   	push   %edi
  80158a:	e8 45 ff ff ff       	call   8014d4 <read>
		if (m < 0)
  80158f:	83 c4 10             	add    $0x10,%esp
  801592:	85 c0                	test   %eax,%eax
  801594:	78 0c                	js     8015a2 <readn+0x3d>
			return m;
		if (m == 0)
  801596:	85 c0                	test   %eax,%eax
  801598:	74 06                	je     8015a0 <readn+0x3b>
ssize_t
readn(int fdnum, void *buf, size_t n)
{
	int m, tot;

	for (tot = 0; tot < n; tot += m) {
  80159a:	01 c3                	add    %eax,%ebx
  80159c:	39 f3                	cmp    %esi,%ebx
  80159e:	72 db                	jb     80157b <readn+0x16>
		if (m < 0)
			return m;
		if (m == 0)
			break;
	}
	return tot;
  8015a0:	89 d8                	mov    %ebx,%eax
}
  8015a2:	8d 65 f4             	lea    -0xc(%ebp),%esp
  8015a5:	5b                   	pop    %ebx
  8015a6:	5e                   	pop    %esi
  8015a7:	5f                   	pop    %edi
  8015a8:	5d                   	pop    %ebp
  8015a9:	c3                   	ret    

008015aa <write>:

ssize_t
write(int fdnum, const void *buf, size_t n)
{
  8015aa:	55                   	push   %ebp
  8015ab:	89 e5                	mov    %esp,%ebp
  8015ad:	53                   	push   %ebx
  8015ae:	83 ec 14             	sub    $0x14,%esp
  8015b1:	8b 5d 08             	mov    0x8(%ebp),%ebx
	int r;
	struct Dev *dev;
	struct Fd *fd;

	if ((r = fd_lookup(fdnum, &fd)) < 0
  8015b4:	8d 45 f0             	lea    -0x10(%ebp),%eax
  8015b7:	50                   	push   %eax
  8015b8:	53                   	push   %ebx
  8015b9:	e8 a7 fc ff ff       	call   801265 <fd_lookup>
  8015be:	83 c4 08             	add    $0x8,%esp
  8015c1:	89 c2                	mov    %eax,%edx
  8015c3:	85 c0                	test   %eax,%eax
  8015c5:	78 68                	js     80162f <write+0x85>
	    || (r = dev_lookup(fd->fd_dev_id, &dev)) < 0)
  8015c7:	83 ec 08             	sub    $0x8,%esp
  8015ca:	8d 45 f4             	lea    -0xc(%ebp),%eax
  8015cd:	50                   	push   %eax
  8015ce:	8b 45 f0             	mov    -0x10(%ebp),%eax
  8015d1:	ff 30                	pushl  (%eax)
  8015d3:	e8 e3 fc ff ff       	call   8012bb <dev_lookup>
  8015d8:	83 c4 10             	add    $0x10,%esp
  8015db:	85 c0                	test   %eax,%eax
  8015dd:	78 47                	js     801626 <write+0x7c>
		return r;
	if ((fd->fd_omode & O_ACCMODE) == O_RDONLY) {
  8015df:	8b 45 f0             	mov    -0x10(%ebp),%eax
  8015e2:	f6 40 08 03          	testb  $0x3,0x8(%eax)
  8015e6:	75 21                	jne    801609 <write+0x5f>
		cprintf("[%08x] write %d -- bad mode\n", thisenv->env_id, fdnum);
  8015e8:	a1 18 40 80 00       	mov    0x804018,%eax
  8015ed:	8b 40 48             	mov    0x48(%eax),%eax
  8015f0:	83 ec 04             	sub    $0x4,%esp
  8015f3:	53                   	push   %ebx
  8015f4:	50                   	push   %eax
  8015f5:	68 a9 2c 80 00       	push   $0x802ca9
  8015fa:	e8 ce ef ff ff       	call   8005cd <cprintf>
		return -E_INVAL;
  8015ff:	83 c4 10             	add    $0x10,%esp
  801602:	ba fd ff ff ff       	mov    $0xfffffffd,%edx
  801607:	eb 26                	jmp    80162f <write+0x85>
	}
	if (debug)
		cprintf("write %d %p %d via dev %s\n",
			fdnum, buf, n, dev->dev_name);
	if (!dev->dev_write)
  801609:	8b 55 f4             	mov    -0xc(%ebp),%edx
  80160c:	8b 52 0c             	mov    0xc(%edx),%edx
  80160f:	85 d2                	test   %edx,%edx
  801611:	74 17                	je     80162a <write+0x80>
		return -E_NOT_SUPP;
	return (*dev->dev_write)(fd, buf, n);
  801613:	83 ec 04             	sub    $0x4,%esp
  801616:	ff 75 10             	pushl  0x10(%ebp)
  801619:	ff 75 0c             	pushl  0xc(%ebp)
  80161c:	50                   	push   %eax
  80161d:	ff d2                	call   *%edx
  80161f:	89 c2                	mov    %eax,%edx
  801621:	83 c4 10             	add    $0x10,%esp
  801624:	eb 09                	jmp    80162f <write+0x85>
	int r;
	struct Dev *dev;
	struct Fd *fd;

	if ((r = fd_lookup(fdnum, &fd)) < 0
	    || (r = dev_lookup(fd->fd_dev_id, &dev)) < 0)
  801626:	89 c2                	mov    %eax,%edx
  801628:	eb 05                	jmp    80162f <write+0x85>
	}
	if (debug)
		cprintf("write %d %p %d via dev %s\n",
			fdnum, buf, n, dev->dev_name);
	if (!dev->dev_write)
		return -E_NOT_SUPP;
  80162a:	ba f1 ff ff ff       	mov    $0xfffffff1,%edx
	return (*dev->dev_write)(fd, buf, n);
}
  80162f:	89 d0                	mov    %edx,%eax
  801631:	8b 5d fc             	mov    -0x4(%ebp),%ebx
  801634:	c9                   	leave  
  801635:	c3                   	ret    

00801636 <seek>:

int
seek(int fdnum, off_t offset)
{
  801636:	55                   	push   %ebp
  801637:	89 e5                	mov    %esp,%ebp
  801639:	83 ec 10             	sub    $0x10,%esp
	int r;
	struct Fd *fd;

	if ((r = fd_lookup(fdnum, &fd)) < 0)
  80163c:	8d 45 fc             	lea    -0x4(%ebp),%eax
  80163f:	50                   	push   %eax
  801640:	ff 75 08             	pushl  0x8(%ebp)
  801643:	e8 1d fc ff ff       	call   801265 <fd_lookup>
  801648:	83 c4 08             	add    $0x8,%esp
  80164b:	85 c0                	test   %eax,%eax
  80164d:	78 0e                	js     80165d <seek+0x27>
		return r;
	fd->fd_offset = offset;
  80164f:	8b 45 fc             	mov    -0x4(%ebp),%eax
  801652:	8b 55 0c             	mov    0xc(%ebp),%edx
  801655:	89 50 04             	mov    %edx,0x4(%eax)
	return 0;
  801658:	b8 00 00 00 00       	mov    $0x0,%eax
}
  80165d:	c9                   	leave  
  80165e:	c3                   	ret    

0080165f <ftruncate>:

int
ftruncate(int fdnum, off_t newsize)
{
  80165f:	55                   	push   %ebp
  801660:	89 e5                	mov    %esp,%ebp
  801662:	53                   	push   %ebx
  801663:	83 ec 14             	sub    $0x14,%esp
  801666:	8b 5d 08             	mov    0x8(%ebp),%ebx
	int r;
	struct Dev *dev;
	struct Fd *fd;
	if ((r = fd_lookup(fdnum, &fd)) < 0
  801669:	8d 45 f0             	lea    -0x10(%ebp),%eax
  80166c:	50                   	push   %eax
  80166d:	53                   	push   %ebx
  80166e:	e8 f2 fb ff ff       	call   801265 <fd_lookup>
  801673:	83 c4 08             	add    $0x8,%esp
  801676:	89 c2                	mov    %eax,%edx
  801678:	85 c0                	test   %eax,%eax
  80167a:	78 65                	js     8016e1 <ftruncate+0x82>
	    || (r = dev_lookup(fd->fd_dev_id, &dev)) < 0)
  80167c:	83 ec 08             	sub    $0x8,%esp
  80167f:	8d 45 f4             	lea    -0xc(%ebp),%eax
  801682:	50                   	push   %eax
  801683:	8b 45 f0             	mov    -0x10(%ebp),%eax
  801686:	ff 30                	pushl  (%eax)
  801688:	e8 2e fc ff ff       	call   8012bb <dev_lookup>
  80168d:	83 c4 10             	add    $0x10,%esp
  801690:	85 c0                	test   %eax,%eax
  801692:	78 44                	js     8016d8 <ftruncate+0x79>
		return r;
	if ((fd->fd_omode & O_ACCMODE) == O_RDONLY) {
  801694:	8b 45 f0             	mov    -0x10(%ebp),%eax
  801697:	f6 40 08 03          	testb  $0x3,0x8(%eax)
  80169b:	75 21                	jne    8016be <ftruncate+0x5f>
		cprintf("[%08x] ftruncate %d -- bad mode\n",
			thisenv->env_id, fdnum);
  80169d:	a1 18 40 80 00       	mov    0x804018,%eax
	struct Fd *fd;
	if ((r = fd_lookup(fdnum, &fd)) < 0
	    || (r = dev_lookup(fd->fd_dev_id, &dev)) < 0)
		return r;
	if ((fd->fd_omode & O_ACCMODE) == O_RDONLY) {
		cprintf("[%08x] ftruncate %d -- bad mode\n",
  8016a2:	8b 40 48             	mov    0x48(%eax),%eax
  8016a5:	83 ec 04             	sub    $0x4,%esp
  8016a8:	53                   	push   %ebx
  8016a9:	50                   	push   %eax
  8016aa:	68 6c 2c 80 00       	push   $0x802c6c
  8016af:	e8 19 ef ff ff       	call   8005cd <cprintf>
			thisenv->env_id, fdnum);
		return -E_INVAL;
  8016b4:	83 c4 10             	add    $0x10,%esp
  8016b7:	ba fd ff ff ff       	mov    $0xfffffffd,%edx
  8016bc:	eb 23                	jmp    8016e1 <ftruncate+0x82>
	}
	if (!dev->dev_trunc)
  8016be:	8b 55 f4             	mov    -0xc(%ebp),%edx
  8016c1:	8b 52 18             	mov    0x18(%edx),%edx
  8016c4:	85 d2                	test   %edx,%edx
  8016c6:	74 14                	je     8016dc <ftruncate+0x7d>
		return -E_NOT_SUPP;
	return (*dev->dev_trunc)(fd, newsize);
  8016c8:	83 ec 08             	sub    $0x8,%esp
  8016cb:	ff 75 0c             	pushl  0xc(%ebp)
  8016ce:	50                   	push   %eax
  8016cf:	ff d2                	call   *%edx
  8016d1:	89 c2                	mov    %eax,%edx
  8016d3:	83 c4 10             	add    $0x10,%esp
  8016d6:	eb 09                	jmp    8016e1 <ftruncate+0x82>
{
	int r;
	struct Dev *dev;
	struct Fd *fd;
	if ((r = fd_lookup(fdnum, &fd)) < 0
	    || (r = dev_lookup(fd->fd_dev_id, &dev)) < 0)
  8016d8:	89 c2                	mov    %eax,%edx
  8016da:	eb 05                	jmp    8016e1 <ftruncate+0x82>
		cprintf("[%08x] ftruncate %d -- bad mode\n",
			thisenv->env_id, fdnum);
		return -E_INVAL;
	}
	if (!dev->dev_trunc)
		return -E_NOT_SUPP;
  8016dc:	ba f1 ff ff ff       	mov    $0xfffffff1,%edx
	return (*dev->dev_trunc)(fd, newsize);
}
  8016e1:	89 d0                	mov    %edx,%eax
  8016e3:	8b 5d fc             	mov    -0x4(%ebp),%ebx
  8016e6:	c9                   	leave  
  8016e7:	c3                   	ret    

008016e8 <fstat>:

int
fstat(int fdnum, struct Stat *stat)
{
  8016e8:	55                   	push   %ebp
  8016e9:	89 e5                	mov    %esp,%ebp
  8016eb:	53                   	push   %ebx
  8016ec:	83 ec 14             	sub    $0x14,%esp
  8016ef:	8b 5d 0c             	mov    0xc(%ebp),%ebx
	int r;
	struct Dev *dev;
	struct Fd *fd;

	if ((r = fd_lookup(fdnum, &fd)) < 0
  8016f2:	8d 45 f0             	lea    -0x10(%ebp),%eax
  8016f5:	50                   	push   %eax
  8016f6:	ff 75 08             	pushl  0x8(%ebp)
  8016f9:	e8 67 fb ff ff       	call   801265 <fd_lookup>
  8016fe:	83 c4 08             	add    $0x8,%esp
  801701:	89 c2                	mov    %eax,%edx
  801703:	85 c0                	test   %eax,%eax
  801705:	78 58                	js     80175f <fstat+0x77>
	    || (r = dev_lookup(fd->fd_dev_id, &dev)) < 0)
  801707:	83 ec 08             	sub    $0x8,%esp
  80170a:	8d 45 f4             	lea    -0xc(%ebp),%eax
  80170d:	50                   	push   %eax
  80170e:	8b 45 f0             	mov    -0x10(%ebp),%eax
  801711:	ff 30                	pushl  (%eax)
  801713:	e8 a3 fb ff ff       	call   8012bb <dev_lookup>
  801718:	83 c4 10             	add    $0x10,%esp
  80171b:	85 c0                	test   %eax,%eax
  80171d:	78 37                	js     801756 <fstat+0x6e>
		return r;
	if (!dev->dev_stat)
  80171f:	8b 45 f4             	mov    -0xc(%ebp),%eax
  801722:	83 78 14 00          	cmpl   $0x0,0x14(%eax)
  801726:	74 32                	je     80175a <fstat+0x72>
		return -E_NOT_SUPP;
	stat->st_name[0] = 0;
  801728:	c6 03 00             	movb   $0x0,(%ebx)
	stat->st_size = 0;
  80172b:	c7 83 80 00 00 00 00 	movl   $0x0,0x80(%ebx)
  801732:	00 00 00 
	stat->st_isdir = 0;
  801735:	c7 83 84 00 00 00 00 	movl   $0x0,0x84(%ebx)
  80173c:	00 00 00 
	stat->st_dev = dev;
  80173f:	89 83 88 00 00 00    	mov    %eax,0x88(%ebx)
	return (*dev->dev_stat)(fd, stat);
  801745:	83 ec 08             	sub    $0x8,%esp
  801748:	53                   	push   %ebx
  801749:	ff 75 f0             	pushl  -0x10(%ebp)
  80174c:	ff 50 14             	call   *0x14(%eax)
  80174f:	89 c2                	mov    %eax,%edx
  801751:	83 c4 10             	add    $0x10,%esp
  801754:	eb 09                	jmp    80175f <fstat+0x77>
	int r;
	struct Dev *dev;
	struct Fd *fd;

	if ((r = fd_lookup(fdnum, &fd)) < 0
	    || (r = dev_lookup(fd->fd_dev_id, &dev)) < 0)
  801756:	89 c2                	mov    %eax,%edx
  801758:	eb 05                	jmp    80175f <fstat+0x77>
		return r;
	if (!dev->dev_stat)
		return -E_NOT_SUPP;
  80175a:	ba f1 ff ff ff       	mov    $0xfffffff1,%edx
	stat->st_name[0] = 0;
	stat->st_size = 0;
	stat->st_isdir = 0;
	stat->st_dev = dev;
	return (*dev->dev_stat)(fd, stat);
}
  80175f:	89 d0                	mov    %edx,%eax
  801761:	8b 5d fc             	mov    -0x4(%ebp),%ebx
  801764:	c9                   	leave  
  801765:	c3                   	ret    

00801766 <stat>:

int
stat(const char *path, struct Stat *stat)
{
  801766:	55                   	push   %ebp
  801767:	89 e5                	mov    %esp,%ebp
  801769:	56                   	push   %esi
  80176a:	53                   	push   %ebx
	int fd, r;

	if ((fd = open(path, O_RDONLY)) < 0)
  80176b:	83 ec 08             	sub    $0x8,%esp
  80176e:	6a 00                	push   $0x0
  801770:	ff 75 08             	pushl  0x8(%ebp)
  801773:	e8 09 02 00 00       	call   801981 <open>
  801778:	89 c3                	mov    %eax,%ebx
  80177a:	83 c4 10             	add    $0x10,%esp
  80177d:	85 db                	test   %ebx,%ebx
  80177f:	78 1b                	js     80179c <stat+0x36>
		return fd;
	r = fstat(fd, stat);
  801781:	83 ec 08             	sub    $0x8,%esp
  801784:	ff 75 0c             	pushl  0xc(%ebp)
  801787:	53                   	push   %ebx
  801788:	e8 5b ff ff ff       	call   8016e8 <fstat>
  80178d:	89 c6                	mov    %eax,%esi
	close(fd);
  80178f:	89 1c 24             	mov    %ebx,(%esp)
  801792:	e8 fd fb ff ff       	call   801394 <close>
	return r;
  801797:	83 c4 10             	add    $0x10,%esp
  80179a:	89 f0                	mov    %esi,%eax
}
  80179c:	8d 65 f8             	lea    -0x8(%ebp),%esp
  80179f:	5b                   	pop    %ebx
  8017a0:	5e                   	pop    %esi
  8017a1:	5d                   	pop    %ebp
  8017a2:	c3                   	ret    

008017a3 <fsipc>:
// type: request code, passed as the simple integer IPC value.
// dstva: virtual address at which to receive reply page, 0 if none.
// Returns result from the file server.
static int
fsipc(unsigned type, void *dstva)
{
  8017a3:	55                   	push   %ebp
  8017a4:	89 e5                	mov    %esp,%ebp
  8017a6:	56                   	push   %esi
  8017a7:	53                   	push   %ebx
  8017a8:	89 c6                	mov    %eax,%esi
  8017aa:	89 d3                	mov    %edx,%ebx
	static envid_t fsenv;
	if (fsenv == 0)
  8017ac:	83 3d 10 40 80 00 00 	cmpl   $0x0,0x804010
  8017b3:	75 12                	jne    8017c7 <fsipc+0x24>
		fsenv = ipc_find_env(ENV_TYPE_FS);
  8017b5:	83 ec 0c             	sub    $0xc,%esp
  8017b8:	6a 01                	push   $0x1
  8017ba:	e8 b6 0c 00 00       	call   802475 <ipc_find_env>
  8017bf:	a3 10 40 80 00       	mov    %eax,0x804010
  8017c4:	83 c4 10             	add    $0x10,%esp
	static_assert(sizeof(fsipcbuf) == PGSIZE);

	if (debug)
		cprintf("[%08x] fsipc %d %08x\n", thisenv->env_id, type, *(uint32_t *)&fsipcbuf);

	ipc_send(fsenv, type, &fsipcbuf, PTE_P | PTE_W | PTE_U);
  8017c7:	6a 07                	push   $0x7
  8017c9:	68 00 50 80 00       	push   $0x805000
  8017ce:	56                   	push   %esi
  8017cf:	ff 35 10 40 80 00    	pushl  0x804010
  8017d5:	e8 47 0c 00 00       	call   802421 <ipc_send>
	return ipc_recv(NULL, dstva, NULL);
  8017da:	83 c4 0c             	add    $0xc,%esp
  8017dd:	6a 00                	push   $0x0
  8017df:	53                   	push   %ebx
  8017e0:	6a 00                	push   $0x0
  8017e2:	e8 d1 0b 00 00       	call   8023b8 <ipc_recv>
}
  8017e7:	8d 65 f8             	lea    -0x8(%ebp),%esp
  8017ea:	5b                   	pop    %ebx
  8017eb:	5e                   	pop    %esi
  8017ec:	5d                   	pop    %ebp
  8017ed:	c3                   	ret    

008017ee <devfile_trunc>:
}

// Truncate or extend an open file to 'size' bytes
static int
devfile_trunc(struct Fd *fd, off_t newsize)
{
  8017ee:	55                   	push   %ebp
  8017ef:	89 e5                	mov    %esp,%ebp
  8017f1:	83 ec 08             	sub    $0x8,%esp
	fsipcbuf.set_size.req_fileid = fd->fd_file.id;
  8017f4:	8b 45 08             	mov    0x8(%ebp),%eax
  8017f7:	8b 40 0c             	mov    0xc(%eax),%eax
  8017fa:	a3 00 50 80 00       	mov    %eax,0x805000
	fsipcbuf.set_size.req_size = newsize;
  8017ff:	8b 45 0c             	mov    0xc(%ebp),%eax
  801802:	a3 04 50 80 00       	mov    %eax,0x805004
	return fsipc(FSREQ_SET_SIZE, NULL);
  801807:	ba 00 00 00 00       	mov    $0x0,%edx
  80180c:	b8 02 00 00 00       	mov    $0x2,%eax
  801811:	e8 8d ff ff ff       	call   8017a3 <fsipc>
}
  801816:	c9                   	leave  
  801817:	c3                   	ret    

00801818 <devfile_flush>:
// open, unmapping it is enough to free up server-side resources.
// Other than that, we just have to make sure our changes are flushed
// to disk.
static int
devfile_flush(struct Fd *fd)
{
  801818:	55                   	push   %ebp
  801819:	89 e5                	mov    %esp,%ebp
  80181b:	83 ec 08             	sub    $0x8,%esp
	fsipcbuf.flush.req_fileid = fd->fd_file.id;
  80181e:	8b 45 08             	mov    0x8(%ebp),%eax
  801821:	8b 40 0c             	mov    0xc(%eax),%eax
  801824:	a3 00 50 80 00       	mov    %eax,0x805000
	return fsipc(FSREQ_FLUSH, NULL);
  801829:	ba 00 00 00 00       	mov    $0x0,%edx
  80182e:	b8 06 00 00 00       	mov    $0x6,%eax
  801833:	e8 6b ff ff ff       	call   8017a3 <fsipc>
}
  801838:	c9                   	leave  
  801839:	c3                   	ret    

0080183a <devfile_stat>:
        return src_buf - buf;
}

static int
devfile_stat(struct Fd *fd, struct Stat *st)
{
  80183a:	55                   	push   %ebp
  80183b:	89 e5                	mov    %esp,%ebp
  80183d:	53                   	push   %ebx
  80183e:	83 ec 04             	sub    $0x4,%esp
  801841:	8b 5d 0c             	mov    0xc(%ebp),%ebx
	int r;

	fsipcbuf.stat.req_fileid = fd->fd_file.id;
  801844:	8b 45 08             	mov    0x8(%ebp),%eax
  801847:	8b 40 0c             	mov    0xc(%eax),%eax
  80184a:	a3 00 50 80 00       	mov    %eax,0x805000
	if ((r = fsipc(FSREQ_STAT, NULL)) < 0)
  80184f:	ba 00 00 00 00       	mov    $0x0,%edx
  801854:	b8 05 00 00 00       	mov    $0x5,%eax
  801859:	e8 45 ff ff ff       	call   8017a3 <fsipc>
  80185e:	89 c2                	mov    %eax,%edx
  801860:	85 d2                	test   %edx,%edx
  801862:	78 2c                	js     801890 <devfile_stat+0x56>
		return r;
	strcpy(st->st_name, fsipcbuf.statRet.ret_name);
  801864:	83 ec 08             	sub    $0x8,%esp
  801867:	68 00 50 80 00       	push   $0x805000
  80186c:	53                   	push   %ebx
  80186d:	e8 e2 f2 ff ff       	call   800b54 <strcpy>
	st->st_size = fsipcbuf.statRet.ret_size;
  801872:	a1 80 50 80 00       	mov    0x805080,%eax
  801877:	89 83 80 00 00 00    	mov    %eax,0x80(%ebx)
	st->st_isdir = fsipcbuf.statRet.ret_isdir;
  80187d:	a1 84 50 80 00       	mov    0x805084,%eax
  801882:	89 83 84 00 00 00    	mov    %eax,0x84(%ebx)
	return 0;
  801888:	83 c4 10             	add    $0x10,%esp
  80188b:	b8 00 00 00 00       	mov    $0x0,%eax
}
  801890:	8b 5d fc             	mov    -0x4(%ebp),%ebx
  801893:	c9                   	leave  
  801894:	c3                   	ret    

00801895 <devfile_write>:
// Returns:
//	 The number of bytes successfully written.
//	 < 0 on error.
static ssize_t
devfile_write(struct Fd *fd, const void *buf, size_t n)
{
  801895:	55                   	push   %ebp
  801896:	89 e5                	mov    %esp,%ebp
  801898:	57                   	push   %edi
  801899:	56                   	push   %esi
  80189a:	53                   	push   %ebx
  80189b:	83 ec 0c             	sub    $0xc,%esp
  80189e:	8b 75 10             	mov    0x10(%ebp),%esi
	//panic("devfile_write not implemented");
        int r;
        const void *src_buf = buf;
        size_t ipc_buf_size = PGSIZE - (sizeof(int) + sizeof(size_t));
 
        fsipcbuf.write.req_fileid = fd->fd_file.id;       
  8018a1:	8b 45 08             	mov    0x8(%ebp),%eax
  8018a4:	8b 40 0c             	mov    0xc(%eax),%eax
  8018a7:	a3 00 50 80 00       	mov    %eax,0x805000
	// remember that write is always allowed to write *fewer*
	// bytes than requested.
	// LAB 5: Your code here
	//panic("devfile_write not implemented");
        int r;
        const void *src_buf = buf;
  8018ac:	8b 5d 0c             	mov    0xc(%ebp),%ebx
        size_t ipc_buf_size = PGSIZE - (sizeof(int) + sizeof(size_t));
 
        fsipcbuf.write.req_fileid = fd->fd_file.id;       
        while( n > 0) {
  8018af:	eb 3d                	jmp    8018ee <devfile_write+0x59>
                size_t tmp = MIN(ipc_buf_size, n);
  8018b1:	81 fe f8 0f 00 00    	cmp    $0xff8,%esi
  8018b7:	bf f8 0f 00 00       	mov    $0xff8,%edi
  8018bc:	0f 46 fe             	cmovbe %esi,%edi
                memmove(fsipcbuf.write.req_buf, src_buf, tmp);
  8018bf:	83 ec 04             	sub    $0x4,%esp
  8018c2:	57                   	push   %edi
  8018c3:	53                   	push   %ebx
  8018c4:	68 08 50 80 00       	push   $0x805008
  8018c9:	e8 18 f4 ff ff       	call   800ce6 <memmove>
                fsipcbuf.write.req_n = tmp; 
  8018ce:	89 3d 04 50 80 00    	mov    %edi,0x805004
                if ((r = fsipc(FSREQ_WRITE, NULL)) < 0)
  8018d4:	ba 00 00 00 00       	mov    $0x0,%edx
  8018d9:	b8 04 00 00 00       	mov    $0x4,%eax
  8018de:	e8 c0 fe ff ff       	call   8017a3 <fsipc>
  8018e3:	83 c4 10             	add    $0x10,%esp
  8018e6:	85 c0                	test   %eax,%eax
  8018e8:	78 0d                	js     8018f7 <devfile_write+0x62>
		        return r;
                n -= tmp;
  8018ea:	29 fe                	sub    %edi,%esi
                src_buf += tmp;
  8018ec:	01 fb                	add    %edi,%ebx
        int r;
        const void *src_buf = buf;
        size_t ipc_buf_size = PGSIZE - (sizeof(int) + sizeof(size_t));
 
        fsipcbuf.write.req_fileid = fd->fd_file.id;       
        while( n > 0) {
  8018ee:	85 f6                	test   %esi,%esi
  8018f0:	75 bf                	jne    8018b1 <devfile_write+0x1c>
                if ((r = fsipc(FSREQ_WRITE, NULL)) < 0)
		        return r;
                n -= tmp;
                src_buf += tmp;
        } 
        return src_buf - buf;
  8018f2:	89 d8                	mov    %ebx,%eax
  8018f4:	2b 45 0c             	sub    0xc(%ebp),%eax
}
  8018f7:	8d 65 f4             	lea    -0xc(%ebp),%esp
  8018fa:	5b                   	pop    %ebx
  8018fb:	5e                   	pop    %esi
  8018fc:	5f                   	pop    %edi
  8018fd:	5d                   	pop    %ebp
  8018fe:	c3                   	ret    

008018ff <devfile_read>:
// Returns:
// 	The number of bytes successfully read.
// 	< 0 on error.
static ssize_t
devfile_read(struct Fd *fd, void *buf, size_t n)
{
  8018ff:	55                   	push   %ebp
  801900:	89 e5                	mov    %esp,%ebp
  801902:	56                   	push   %esi
  801903:	53                   	push   %ebx
  801904:	8b 75 10             	mov    0x10(%ebp),%esi
	// filling fsipcbuf.read with the request arguments.  The
	// bytes read will be written back to fsipcbuf by the file
	// system server.
	int r;

	fsipcbuf.read.req_fileid = fd->fd_file.id;
  801907:	8b 45 08             	mov    0x8(%ebp),%eax
  80190a:	8b 40 0c             	mov    0xc(%eax),%eax
  80190d:	a3 00 50 80 00       	mov    %eax,0x805000
	fsipcbuf.read.req_n = n;
  801912:	89 35 04 50 80 00    	mov    %esi,0x805004
	if ((r = fsipc(FSREQ_READ, NULL)) < 0)
  801918:	ba 00 00 00 00       	mov    $0x0,%edx
  80191d:	b8 03 00 00 00       	mov    $0x3,%eax
  801922:	e8 7c fe ff ff       	call   8017a3 <fsipc>
  801927:	89 c3                	mov    %eax,%ebx
  801929:	85 c0                	test   %eax,%eax
  80192b:	78 4b                	js     801978 <devfile_read+0x79>
		return r;
	assert(r <= n);
  80192d:	39 c6                	cmp    %eax,%esi
  80192f:	73 16                	jae    801947 <devfile_read+0x48>
  801931:	68 dc 2c 80 00       	push   $0x802cdc
  801936:	68 e3 2c 80 00       	push   $0x802ce3
  80193b:	6a 7c                	push   $0x7c
  80193d:	68 f8 2c 80 00       	push   $0x802cf8
  801942:	e8 2b 0a 00 00       	call   802372 <_panic>
	assert(r <= PGSIZE);
  801947:	3d 00 10 00 00       	cmp    $0x1000,%eax
  80194c:	7e 16                	jle    801964 <devfile_read+0x65>
  80194e:	68 03 2d 80 00       	push   $0x802d03
  801953:	68 e3 2c 80 00       	push   $0x802ce3
  801958:	6a 7d                	push   $0x7d
  80195a:	68 f8 2c 80 00       	push   $0x802cf8
  80195f:	e8 0e 0a 00 00       	call   802372 <_panic>
	memmove(buf, fsipcbuf.readRet.ret_buf, r);
  801964:	83 ec 04             	sub    $0x4,%esp
  801967:	50                   	push   %eax
  801968:	68 00 50 80 00       	push   $0x805000
  80196d:	ff 75 0c             	pushl  0xc(%ebp)
  801970:	e8 71 f3 ff ff       	call   800ce6 <memmove>
	return r;
  801975:	83 c4 10             	add    $0x10,%esp
}
  801978:	89 d8                	mov    %ebx,%eax
  80197a:	8d 65 f8             	lea    -0x8(%ebp),%esp
  80197d:	5b                   	pop    %ebx
  80197e:	5e                   	pop    %esi
  80197f:	5d                   	pop    %ebp
  801980:	c3                   	ret    

00801981 <open>:
// 	The file descriptor index on success
// 	-E_BAD_PATH if the path is too long (>= MAXPATHLEN)
// 	< 0 for other errors.
int
open(const char *path, int mode)
{
  801981:	55                   	push   %ebp
  801982:	89 e5                	mov    %esp,%ebp
  801984:	53                   	push   %ebx
  801985:	83 ec 20             	sub    $0x20,%esp
  801988:	8b 5d 08             	mov    0x8(%ebp),%ebx
	// file descriptor.

	int r;
	struct Fd *fd;

	if (strlen(path) >= MAXPATHLEN)
  80198b:	53                   	push   %ebx
  80198c:	e8 8a f1 ff ff       	call   800b1b <strlen>
  801991:	83 c4 10             	add    $0x10,%esp
  801994:	3d ff 03 00 00       	cmp    $0x3ff,%eax
  801999:	7f 67                	jg     801a02 <open+0x81>
		return -E_BAD_PATH;

	if ((r = fd_alloc(&fd)) < 0)
  80199b:	83 ec 0c             	sub    $0xc,%esp
  80199e:	8d 45 f4             	lea    -0xc(%ebp),%eax
  8019a1:	50                   	push   %eax
  8019a2:	e8 6f f8 ff ff       	call   801216 <fd_alloc>
  8019a7:	83 c4 10             	add    $0x10,%esp
		return r;
  8019aa:	89 c2                	mov    %eax,%edx
	struct Fd *fd;

	if (strlen(path) >= MAXPATHLEN)
		return -E_BAD_PATH;

	if ((r = fd_alloc(&fd)) < 0)
  8019ac:	85 c0                	test   %eax,%eax
  8019ae:	78 57                	js     801a07 <open+0x86>
		return r;

	strcpy(fsipcbuf.open.req_path, path);
  8019b0:	83 ec 08             	sub    $0x8,%esp
  8019b3:	53                   	push   %ebx
  8019b4:	68 00 50 80 00       	push   $0x805000
  8019b9:	e8 96 f1 ff ff       	call   800b54 <strcpy>
	fsipcbuf.open.req_omode = mode;
  8019be:	8b 45 0c             	mov    0xc(%ebp),%eax
  8019c1:	a3 00 54 80 00       	mov    %eax,0x805400

	if ((r = fsipc(FSREQ_OPEN, fd)) < 0) {
  8019c6:	8b 55 f4             	mov    -0xc(%ebp),%edx
  8019c9:	b8 01 00 00 00       	mov    $0x1,%eax
  8019ce:	e8 d0 fd ff ff       	call   8017a3 <fsipc>
  8019d3:	89 c3                	mov    %eax,%ebx
  8019d5:	83 c4 10             	add    $0x10,%esp
  8019d8:	85 c0                	test   %eax,%eax
  8019da:	79 14                	jns    8019f0 <open+0x6f>
		fd_close(fd, 0);
  8019dc:	83 ec 08             	sub    $0x8,%esp
  8019df:	6a 00                	push   $0x0
  8019e1:	ff 75 f4             	pushl  -0xc(%ebp)
  8019e4:	e8 2a f9 ff ff       	call   801313 <fd_close>
		return r;
  8019e9:	83 c4 10             	add    $0x10,%esp
  8019ec:	89 da                	mov    %ebx,%edx
  8019ee:	eb 17                	jmp    801a07 <open+0x86>
	}

	return fd2num(fd);
  8019f0:	83 ec 0c             	sub    $0xc,%esp
  8019f3:	ff 75 f4             	pushl  -0xc(%ebp)
  8019f6:	e8 f4 f7 ff ff       	call   8011ef <fd2num>
  8019fb:	89 c2                	mov    %eax,%edx
  8019fd:	83 c4 10             	add    $0x10,%esp
  801a00:	eb 05                	jmp    801a07 <open+0x86>

	int r;
	struct Fd *fd;

	if (strlen(path) >= MAXPATHLEN)
		return -E_BAD_PATH;
  801a02:	ba f4 ff ff ff       	mov    $0xfffffff4,%edx
		fd_close(fd, 0);
		return r;
	}

	return fd2num(fd);
}
  801a07:	89 d0                	mov    %edx,%eax
  801a09:	8b 5d fc             	mov    -0x4(%ebp),%ebx
  801a0c:	c9                   	leave  
  801a0d:	c3                   	ret    

00801a0e <sync>:


// Synchronize disk with buffer cache
int
sync(void)
{
  801a0e:	55                   	push   %ebp
  801a0f:	89 e5                	mov    %esp,%ebp
  801a11:	83 ec 08             	sub    $0x8,%esp
	// Ask the file server to update the disk
	// by writing any dirty blocks in the buffer cache.

	return fsipc(FSREQ_SYNC, NULL);
  801a14:	ba 00 00 00 00       	mov    $0x0,%edx
  801a19:	b8 08 00 00 00       	mov    $0x8,%eax
  801a1e:	e8 80 fd ff ff       	call   8017a3 <fsipc>
}
  801a23:	c9                   	leave  
  801a24:	c3                   	ret    

00801a25 <devsock_stat>:
	return nsipc_send(fd->fd_sock.sockid, buf, n, 0);
}

static int
devsock_stat(struct Fd *fd, struct Stat *stat)
{
  801a25:	55                   	push   %ebp
  801a26:	89 e5                	mov    %esp,%ebp
  801a28:	83 ec 10             	sub    $0x10,%esp
	strcpy(stat->st_name, "<sock>");
  801a2b:	68 0f 2d 80 00       	push   $0x802d0f
  801a30:	ff 75 0c             	pushl  0xc(%ebp)
  801a33:	e8 1c f1 ff ff       	call   800b54 <strcpy>
	return 0;
}
  801a38:	b8 00 00 00 00       	mov    $0x0,%eax
  801a3d:	c9                   	leave  
  801a3e:	c3                   	ret    

00801a3f <devsock_close>:
	return nsipc_shutdown(r, how);
}

static int
devsock_close(struct Fd *fd)
{
  801a3f:	55                   	push   %ebp
  801a40:	89 e5                	mov    %esp,%ebp
  801a42:	53                   	push   %ebx
  801a43:	83 ec 10             	sub    $0x10,%esp
  801a46:	8b 5d 08             	mov    0x8(%ebp),%ebx
	if (pageref(fd) == 1)
  801a49:	53                   	push   %ebx
  801a4a:	e8 5e 0a 00 00       	call   8024ad <pageref>
  801a4f:	83 c4 10             	add    $0x10,%esp
		return nsipc_close(fd->fd_sock.sockid);
	else
		return 0;
  801a52:	ba 00 00 00 00       	mov    $0x0,%edx
}

static int
devsock_close(struct Fd *fd)
{
	if (pageref(fd) == 1)
  801a57:	83 f8 01             	cmp    $0x1,%eax
  801a5a:	75 10                	jne    801a6c <devsock_close+0x2d>
		return nsipc_close(fd->fd_sock.sockid);
  801a5c:	83 ec 0c             	sub    $0xc,%esp
  801a5f:	ff 73 0c             	pushl  0xc(%ebx)
  801a62:	e8 ca 02 00 00       	call   801d31 <nsipc_close>
  801a67:	89 c2                	mov    %eax,%edx
  801a69:	83 c4 10             	add    $0x10,%esp
	else
		return 0;
}
  801a6c:	89 d0                	mov    %edx,%eax
  801a6e:	8b 5d fc             	mov    -0x4(%ebp),%ebx
  801a71:	c9                   	leave  
  801a72:	c3                   	ret    

00801a73 <devsock_write>:
	return nsipc_recv(fd->fd_sock.sockid, buf, n, 0);
}

static ssize_t
devsock_write(struct Fd *fd, const void *buf, size_t n)
{
  801a73:	55                   	push   %ebp
  801a74:	89 e5                	mov    %esp,%ebp
  801a76:	83 ec 08             	sub    $0x8,%esp
	return nsipc_send(fd->fd_sock.sockid, buf, n, 0);
  801a79:	6a 00                	push   $0x0
  801a7b:	ff 75 10             	pushl  0x10(%ebp)
  801a7e:	ff 75 0c             	pushl  0xc(%ebp)
  801a81:	8b 45 08             	mov    0x8(%ebp),%eax
  801a84:	ff 70 0c             	pushl  0xc(%eax)
  801a87:	e8 82 03 00 00       	call   801e0e <nsipc_send>
}
  801a8c:	c9                   	leave  
  801a8d:	c3                   	ret    

00801a8e <devsock_read>:
	return nsipc_listen(r, backlog);
}

static ssize_t
devsock_read(struct Fd *fd, void *buf, size_t n)
{
  801a8e:	55                   	push   %ebp
  801a8f:	89 e5                	mov    %esp,%ebp
  801a91:	83 ec 08             	sub    $0x8,%esp
	return nsipc_recv(fd->fd_sock.sockid, buf, n, 0);
  801a94:	6a 00                	push   $0x0
  801a96:	ff 75 10             	pushl  0x10(%ebp)
  801a99:	ff 75 0c             	pushl  0xc(%ebp)
  801a9c:	8b 45 08             	mov    0x8(%ebp),%eax
  801a9f:	ff 70 0c             	pushl  0xc(%eax)
  801aa2:	e8 fb 02 00 00       	call   801da2 <nsipc_recv>
}
  801aa7:	c9                   	leave  
  801aa8:	c3                   	ret    

00801aa9 <fd2sockid>:
	.dev_stat =	devsock_stat,
};

static int
fd2sockid(int fd)
{
  801aa9:	55                   	push   %ebp
  801aaa:	89 e5                	mov    %esp,%ebp
  801aac:	83 ec 20             	sub    $0x20,%esp
	struct Fd *sfd;
	int r;

	if ((r = fd_lookup(fd, &sfd)) < 0)
  801aaf:	8d 55 f4             	lea    -0xc(%ebp),%edx
  801ab2:	52                   	push   %edx
  801ab3:	50                   	push   %eax
  801ab4:	e8 ac f7 ff ff       	call   801265 <fd_lookup>
  801ab9:	83 c4 10             	add    $0x10,%esp
  801abc:	85 c0                	test   %eax,%eax
  801abe:	78 17                	js     801ad7 <fd2sockid+0x2e>
		return r;
	if (sfd->fd_dev_id != devsock.dev_id)
  801ac0:	8b 45 f4             	mov    -0xc(%ebp),%eax
  801ac3:	8b 0d 20 30 80 00    	mov    0x803020,%ecx
  801ac9:	39 08                	cmp    %ecx,(%eax)
  801acb:	75 05                	jne    801ad2 <fd2sockid+0x29>
		return -E_NOT_SUPP;
	return sfd->fd_sock.sockid;
  801acd:	8b 40 0c             	mov    0xc(%eax),%eax
  801ad0:	eb 05                	jmp    801ad7 <fd2sockid+0x2e>
	int r;

	if ((r = fd_lookup(fd, &sfd)) < 0)
		return r;
	if (sfd->fd_dev_id != devsock.dev_id)
		return -E_NOT_SUPP;
  801ad2:	b8 f1 ff ff ff       	mov    $0xfffffff1,%eax
	return sfd->fd_sock.sockid;
}
  801ad7:	c9                   	leave  
  801ad8:	c3                   	ret    

00801ad9 <alloc_sockfd>:

static int
alloc_sockfd(int sockid)
{
  801ad9:	55                   	push   %ebp
  801ada:	89 e5                	mov    %esp,%ebp
  801adc:	56                   	push   %esi
  801add:	53                   	push   %ebx
  801ade:	83 ec 1c             	sub    $0x1c,%esp
  801ae1:	89 c6                	mov    %eax,%esi
	struct Fd *sfd;
	int r;

	if ((r = fd_alloc(&sfd)) < 0
  801ae3:	8d 45 f4             	lea    -0xc(%ebp),%eax
  801ae6:	50                   	push   %eax
  801ae7:	e8 2a f7 ff ff       	call   801216 <fd_alloc>
  801aec:	89 c3                	mov    %eax,%ebx
  801aee:	83 c4 10             	add    $0x10,%esp
  801af1:	85 c0                	test   %eax,%eax
  801af3:	78 1b                	js     801b10 <alloc_sockfd+0x37>
	    || (r = sys_page_alloc(0, sfd, PTE_P|PTE_W|PTE_U|PTE_SHARE)) < 0) {
  801af5:	83 ec 04             	sub    $0x4,%esp
  801af8:	68 07 04 00 00       	push   $0x407
  801afd:	ff 75 f4             	pushl  -0xc(%ebp)
  801b00:	6a 00                	push   $0x0
  801b02:	e8 56 f4 ff ff       	call   800f5d <sys_page_alloc>
  801b07:	89 c3                	mov    %eax,%ebx
  801b09:	83 c4 10             	add    $0x10,%esp
  801b0c:	85 c0                	test   %eax,%eax
  801b0e:	79 10                	jns    801b20 <alloc_sockfd+0x47>
		nsipc_close(sockid);
  801b10:	83 ec 0c             	sub    $0xc,%esp
  801b13:	56                   	push   %esi
  801b14:	e8 18 02 00 00       	call   801d31 <nsipc_close>
		return r;
  801b19:	83 c4 10             	add    $0x10,%esp
  801b1c:	89 d8                	mov    %ebx,%eax
  801b1e:	eb 24                	jmp    801b44 <alloc_sockfd+0x6b>
	}

	sfd->fd_dev_id = devsock.dev_id;
  801b20:	8b 15 20 30 80 00    	mov    0x803020,%edx
  801b26:	8b 45 f4             	mov    -0xc(%ebp),%eax
  801b29:	89 10                	mov    %edx,(%eax)
	sfd->fd_omode = O_RDWR;
  801b2b:	8b 55 f4             	mov    -0xc(%ebp),%edx
  801b2e:	c7 42 08 02 00 00 00 	movl   $0x2,0x8(%edx)
	sfd->fd_sock.sockid = sockid;
  801b35:	89 72 0c             	mov    %esi,0xc(%edx)
	return fd2num(sfd);
  801b38:	83 ec 0c             	sub    $0xc,%esp
  801b3b:	52                   	push   %edx
  801b3c:	e8 ae f6 ff ff       	call   8011ef <fd2num>
  801b41:	83 c4 10             	add    $0x10,%esp
}
  801b44:	8d 65 f8             	lea    -0x8(%ebp),%esp
  801b47:	5b                   	pop    %ebx
  801b48:	5e                   	pop    %esi
  801b49:	5d                   	pop    %ebp
  801b4a:	c3                   	ret    

00801b4b <accept>:

int
accept(int s, struct sockaddr *addr, socklen_t *addrlen)
{
  801b4b:	55                   	push   %ebp
  801b4c:	89 e5                	mov    %esp,%ebp
  801b4e:	83 ec 08             	sub    $0x8,%esp
	int r;
	if ((r = fd2sockid(s)) < 0)
  801b51:	8b 45 08             	mov    0x8(%ebp),%eax
  801b54:	e8 50 ff ff ff       	call   801aa9 <fd2sockid>
		return r;
  801b59:	89 c1                	mov    %eax,%ecx

int
accept(int s, struct sockaddr *addr, socklen_t *addrlen)
{
	int r;
	if ((r = fd2sockid(s)) < 0)
  801b5b:	85 c0                	test   %eax,%eax
  801b5d:	78 1f                	js     801b7e <accept+0x33>
		return r;
	if ((r = nsipc_accept(r, addr, addrlen)) < 0)
  801b5f:	83 ec 04             	sub    $0x4,%esp
  801b62:	ff 75 10             	pushl  0x10(%ebp)
  801b65:	ff 75 0c             	pushl  0xc(%ebp)
  801b68:	50                   	push   %eax
  801b69:	e8 1c 01 00 00       	call   801c8a <nsipc_accept>
  801b6e:	83 c4 10             	add    $0x10,%esp
		return r;
  801b71:	89 c1                	mov    %eax,%ecx
accept(int s, struct sockaddr *addr, socklen_t *addrlen)
{
	int r;
	if ((r = fd2sockid(s)) < 0)
		return r;
	if ((r = nsipc_accept(r, addr, addrlen)) < 0)
  801b73:	85 c0                	test   %eax,%eax
  801b75:	78 07                	js     801b7e <accept+0x33>
		return r;
	return alloc_sockfd(r);
  801b77:	e8 5d ff ff ff       	call   801ad9 <alloc_sockfd>
  801b7c:	89 c1                	mov    %eax,%ecx
}
  801b7e:	89 c8                	mov    %ecx,%eax
  801b80:	c9                   	leave  
  801b81:	c3                   	ret    

00801b82 <bind>:

int
bind(int s, struct sockaddr *name, socklen_t namelen)
{
  801b82:	55                   	push   %ebp
  801b83:	89 e5                	mov    %esp,%ebp
  801b85:	83 ec 08             	sub    $0x8,%esp
	int r;
	if ((r = fd2sockid(s)) < 0)
  801b88:	8b 45 08             	mov    0x8(%ebp),%eax
  801b8b:	e8 19 ff ff ff       	call   801aa9 <fd2sockid>
  801b90:	89 c2                	mov    %eax,%edx
  801b92:	85 d2                	test   %edx,%edx
  801b94:	78 12                	js     801ba8 <bind+0x26>
		return r;
	return nsipc_bind(r, name, namelen);
  801b96:	83 ec 04             	sub    $0x4,%esp
  801b99:	ff 75 10             	pushl  0x10(%ebp)
  801b9c:	ff 75 0c             	pushl  0xc(%ebp)
  801b9f:	52                   	push   %edx
  801ba0:	e8 35 01 00 00       	call   801cda <nsipc_bind>
  801ba5:	83 c4 10             	add    $0x10,%esp
}
  801ba8:	c9                   	leave  
  801ba9:	c3                   	ret    

00801baa <shutdown>:

int
shutdown(int s, int how)
{
  801baa:	55                   	push   %ebp
  801bab:	89 e5                	mov    %esp,%ebp
  801bad:	83 ec 08             	sub    $0x8,%esp
	int r;
	if ((r = fd2sockid(s)) < 0)
  801bb0:	8b 45 08             	mov    0x8(%ebp),%eax
  801bb3:	e8 f1 fe ff ff       	call   801aa9 <fd2sockid>
  801bb8:	89 c2                	mov    %eax,%edx
  801bba:	85 d2                	test   %edx,%edx
  801bbc:	78 0f                	js     801bcd <shutdown+0x23>
		return r;
	return nsipc_shutdown(r, how);
  801bbe:	83 ec 08             	sub    $0x8,%esp
  801bc1:	ff 75 0c             	pushl  0xc(%ebp)
  801bc4:	52                   	push   %edx
  801bc5:	e8 45 01 00 00       	call   801d0f <nsipc_shutdown>
  801bca:	83 c4 10             	add    $0x10,%esp
}
  801bcd:	c9                   	leave  
  801bce:	c3                   	ret    

00801bcf <connect>:
		return 0;
}

int
connect(int s, const struct sockaddr *name, socklen_t namelen)
{
  801bcf:	55                   	push   %ebp
  801bd0:	89 e5                	mov    %esp,%ebp
  801bd2:	83 ec 08             	sub    $0x8,%esp
	int r;
	if ((r = fd2sockid(s)) < 0)
  801bd5:	8b 45 08             	mov    0x8(%ebp),%eax
  801bd8:	e8 cc fe ff ff       	call   801aa9 <fd2sockid>
  801bdd:	89 c2                	mov    %eax,%edx
  801bdf:	85 d2                	test   %edx,%edx
  801be1:	78 12                	js     801bf5 <connect+0x26>
		return r;
	return nsipc_connect(r, name, namelen);
  801be3:	83 ec 04             	sub    $0x4,%esp
  801be6:	ff 75 10             	pushl  0x10(%ebp)
  801be9:	ff 75 0c             	pushl  0xc(%ebp)
  801bec:	52                   	push   %edx
  801bed:	e8 59 01 00 00       	call   801d4b <nsipc_connect>
  801bf2:	83 c4 10             	add    $0x10,%esp
}
  801bf5:	c9                   	leave  
  801bf6:	c3                   	ret    

00801bf7 <listen>:

int
listen(int s, int backlog)
{
  801bf7:	55                   	push   %ebp
  801bf8:	89 e5                	mov    %esp,%ebp
  801bfa:	83 ec 08             	sub    $0x8,%esp
	int r;
	if ((r = fd2sockid(s)) < 0)
  801bfd:	8b 45 08             	mov    0x8(%ebp),%eax
  801c00:	e8 a4 fe ff ff       	call   801aa9 <fd2sockid>
  801c05:	89 c2                	mov    %eax,%edx
  801c07:	85 d2                	test   %edx,%edx
  801c09:	78 0f                	js     801c1a <listen+0x23>
		return r;
	return nsipc_listen(r, backlog);
  801c0b:	83 ec 08             	sub    $0x8,%esp
  801c0e:	ff 75 0c             	pushl  0xc(%ebp)
  801c11:	52                   	push   %edx
  801c12:	e8 69 01 00 00       	call   801d80 <nsipc_listen>
  801c17:	83 c4 10             	add    $0x10,%esp
}
  801c1a:	c9                   	leave  
  801c1b:	c3                   	ret    

00801c1c <socket>:
	return 0;
}

int
socket(int domain, int type, int protocol)
{
  801c1c:	55                   	push   %ebp
  801c1d:	89 e5                	mov    %esp,%ebp
  801c1f:	83 ec 0c             	sub    $0xc,%esp
	int r;
	if ((r = nsipc_socket(domain, type, protocol)) < 0)
  801c22:	ff 75 10             	pushl  0x10(%ebp)
  801c25:	ff 75 0c             	pushl  0xc(%ebp)
  801c28:	ff 75 08             	pushl  0x8(%ebp)
  801c2b:	e8 3c 02 00 00       	call   801e6c <nsipc_socket>
  801c30:	89 c2                	mov    %eax,%edx
  801c32:	83 c4 10             	add    $0x10,%esp
  801c35:	85 d2                	test   %edx,%edx
  801c37:	78 05                	js     801c3e <socket+0x22>
		return r;
	return alloc_sockfd(r);
  801c39:	e8 9b fe ff ff       	call   801ad9 <alloc_sockfd>
}
  801c3e:	c9                   	leave  
  801c3f:	c3                   	ret    

00801c40 <nsipc>:
// may be written back to nsipcbuf.
// type: request code, passed as the simple integer IPC value.
// Returns 0 if successful, < 0 on failure.
static int
nsipc(unsigned type)
{
  801c40:	55                   	push   %ebp
  801c41:	89 e5                	mov    %esp,%ebp
  801c43:	53                   	push   %ebx
  801c44:	83 ec 04             	sub    $0x4,%esp
  801c47:	89 c3                	mov    %eax,%ebx
	static envid_t nsenv;
	if (nsenv == 0)
  801c49:	83 3d 14 40 80 00 00 	cmpl   $0x0,0x804014
  801c50:	75 12                	jne    801c64 <nsipc+0x24>
		nsenv = ipc_find_env(ENV_TYPE_NS);
  801c52:	83 ec 0c             	sub    $0xc,%esp
  801c55:	6a 02                	push   $0x2
  801c57:	e8 19 08 00 00       	call   802475 <ipc_find_env>
  801c5c:	a3 14 40 80 00       	mov    %eax,0x804014
  801c61:	83 c4 10             	add    $0x10,%esp
	static_assert(sizeof(nsipcbuf) == PGSIZE);

	if (debug)
		cprintf("[%08x] nsipc %d\n", thisenv->env_id, type);

	ipc_send(nsenv, type, &nsipcbuf, PTE_P|PTE_W|PTE_U);
  801c64:	6a 07                	push   $0x7
  801c66:	68 00 60 80 00       	push   $0x806000
  801c6b:	53                   	push   %ebx
  801c6c:	ff 35 14 40 80 00    	pushl  0x804014
  801c72:	e8 aa 07 00 00       	call   802421 <ipc_send>
	return ipc_recv(NULL, NULL, NULL);
  801c77:	83 c4 0c             	add    $0xc,%esp
  801c7a:	6a 00                	push   $0x0
  801c7c:	6a 00                	push   $0x0
  801c7e:	6a 00                	push   $0x0
  801c80:	e8 33 07 00 00       	call   8023b8 <ipc_recv>
}
  801c85:	8b 5d fc             	mov    -0x4(%ebp),%ebx
  801c88:	c9                   	leave  
  801c89:	c3                   	ret    

00801c8a <nsipc_accept>:

int
nsipc_accept(int s, struct sockaddr *addr, socklen_t *addrlen)
{
  801c8a:	55                   	push   %ebp
  801c8b:	89 e5                	mov    %esp,%ebp
  801c8d:	56                   	push   %esi
  801c8e:	53                   	push   %ebx
  801c8f:	8b 75 10             	mov    0x10(%ebp),%esi
	int r;

	nsipcbuf.accept.req_s = s;
  801c92:	8b 45 08             	mov    0x8(%ebp),%eax
  801c95:	a3 00 60 80 00       	mov    %eax,0x806000
	nsipcbuf.accept.req_addrlen = *addrlen;
  801c9a:	8b 06                	mov    (%esi),%eax
  801c9c:	a3 04 60 80 00       	mov    %eax,0x806004
	if ((r = nsipc(NSREQ_ACCEPT)) >= 0) {
  801ca1:	b8 01 00 00 00       	mov    $0x1,%eax
  801ca6:	e8 95 ff ff ff       	call   801c40 <nsipc>
  801cab:	89 c3                	mov    %eax,%ebx
  801cad:	85 c0                	test   %eax,%eax
  801caf:	78 20                	js     801cd1 <nsipc_accept+0x47>
		struct Nsret_accept *ret = &nsipcbuf.acceptRet;
		memmove(addr, &ret->ret_addr, ret->ret_addrlen);
  801cb1:	83 ec 04             	sub    $0x4,%esp
  801cb4:	ff 35 10 60 80 00    	pushl  0x806010
  801cba:	68 00 60 80 00       	push   $0x806000
  801cbf:	ff 75 0c             	pushl  0xc(%ebp)
  801cc2:	e8 1f f0 ff ff       	call   800ce6 <memmove>
		*addrlen = ret->ret_addrlen;
  801cc7:	a1 10 60 80 00       	mov    0x806010,%eax
  801ccc:	89 06                	mov    %eax,(%esi)
  801cce:	83 c4 10             	add    $0x10,%esp
	}
	return r;
}
  801cd1:	89 d8                	mov    %ebx,%eax
  801cd3:	8d 65 f8             	lea    -0x8(%ebp),%esp
  801cd6:	5b                   	pop    %ebx
  801cd7:	5e                   	pop    %esi
  801cd8:	5d                   	pop    %ebp
  801cd9:	c3                   	ret    

00801cda <nsipc_bind>:

int
nsipc_bind(int s, struct sockaddr *name, socklen_t namelen)
{
  801cda:	55                   	push   %ebp
  801cdb:	89 e5                	mov    %esp,%ebp
  801cdd:	53                   	push   %ebx
  801cde:	83 ec 08             	sub    $0x8,%esp
  801ce1:	8b 5d 10             	mov    0x10(%ebp),%ebx
	nsipcbuf.bind.req_s = s;
  801ce4:	8b 45 08             	mov    0x8(%ebp),%eax
  801ce7:	a3 00 60 80 00       	mov    %eax,0x806000
	memmove(&nsipcbuf.bind.req_name, name, namelen);
  801cec:	53                   	push   %ebx
  801ced:	ff 75 0c             	pushl  0xc(%ebp)
  801cf0:	68 04 60 80 00       	push   $0x806004
  801cf5:	e8 ec ef ff ff       	call   800ce6 <memmove>
	nsipcbuf.bind.req_namelen = namelen;
  801cfa:	89 1d 14 60 80 00    	mov    %ebx,0x806014
	return nsipc(NSREQ_BIND);
  801d00:	b8 02 00 00 00       	mov    $0x2,%eax
  801d05:	e8 36 ff ff ff       	call   801c40 <nsipc>
}
  801d0a:	8b 5d fc             	mov    -0x4(%ebp),%ebx
  801d0d:	c9                   	leave  
  801d0e:	c3                   	ret    

00801d0f <nsipc_shutdown>:

int
nsipc_shutdown(int s, int how)
{
  801d0f:	55                   	push   %ebp
  801d10:	89 e5                	mov    %esp,%ebp
  801d12:	83 ec 08             	sub    $0x8,%esp
	nsipcbuf.shutdown.req_s = s;
  801d15:	8b 45 08             	mov    0x8(%ebp),%eax
  801d18:	a3 00 60 80 00       	mov    %eax,0x806000
	nsipcbuf.shutdown.req_how = how;
  801d1d:	8b 45 0c             	mov    0xc(%ebp),%eax
  801d20:	a3 04 60 80 00       	mov    %eax,0x806004
	return nsipc(NSREQ_SHUTDOWN);
  801d25:	b8 03 00 00 00       	mov    $0x3,%eax
  801d2a:	e8 11 ff ff ff       	call   801c40 <nsipc>
}
  801d2f:	c9                   	leave  
  801d30:	c3                   	ret    

00801d31 <nsipc_close>:

int
nsipc_close(int s)
{
  801d31:	55                   	push   %ebp
  801d32:	89 e5                	mov    %esp,%ebp
  801d34:	83 ec 08             	sub    $0x8,%esp
	nsipcbuf.close.req_s = s;
  801d37:	8b 45 08             	mov    0x8(%ebp),%eax
  801d3a:	a3 00 60 80 00       	mov    %eax,0x806000
	return nsipc(NSREQ_CLOSE);
  801d3f:	b8 04 00 00 00       	mov    $0x4,%eax
  801d44:	e8 f7 fe ff ff       	call   801c40 <nsipc>
}
  801d49:	c9                   	leave  
  801d4a:	c3                   	ret    

00801d4b <nsipc_connect>:

int
nsipc_connect(int s, const struct sockaddr *name, socklen_t namelen)
{
  801d4b:	55                   	push   %ebp
  801d4c:	89 e5                	mov    %esp,%ebp
  801d4e:	53                   	push   %ebx
  801d4f:	83 ec 08             	sub    $0x8,%esp
  801d52:	8b 5d 10             	mov    0x10(%ebp),%ebx
	nsipcbuf.connect.req_s = s;
  801d55:	8b 45 08             	mov    0x8(%ebp),%eax
  801d58:	a3 00 60 80 00       	mov    %eax,0x806000
	memmove(&nsipcbuf.connect.req_name, name, namelen);
  801d5d:	53                   	push   %ebx
  801d5e:	ff 75 0c             	pushl  0xc(%ebp)
  801d61:	68 04 60 80 00       	push   $0x806004
  801d66:	e8 7b ef ff ff       	call   800ce6 <memmove>
	nsipcbuf.connect.req_namelen = namelen;
  801d6b:	89 1d 14 60 80 00    	mov    %ebx,0x806014
	return nsipc(NSREQ_CONNECT);
  801d71:	b8 05 00 00 00       	mov    $0x5,%eax
  801d76:	e8 c5 fe ff ff       	call   801c40 <nsipc>
}
  801d7b:	8b 5d fc             	mov    -0x4(%ebp),%ebx
  801d7e:	c9                   	leave  
  801d7f:	c3                   	ret    

00801d80 <nsipc_listen>:

int
nsipc_listen(int s, int backlog)
{
  801d80:	55                   	push   %ebp
  801d81:	89 e5                	mov    %esp,%ebp
  801d83:	83 ec 08             	sub    $0x8,%esp
	nsipcbuf.listen.req_s = s;
  801d86:	8b 45 08             	mov    0x8(%ebp),%eax
  801d89:	a3 00 60 80 00       	mov    %eax,0x806000
	nsipcbuf.listen.req_backlog = backlog;
  801d8e:	8b 45 0c             	mov    0xc(%ebp),%eax
  801d91:	a3 04 60 80 00       	mov    %eax,0x806004
	return nsipc(NSREQ_LISTEN);
  801d96:	b8 06 00 00 00       	mov    $0x6,%eax
  801d9b:	e8 a0 fe ff ff       	call   801c40 <nsipc>
}
  801da0:	c9                   	leave  
  801da1:	c3                   	ret    

00801da2 <nsipc_recv>:

int
nsipc_recv(int s, void *mem, int len, unsigned int flags)
{
  801da2:	55                   	push   %ebp
  801da3:	89 e5                	mov    %esp,%ebp
  801da5:	56                   	push   %esi
  801da6:	53                   	push   %ebx
  801da7:	8b 75 10             	mov    0x10(%ebp),%esi
	int r;

	nsipcbuf.recv.req_s = s;
  801daa:	8b 45 08             	mov    0x8(%ebp),%eax
  801dad:	a3 00 60 80 00       	mov    %eax,0x806000
	nsipcbuf.recv.req_len = len;
  801db2:	89 35 04 60 80 00    	mov    %esi,0x806004
	nsipcbuf.recv.req_flags = flags;
  801db8:	8b 45 14             	mov    0x14(%ebp),%eax
  801dbb:	a3 08 60 80 00       	mov    %eax,0x806008

	if ((r = nsipc(NSREQ_RECV)) >= 0) {
  801dc0:	b8 07 00 00 00       	mov    $0x7,%eax
  801dc5:	e8 76 fe ff ff       	call   801c40 <nsipc>
  801dca:	89 c3                	mov    %eax,%ebx
  801dcc:	85 c0                	test   %eax,%eax
  801dce:	78 35                	js     801e05 <nsipc_recv+0x63>
		assert(r < 1600 && r <= len);
  801dd0:	39 f0                	cmp    %esi,%eax
  801dd2:	7f 07                	jg     801ddb <nsipc_recv+0x39>
  801dd4:	3d 3f 06 00 00       	cmp    $0x63f,%eax
  801dd9:	7e 16                	jle    801df1 <nsipc_recv+0x4f>
  801ddb:	68 1b 2d 80 00       	push   $0x802d1b
  801de0:	68 e3 2c 80 00       	push   $0x802ce3
  801de5:	6a 62                	push   $0x62
  801de7:	68 30 2d 80 00       	push   $0x802d30
  801dec:	e8 81 05 00 00       	call   802372 <_panic>
		memmove(mem, nsipcbuf.recvRet.ret_buf, r);
  801df1:	83 ec 04             	sub    $0x4,%esp
  801df4:	50                   	push   %eax
  801df5:	68 00 60 80 00       	push   $0x806000
  801dfa:	ff 75 0c             	pushl  0xc(%ebp)
  801dfd:	e8 e4 ee ff ff       	call   800ce6 <memmove>
  801e02:	83 c4 10             	add    $0x10,%esp
	}

	return r;
}
  801e05:	89 d8                	mov    %ebx,%eax
  801e07:	8d 65 f8             	lea    -0x8(%ebp),%esp
  801e0a:	5b                   	pop    %ebx
  801e0b:	5e                   	pop    %esi
  801e0c:	5d                   	pop    %ebp
  801e0d:	c3                   	ret    

00801e0e <nsipc_send>:

int
nsipc_send(int s, const void *buf, int size, unsigned int flags)
{
  801e0e:	55                   	push   %ebp
  801e0f:	89 e5                	mov    %esp,%ebp
  801e11:	53                   	push   %ebx
  801e12:	83 ec 04             	sub    $0x4,%esp
  801e15:	8b 5d 10             	mov    0x10(%ebp),%ebx
	nsipcbuf.send.req_s = s;
  801e18:	8b 45 08             	mov    0x8(%ebp),%eax
  801e1b:	a3 00 60 80 00       	mov    %eax,0x806000
	assert(size < 1600);
  801e20:	81 fb 3f 06 00 00    	cmp    $0x63f,%ebx
  801e26:	7e 16                	jle    801e3e <nsipc_send+0x30>
  801e28:	68 3c 2d 80 00       	push   $0x802d3c
  801e2d:	68 e3 2c 80 00       	push   $0x802ce3
  801e32:	6a 6d                	push   $0x6d
  801e34:	68 30 2d 80 00       	push   $0x802d30
  801e39:	e8 34 05 00 00       	call   802372 <_panic>
	memmove(&nsipcbuf.send.req_buf, buf, size);
  801e3e:	83 ec 04             	sub    $0x4,%esp
  801e41:	53                   	push   %ebx
  801e42:	ff 75 0c             	pushl  0xc(%ebp)
  801e45:	68 0c 60 80 00       	push   $0x80600c
  801e4a:	e8 97 ee ff ff       	call   800ce6 <memmove>
	nsipcbuf.send.req_size = size;
  801e4f:	89 1d 04 60 80 00    	mov    %ebx,0x806004
	nsipcbuf.send.req_flags = flags;
  801e55:	8b 45 14             	mov    0x14(%ebp),%eax
  801e58:	a3 08 60 80 00       	mov    %eax,0x806008
	return nsipc(NSREQ_SEND);
  801e5d:	b8 08 00 00 00       	mov    $0x8,%eax
  801e62:	e8 d9 fd ff ff       	call   801c40 <nsipc>
}
  801e67:	8b 5d fc             	mov    -0x4(%ebp),%ebx
  801e6a:	c9                   	leave  
  801e6b:	c3                   	ret    

00801e6c <nsipc_socket>:

int
nsipc_socket(int domain, int type, int protocol)
{
  801e6c:	55                   	push   %ebp
  801e6d:	89 e5                	mov    %esp,%ebp
  801e6f:	83 ec 08             	sub    $0x8,%esp
	nsipcbuf.socket.req_domain = domain;
  801e72:	8b 45 08             	mov    0x8(%ebp),%eax
  801e75:	a3 00 60 80 00       	mov    %eax,0x806000
	nsipcbuf.socket.req_type = type;
  801e7a:	8b 45 0c             	mov    0xc(%ebp),%eax
  801e7d:	a3 04 60 80 00       	mov    %eax,0x806004
	nsipcbuf.socket.req_protocol = protocol;
  801e82:	8b 45 10             	mov    0x10(%ebp),%eax
  801e85:	a3 08 60 80 00       	mov    %eax,0x806008
	return nsipc(NSREQ_SOCKET);
  801e8a:	b8 09 00 00 00       	mov    $0x9,%eax
  801e8f:	e8 ac fd ff ff       	call   801c40 <nsipc>
}
  801e94:	c9                   	leave  
  801e95:	c3                   	ret    

00801e96 <devpipe_stat>:
	return i;
}

static int
devpipe_stat(struct Fd *fd, struct Stat *stat)
{
  801e96:	55                   	push   %ebp
  801e97:	89 e5                	mov    %esp,%ebp
  801e99:	56                   	push   %esi
  801e9a:	53                   	push   %ebx
  801e9b:	8b 5d 0c             	mov    0xc(%ebp),%ebx
	struct Pipe *p = (struct Pipe*) fd2data(fd);
  801e9e:	83 ec 0c             	sub    $0xc,%esp
  801ea1:	ff 75 08             	pushl  0x8(%ebp)
  801ea4:	e8 56 f3 ff ff       	call   8011ff <fd2data>
  801ea9:	89 c6                	mov    %eax,%esi
	strcpy(stat->st_name, "<pipe>");
  801eab:	83 c4 08             	add    $0x8,%esp
  801eae:	68 48 2d 80 00       	push   $0x802d48
  801eb3:	53                   	push   %ebx
  801eb4:	e8 9b ec ff ff       	call   800b54 <strcpy>
	stat->st_size = p->p_wpos - p->p_rpos;
  801eb9:	8b 56 04             	mov    0x4(%esi),%edx
  801ebc:	89 d0                	mov    %edx,%eax
  801ebe:	2b 06                	sub    (%esi),%eax
  801ec0:	89 83 80 00 00 00    	mov    %eax,0x80(%ebx)
	stat->st_isdir = 0;
  801ec6:	c7 83 84 00 00 00 00 	movl   $0x0,0x84(%ebx)
  801ecd:	00 00 00 
	stat->st_dev = &devpipe;
  801ed0:	c7 83 88 00 00 00 3c 	movl   $0x80303c,0x88(%ebx)
  801ed7:	30 80 00 
	return 0;
}
  801eda:	b8 00 00 00 00       	mov    $0x0,%eax
  801edf:	8d 65 f8             	lea    -0x8(%ebp),%esp
  801ee2:	5b                   	pop    %ebx
  801ee3:	5e                   	pop    %esi
  801ee4:	5d                   	pop    %ebp
  801ee5:	c3                   	ret    

00801ee6 <devpipe_close>:

static int
devpipe_close(struct Fd *fd)
{
  801ee6:	55                   	push   %ebp
  801ee7:	89 e5                	mov    %esp,%ebp
  801ee9:	53                   	push   %ebx
  801eea:	83 ec 0c             	sub    $0xc,%esp
  801eed:	8b 5d 08             	mov    0x8(%ebp),%ebx
	(void) sys_page_unmap(0, fd);
  801ef0:	53                   	push   %ebx
  801ef1:	6a 00                	push   $0x0
  801ef3:	e8 ea f0 ff ff       	call   800fe2 <sys_page_unmap>
	return sys_page_unmap(0, fd2data(fd));
  801ef8:	89 1c 24             	mov    %ebx,(%esp)
  801efb:	e8 ff f2 ff ff       	call   8011ff <fd2data>
  801f00:	83 c4 08             	add    $0x8,%esp
  801f03:	50                   	push   %eax
  801f04:	6a 00                	push   $0x0
  801f06:	e8 d7 f0 ff ff       	call   800fe2 <sys_page_unmap>
}
  801f0b:	8b 5d fc             	mov    -0x4(%ebp),%ebx
  801f0e:	c9                   	leave  
  801f0f:	c3                   	ret    

00801f10 <_pipeisclosed>:
	return r;
}

static int
_pipeisclosed(struct Fd *fd, struct Pipe *p)
{
  801f10:	55                   	push   %ebp
  801f11:	89 e5                	mov    %esp,%ebp
  801f13:	57                   	push   %edi
  801f14:	56                   	push   %esi
  801f15:	53                   	push   %ebx
  801f16:	83 ec 1c             	sub    $0x1c,%esp
  801f19:	89 c6                	mov    %eax,%esi
  801f1b:	89 55 e4             	mov    %edx,-0x1c(%ebp)
	int n, nn, ret;

	while (1) {
		n = thisenv->env_runs;
  801f1e:	a1 18 40 80 00       	mov    0x804018,%eax
  801f23:	8b 58 58             	mov    0x58(%eax),%ebx
		ret = pageref(fd) == pageref(p);
  801f26:	83 ec 0c             	sub    $0xc,%esp
  801f29:	56                   	push   %esi
  801f2a:	e8 7e 05 00 00       	call   8024ad <pageref>
  801f2f:	89 c7                	mov    %eax,%edi
  801f31:	83 c4 04             	add    $0x4,%esp
  801f34:	ff 75 e4             	pushl  -0x1c(%ebp)
  801f37:	e8 71 05 00 00       	call   8024ad <pageref>
  801f3c:	83 c4 10             	add    $0x10,%esp
  801f3f:	39 c7                	cmp    %eax,%edi
  801f41:	0f 94 c2             	sete   %dl
  801f44:	0f b6 c2             	movzbl %dl,%eax
		nn = thisenv->env_runs;
  801f47:	8b 0d 18 40 80 00    	mov    0x804018,%ecx
  801f4d:	8b 79 58             	mov    0x58(%ecx),%edi
		if (n == nn)
  801f50:	39 fb                	cmp    %edi,%ebx
  801f52:	74 19                	je     801f6d <_pipeisclosed+0x5d>
			return ret;
		if (n != nn && ret == 1)
  801f54:	84 d2                	test   %dl,%dl
  801f56:	74 c6                	je     801f1e <_pipeisclosed+0xe>
			cprintf("pipe race avoided\n", n, thisenv->env_runs, ret);
  801f58:	8b 51 58             	mov    0x58(%ecx),%edx
  801f5b:	50                   	push   %eax
  801f5c:	52                   	push   %edx
  801f5d:	53                   	push   %ebx
  801f5e:	68 4f 2d 80 00       	push   $0x802d4f
  801f63:	e8 65 e6 ff ff       	call   8005cd <cprintf>
  801f68:	83 c4 10             	add    $0x10,%esp
  801f6b:	eb b1                	jmp    801f1e <_pipeisclosed+0xe>
	}
}
  801f6d:	8d 65 f4             	lea    -0xc(%ebp),%esp
  801f70:	5b                   	pop    %ebx
  801f71:	5e                   	pop    %esi
  801f72:	5f                   	pop    %edi
  801f73:	5d                   	pop    %ebp
  801f74:	c3                   	ret    

00801f75 <devpipe_write>:
	return i;
}

static ssize_t
devpipe_write(struct Fd *fd, const void *vbuf, size_t n)
{
  801f75:	55                   	push   %ebp
  801f76:	89 e5                	mov    %esp,%ebp
  801f78:	57                   	push   %edi
  801f79:	56                   	push   %esi
  801f7a:	53                   	push   %ebx
  801f7b:	83 ec 28             	sub    $0x28,%esp
  801f7e:	8b 75 08             	mov    0x8(%ebp),%esi
	const uint8_t *buf;
	size_t i;
	struct Pipe *p;

	p = (struct Pipe*) fd2data(fd);
  801f81:	56                   	push   %esi
  801f82:	e8 78 f2 ff ff       	call   8011ff <fd2data>
  801f87:	89 c3                	mov    %eax,%ebx
	if (debug)
		cprintf("[%08x] devpipe_write %08x %d rpos %d wpos %d\n",
			thisenv->env_id, uvpt[PGNUM(p)], n, p->p_rpos, p->p_wpos);

	buf = vbuf;
	for (i = 0; i < n; i++) {
  801f89:	83 c4 10             	add    $0x10,%esp
  801f8c:	bf 00 00 00 00       	mov    $0x0,%edi
  801f91:	eb 4b                	jmp    801fde <devpipe_write+0x69>
		while (p->p_wpos >= p->p_rpos + sizeof(p->p_buf)) {
			// pipe is full
			// if all the readers are gone
			// (it's only writers like us now),
			// note eof
			if (_pipeisclosed(fd, p))
  801f93:	89 da                	mov    %ebx,%edx
  801f95:	89 f0                	mov    %esi,%eax
  801f97:	e8 74 ff ff ff       	call   801f10 <_pipeisclosed>
  801f9c:	85 c0                	test   %eax,%eax
  801f9e:	75 48                	jne    801fe8 <devpipe_write+0x73>
				return 0;
			// yield and see what happens
			if (debug)
				cprintf("devpipe_write yield\n");
			sys_yield();
  801fa0:	e8 99 ef ff ff       	call   800f3e <sys_yield>
		cprintf("[%08x] devpipe_write %08x %d rpos %d wpos %d\n",
			thisenv->env_id, uvpt[PGNUM(p)], n, p->p_rpos, p->p_wpos);

	buf = vbuf;
	for (i = 0; i < n; i++) {
		while (p->p_wpos >= p->p_rpos + sizeof(p->p_buf)) {
  801fa5:	8b 43 04             	mov    0x4(%ebx),%eax
  801fa8:	8b 0b                	mov    (%ebx),%ecx
  801faa:	8d 51 20             	lea    0x20(%ecx),%edx
  801fad:	39 d0                	cmp    %edx,%eax
  801faf:	73 e2                	jae    801f93 <devpipe_write+0x1e>
				cprintf("devpipe_write yield\n");
			sys_yield();
		}
		// there's room for a byte.  store it.
		// wait to increment wpos until the byte is stored!
		p->p_buf[p->p_wpos % PIPEBUFSIZ] = buf[i];
  801fb1:	8b 4d 0c             	mov    0xc(%ebp),%ecx
  801fb4:	0f b6 0c 39          	movzbl (%ecx,%edi,1),%ecx
  801fb8:	88 4d e7             	mov    %cl,-0x19(%ebp)
  801fbb:	89 c2                	mov    %eax,%edx
  801fbd:	c1 fa 1f             	sar    $0x1f,%edx
  801fc0:	89 d1                	mov    %edx,%ecx
  801fc2:	c1 e9 1b             	shr    $0x1b,%ecx
  801fc5:	8d 14 08             	lea    (%eax,%ecx,1),%edx
  801fc8:	83 e2 1f             	and    $0x1f,%edx
  801fcb:	29 ca                	sub    %ecx,%edx
  801fcd:	0f b6 4d e7          	movzbl -0x19(%ebp),%ecx
  801fd1:	88 4c 13 08          	mov    %cl,0x8(%ebx,%edx,1)
		p->p_wpos++;
  801fd5:	83 c0 01             	add    $0x1,%eax
  801fd8:	89 43 04             	mov    %eax,0x4(%ebx)
	if (debug)
		cprintf("[%08x] devpipe_write %08x %d rpos %d wpos %d\n",
			thisenv->env_id, uvpt[PGNUM(p)], n, p->p_rpos, p->p_wpos);

	buf = vbuf;
	for (i = 0; i < n; i++) {
  801fdb:	83 c7 01             	add    $0x1,%edi
  801fde:	3b 7d 10             	cmp    0x10(%ebp),%edi
  801fe1:	75 c2                	jne    801fa5 <devpipe_write+0x30>
		// wait to increment wpos until the byte is stored!
		p->p_buf[p->p_wpos % PIPEBUFSIZ] = buf[i];
		p->p_wpos++;
	}

	return i;
  801fe3:	8b 45 10             	mov    0x10(%ebp),%eax
  801fe6:	eb 05                	jmp    801fed <devpipe_write+0x78>
			// pipe is full
			// if all the readers are gone
			// (it's only writers like us now),
			// note eof
			if (_pipeisclosed(fd, p))
				return 0;
  801fe8:	b8 00 00 00 00       	mov    $0x0,%eax
		p->p_buf[p->p_wpos % PIPEBUFSIZ] = buf[i];
		p->p_wpos++;
	}

	return i;
}
  801fed:	8d 65 f4             	lea    -0xc(%ebp),%esp
  801ff0:	5b                   	pop    %ebx
  801ff1:	5e                   	pop    %esi
  801ff2:	5f                   	pop    %edi
  801ff3:	5d                   	pop    %ebp
  801ff4:	c3                   	ret    

00801ff5 <devpipe_read>:
	return _pipeisclosed(fd, p);
}

static ssize_t
devpipe_read(struct Fd *fd, void *vbuf, size_t n)
{
  801ff5:	55                   	push   %ebp
  801ff6:	89 e5                	mov    %esp,%ebp
  801ff8:	57                   	push   %edi
  801ff9:	56                   	push   %esi
  801ffa:	53                   	push   %ebx
  801ffb:	83 ec 18             	sub    $0x18,%esp
  801ffe:	8b 7d 08             	mov    0x8(%ebp),%edi
	uint8_t *buf;
	size_t i;
	struct Pipe *p;

	p = (struct Pipe*)fd2data(fd);
  802001:	57                   	push   %edi
  802002:	e8 f8 f1 ff ff       	call   8011ff <fd2data>
  802007:	89 c6                	mov    %eax,%esi
	if (debug)
		cprintf("[%08x] devpipe_read %08x %d rpos %d wpos %d\n",
			thisenv->env_id, uvpt[PGNUM(p)], n, p->p_rpos, p->p_wpos);

	buf = vbuf;
	for (i = 0; i < n; i++) {
  802009:	83 c4 10             	add    $0x10,%esp
  80200c:	bb 00 00 00 00       	mov    $0x0,%ebx
  802011:	eb 3d                	jmp    802050 <devpipe_read+0x5b>
		while (p->p_rpos == p->p_wpos) {
			// pipe is empty
			// if we got any data, return it
			if (i > 0)
  802013:	85 db                	test   %ebx,%ebx
  802015:	74 04                	je     80201b <devpipe_read+0x26>
				return i;
  802017:	89 d8                	mov    %ebx,%eax
  802019:	eb 44                	jmp    80205f <devpipe_read+0x6a>
			// if all the writers are gone, note eof
			if (_pipeisclosed(fd, p))
  80201b:	89 f2                	mov    %esi,%edx
  80201d:	89 f8                	mov    %edi,%eax
  80201f:	e8 ec fe ff ff       	call   801f10 <_pipeisclosed>
  802024:	85 c0                	test   %eax,%eax
  802026:	75 32                	jne    80205a <devpipe_read+0x65>
				return 0;
			// yield and see what happens
			if (debug)
				cprintf("devpipe_read yield\n");
			sys_yield();
  802028:	e8 11 ef ff ff       	call   800f3e <sys_yield>
		cprintf("[%08x] devpipe_read %08x %d rpos %d wpos %d\n",
			thisenv->env_id, uvpt[PGNUM(p)], n, p->p_rpos, p->p_wpos);

	buf = vbuf;
	for (i = 0; i < n; i++) {
		while (p->p_rpos == p->p_wpos) {
  80202d:	8b 06                	mov    (%esi),%eax
  80202f:	3b 46 04             	cmp    0x4(%esi),%eax
  802032:	74 df                	je     802013 <devpipe_read+0x1e>
				cprintf("devpipe_read yield\n");
			sys_yield();
		}
		// there's a byte.  take it.
		// wait to increment rpos until the byte is taken!
		buf[i] = p->p_buf[p->p_rpos % PIPEBUFSIZ];
  802034:	99                   	cltd   
  802035:	c1 ea 1b             	shr    $0x1b,%edx
  802038:	01 d0                	add    %edx,%eax
  80203a:	83 e0 1f             	and    $0x1f,%eax
  80203d:	29 d0                	sub    %edx,%eax
  80203f:	0f b6 44 06 08       	movzbl 0x8(%esi,%eax,1),%eax
  802044:	8b 4d 0c             	mov    0xc(%ebp),%ecx
  802047:	88 04 19             	mov    %al,(%ecx,%ebx,1)
		p->p_rpos++;
  80204a:	83 06 01             	addl   $0x1,(%esi)
	if (debug)
		cprintf("[%08x] devpipe_read %08x %d rpos %d wpos %d\n",
			thisenv->env_id, uvpt[PGNUM(p)], n, p->p_rpos, p->p_wpos);

	buf = vbuf;
	for (i = 0; i < n; i++) {
  80204d:	83 c3 01             	add    $0x1,%ebx
  802050:	3b 5d 10             	cmp    0x10(%ebp),%ebx
  802053:	75 d8                	jne    80202d <devpipe_read+0x38>
		// there's a byte.  take it.
		// wait to increment rpos until the byte is taken!
		buf[i] = p->p_buf[p->p_rpos % PIPEBUFSIZ];
		p->p_rpos++;
	}
	return i;
  802055:	8b 45 10             	mov    0x10(%ebp),%eax
  802058:	eb 05                	jmp    80205f <devpipe_read+0x6a>
			// if we got any data, return it
			if (i > 0)
				return i;
			// if all the writers are gone, note eof
			if (_pipeisclosed(fd, p))
				return 0;
  80205a:	b8 00 00 00 00       	mov    $0x0,%eax
		// wait to increment rpos until the byte is taken!
		buf[i] = p->p_buf[p->p_rpos % PIPEBUFSIZ];
		p->p_rpos++;
	}
	return i;
}
  80205f:	8d 65 f4             	lea    -0xc(%ebp),%esp
  802062:	5b                   	pop    %ebx
  802063:	5e                   	pop    %esi
  802064:	5f                   	pop    %edi
  802065:	5d                   	pop    %ebp
  802066:	c3                   	ret    

00802067 <pipe>:
	uint8_t p_buf[PIPEBUFSIZ];	// data buffer
};

int
pipe(int pfd[2])
{
  802067:	55                   	push   %ebp
  802068:	89 e5                	mov    %esp,%ebp
  80206a:	56                   	push   %esi
  80206b:	53                   	push   %ebx
  80206c:	83 ec 1c             	sub    $0x1c,%esp
	int r;
	struct Fd *fd0, *fd1;
	void *va;

	// allocate the file descriptor table entries
	if ((r = fd_alloc(&fd0)) < 0
  80206f:	8d 45 f4             	lea    -0xc(%ebp),%eax
  802072:	50                   	push   %eax
  802073:	e8 9e f1 ff ff       	call   801216 <fd_alloc>
  802078:	83 c4 10             	add    $0x10,%esp
  80207b:	89 c2                	mov    %eax,%edx
  80207d:	85 c0                	test   %eax,%eax
  80207f:	0f 88 2c 01 00 00    	js     8021b1 <pipe+0x14a>
	    || (r = sys_page_alloc(0, fd0, PTE_P|PTE_W|PTE_U|PTE_SHARE)) < 0)
  802085:	83 ec 04             	sub    $0x4,%esp
  802088:	68 07 04 00 00       	push   $0x407
  80208d:	ff 75 f4             	pushl  -0xc(%ebp)
  802090:	6a 00                	push   $0x0
  802092:	e8 c6 ee ff ff       	call   800f5d <sys_page_alloc>
  802097:	83 c4 10             	add    $0x10,%esp
  80209a:	89 c2                	mov    %eax,%edx
  80209c:	85 c0                	test   %eax,%eax
  80209e:	0f 88 0d 01 00 00    	js     8021b1 <pipe+0x14a>
		goto err;

	if ((r = fd_alloc(&fd1)) < 0
  8020a4:	83 ec 0c             	sub    $0xc,%esp
  8020a7:	8d 45 f0             	lea    -0x10(%ebp),%eax
  8020aa:	50                   	push   %eax
  8020ab:	e8 66 f1 ff ff       	call   801216 <fd_alloc>
  8020b0:	89 c3                	mov    %eax,%ebx
  8020b2:	83 c4 10             	add    $0x10,%esp
  8020b5:	85 c0                	test   %eax,%eax
  8020b7:	0f 88 e2 00 00 00    	js     80219f <pipe+0x138>
	    || (r = sys_page_alloc(0, fd1, PTE_P|PTE_W|PTE_U|PTE_SHARE)) < 0)
  8020bd:	83 ec 04             	sub    $0x4,%esp
  8020c0:	68 07 04 00 00       	push   $0x407
  8020c5:	ff 75 f0             	pushl  -0x10(%ebp)
  8020c8:	6a 00                	push   $0x0
  8020ca:	e8 8e ee ff ff       	call   800f5d <sys_page_alloc>
  8020cf:	89 c3                	mov    %eax,%ebx
  8020d1:	83 c4 10             	add    $0x10,%esp
  8020d4:	85 c0                	test   %eax,%eax
  8020d6:	0f 88 c3 00 00 00    	js     80219f <pipe+0x138>
		goto err1;

	// allocate the pipe structure as first data page in both
	va = fd2data(fd0);
  8020dc:	83 ec 0c             	sub    $0xc,%esp
  8020df:	ff 75 f4             	pushl  -0xc(%ebp)
  8020e2:	e8 18 f1 ff ff       	call   8011ff <fd2data>
  8020e7:	89 c6                	mov    %eax,%esi
	if ((r = sys_page_alloc(0, va, PTE_P|PTE_W|PTE_U|PTE_SHARE)) < 0)
  8020e9:	83 c4 0c             	add    $0xc,%esp
  8020ec:	68 07 04 00 00       	push   $0x407
  8020f1:	50                   	push   %eax
  8020f2:	6a 00                	push   $0x0
  8020f4:	e8 64 ee ff ff       	call   800f5d <sys_page_alloc>
  8020f9:	89 c3                	mov    %eax,%ebx
  8020fb:	83 c4 10             	add    $0x10,%esp
  8020fe:	85 c0                	test   %eax,%eax
  802100:	0f 88 89 00 00 00    	js     80218f <pipe+0x128>
		goto err2;
	if ((r = sys_page_map(0, va, 0, fd2data(fd1), PTE_P|PTE_W|PTE_U|PTE_SHARE)) < 0)
  802106:	83 ec 0c             	sub    $0xc,%esp
  802109:	ff 75 f0             	pushl  -0x10(%ebp)
  80210c:	e8 ee f0 ff ff       	call   8011ff <fd2data>
  802111:	c7 04 24 07 04 00 00 	movl   $0x407,(%esp)
  802118:	50                   	push   %eax
  802119:	6a 00                	push   $0x0
  80211b:	56                   	push   %esi
  80211c:	6a 00                	push   $0x0
  80211e:	e8 7d ee ff ff       	call   800fa0 <sys_page_map>
  802123:	89 c3                	mov    %eax,%ebx
  802125:	83 c4 20             	add    $0x20,%esp
  802128:	85 c0                	test   %eax,%eax
  80212a:	78 55                	js     802181 <pipe+0x11a>
		goto err3;

	// set up fd structures
	fd0->fd_dev_id = devpipe.dev_id;
  80212c:	8b 15 3c 30 80 00    	mov    0x80303c,%edx
  802132:	8b 45 f4             	mov    -0xc(%ebp),%eax
  802135:	89 10                	mov    %edx,(%eax)
	fd0->fd_omode = O_RDONLY;
  802137:	8b 45 f4             	mov    -0xc(%ebp),%eax
  80213a:	c7 40 08 00 00 00 00 	movl   $0x0,0x8(%eax)

	fd1->fd_dev_id = devpipe.dev_id;
  802141:	8b 15 3c 30 80 00    	mov    0x80303c,%edx
  802147:	8b 45 f0             	mov    -0x10(%ebp),%eax
  80214a:	89 10                	mov    %edx,(%eax)
	fd1->fd_omode = O_WRONLY;
  80214c:	8b 45 f0             	mov    -0x10(%ebp),%eax
  80214f:	c7 40 08 01 00 00 00 	movl   $0x1,0x8(%eax)

	if (debug)
		cprintf("[%08x] pipecreate %08x\n", thisenv->env_id, uvpt[PGNUM(va)]);

	pfd[0] = fd2num(fd0);
  802156:	83 ec 0c             	sub    $0xc,%esp
  802159:	ff 75 f4             	pushl  -0xc(%ebp)
  80215c:	e8 8e f0 ff ff       	call   8011ef <fd2num>
  802161:	8b 4d 08             	mov    0x8(%ebp),%ecx
  802164:	89 01                	mov    %eax,(%ecx)
	pfd[1] = fd2num(fd1);
  802166:	83 c4 04             	add    $0x4,%esp
  802169:	ff 75 f0             	pushl  -0x10(%ebp)
  80216c:	e8 7e f0 ff ff       	call   8011ef <fd2num>
  802171:	8b 4d 08             	mov    0x8(%ebp),%ecx
  802174:	89 41 04             	mov    %eax,0x4(%ecx)
	return 0;
  802177:	83 c4 10             	add    $0x10,%esp
  80217a:	ba 00 00 00 00       	mov    $0x0,%edx
  80217f:	eb 30                	jmp    8021b1 <pipe+0x14a>

    err3:
	sys_page_unmap(0, va);
  802181:	83 ec 08             	sub    $0x8,%esp
  802184:	56                   	push   %esi
  802185:	6a 00                	push   $0x0
  802187:	e8 56 ee ff ff       	call   800fe2 <sys_page_unmap>
  80218c:	83 c4 10             	add    $0x10,%esp
    err2:
	sys_page_unmap(0, fd1);
  80218f:	83 ec 08             	sub    $0x8,%esp
  802192:	ff 75 f0             	pushl  -0x10(%ebp)
  802195:	6a 00                	push   $0x0
  802197:	e8 46 ee ff ff       	call   800fe2 <sys_page_unmap>
  80219c:	83 c4 10             	add    $0x10,%esp
    err1:
	sys_page_unmap(0, fd0);
  80219f:	83 ec 08             	sub    $0x8,%esp
  8021a2:	ff 75 f4             	pushl  -0xc(%ebp)
  8021a5:	6a 00                	push   $0x0
  8021a7:	e8 36 ee ff ff       	call   800fe2 <sys_page_unmap>
  8021ac:	83 c4 10             	add    $0x10,%esp
  8021af:	89 da                	mov    %ebx,%edx
    err:
	return r;
}
  8021b1:	89 d0                	mov    %edx,%eax
  8021b3:	8d 65 f8             	lea    -0x8(%ebp),%esp
  8021b6:	5b                   	pop    %ebx
  8021b7:	5e                   	pop    %esi
  8021b8:	5d                   	pop    %ebp
  8021b9:	c3                   	ret    

008021ba <pipeisclosed>:
	}
}

int
pipeisclosed(int fdnum)
{
  8021ba:	55                   	push   %ebp
  8021bb:	89 e5                	mov    %esp,%ebp
  8021bd:	83 ec 20             	sub    $0x20,%esp
	struct Fd *fd;
	struct Pipe *p;
	int r;

	if ((r = fd_lookup(fdnum, &fd)) < 0)
  8021c0:	8d 45 f4             	lea    -0xc(%ebp),%eax
  8021c3:	50                   	push   %eax
  8021c4:	ff 75 08             	pushl  0x8(%ebp)
  8021c7:	e8 99 f0 ff ff       	call   801265 <fd_lookup>
  8021cc:	89 c2                	mov    %eax,%edx
  8021ce:	83 c4 10             	add    $0x10,%esp
  8021d1:	85 d2                	test   %edx,%edx
  8021d3:	78 18                	js     8021ed <pipeisclosed+0x33>
		return r;
	p = (struct Pipe*) fd2data(fd);
  8021d5:	83 ec 0c             	sub    $0xc,%esp
  8021d8:	ff 75 f4             	pushl  -0xc(%ebp)
  8021db:	e8 1f f0 ff ff       	call   8011ff <fd2data>
	return _pipeisclosed(fd, p);
  8021e0:	89 c2                	mov    %eax,%edx
  8021e2:	8b 45 f4             	mov    -0xc(%ebp),%eax
  8021e5:	e8 26 fd ff ff       	call   801f10 <_pipeisclosed>
  8021ea:	83 c4 10             	add    $0x10,%esp
}
  8021ed:	c9                   	leave  
  8021ee:	c3                   	ret    

008021ef <devcons_close>:
	return tot;
}

static int
devcons_close(struct Fd *fd)
{
  8021ef:	55                   	push   %ebp
  8021f0:	89 e5                	mov    %esp,%ebp
	USED(fd);

	return 0;
}
  8021f2:	b8 00 00 00 00       	mov    $0x0,%eax
  8021f7:	5d                   	pop    %ebp
  8021f8:	c3                   	ret    

008021f9 <devcons_stat>:

static int
devcons_stat(struct Fd *fd, struct Stat *stat)
{
  8021f9:	55                   	push   %ebp
  8021fa:	89 e5                	mov    %esp,%ebp
  8021fc:	83 ec 10             	sub    $0x10,%esp
	strcpy(stat->st_name, "<cons>");
  8021ff:	68 67 2d 80 00       	push   $0x802d67
  802204:	ff 75 0c             	pushl  0xc(%ebp)
  802207:	e8 48 e9 ff ff       	call   800b54 <strcpy>
	return 0;
}
  80220c:	b8 00 00 00 00       	mov    $0x0,%eax
  802211:	c9                   	leave  
  802212:	c3                   	ret    

00802213 <devcons_write>:
	return 1;
}

static ssize_t
devcons_write(struct Fd *fd, const void *vbuf, size_t n)
{
  802213:	55                   	push   %ebp
  802214:	89 e5                	mov    %esp,%ebp
  802216:	57                   	push   %edi
  802217:	56                   	push   %esi
  802218:	53                   	push   %ebx
  802219:	81 ec 8c 00 00 00    	sub    $0x8c,%esp
	int tot, m;
	char buf[128];

	// mistake: have to nul-terminate arg to sys_cputs,
	// so we have to copy vbuf into buf in chunks and nul-terminate.
	for (tot = 0; tot < n; tot += m) {
  80221f:	be 00 00 00 00       	mov    $0x0,%esi
		m = n - tot;
		if (m > sizeof(buf) - 1)
			m = sizeof(buf) - 1;
		memmove(buf, (char*)vbuf + tot, m);
  802224:	8d bd 68 ff ff ff    	lea    -0x98(%ebp),%edi
	int tot, m;
	char buf[128];

	// mistake: have to nul-terminate arg to sys_cputs,
	// so we have to copy vbuf into buf in chunks and nul-terminate.
	for (tot = 0; tot < n; tot += m) {
  80222a:	eb 2d                	jmp    802259 <devcons_write+0x46>
		m = n - tot;
  80222c:	8b 5d 10             	mov    0x10(%ebp),%ebx
  80222f:	29 f3                	sub    %esi,%ebx
		if (m > sizeof(buf) - 1)
  802231:	83 fb 7f             	cmp    $0x7f,%ebx
	char buf[128];

	// mistake: have to nul-terminate arg to sys_cputs,
	// so we have to copy vbuf into buf in chunks and nul-terminate.
	for (tot = 0; tot < n; tot += m) {
		m = n - tot;
  802234:	ba 7f 00 00 00       	mov    $0x7f,%edx
  802239:	0f 47 da             	cmova  %edx,%ebx
		if (m > sizeof(buf) - 1)
			m = sizeof(buf) - 1;
		memmove(buf, (char*)vbuf + tot, m);
  80223c:	83 ec 04             	sub    $0x4,%esp
  80223f:	53                   	push   %ebx
  802240:	03 45 0c             	add    0xc(%ebp),%eax
  802243:	50                   	push   %eax
  802244:	57                   	push   %edi
  802245:	e8 9c ea ff ff       	call   800ce6 <memmove>
		sys_cputs(buf, m);
  80224a:	83 c4 08             	add    $0x8,%esp
  80224d:	53                   	push   %ebx
  80224e:	57                   	push   %edi
  80224f:	e8 4d ec ff ff       	call   800ea1 <sys_cputs>
	int tot, m;
	char buf[128];

	// mistake: have to nul-terminate arg to sys_cputs,
	// so we have to copy vbuf into buf in chunks and nul-terminate.
	for (tot = 0; tot < n; tot += m) {
  802254:	01 de                	add    %ebx,%esi
  802256:	83 c4 10             	add    $0x10,%esp
  802259:	89 f0                	mov    %esi,%eax
  80225b:	3b 75 10             	cmp    0x10(%ebp),%esi
  80225e:	72 cc                	jb     80222c <devcons_write+0x19>
			m = sizeof(buf) - 1;
		memmove(buf, (char*)vbuf + tot, m);
		sys_cputs(buf, m);
	}
	return tot;
}
  802260:	8d 65 f4             	lea    -0xc(%ebp),%esp
  802263:	5b                   	pop    %ebx
  802264:	5e                   	pop    %esi
  802265:	5f                   	pop    %edi
  802266:	5d                   	pop    %ebp
  802267:	c3                   	ret    

00802268 <devcons_read>:
	return fd2num(fd);
}

static ssize_t
devcons_read(struct Fd *fd, void *vbuf, size_t n)
{
  802268:	55                   	push   %ebp
  802269:	89 e5                	mov    %esp,%ebp
  80226b:	83 ec 08             	sub    $0x8,%esp
	int c;

	if (n == 0)
		return 0;
  80226e:	b8 00 00 00 00       	mov    $0x0,%eax
static ssize_t
devcons_read(struct Fd *fd, void *vbuf, size_t n)
{
	int c;

	if (n == 0)
  802273:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
  802277:	75 07                	jne    802280 <devcons_read+0x18>
  802279:	eb 28                	jmp    8022a3 <devcons_read+0x3b>
		return 0;

	while ((c = sys_cgetc()) == 0)
		sys_yield();
  80227b:	e8 be ec ff ff       	call   800f3e <sys_yield>
	int c;

	if (n == 0)
		return 0;

	while ((c = sys_cgetc()) == 0)
  802280:	e8 3a ec ff ff       	call   800ebf <sys_cgetc>
  802285:	85 c0                	test   %eax,%eax
  802287:	74 f2                	je     80227b <devcons_read+0x13>
		sys_yield();
	if (c < 0)
  802289:	85 c0                	test   %eax,%eax
  80228b:	78 16                	js     8022a3 <devcons_read+0x3b>
		return c;
	if (c == 0x04)	// ctl-d is eof
  80228d:	83 f8 04             	cmp    $0x4,%eax
  802290:	74 0c                	je     80229e <devcons_read+0x36>
		return 0;
	*(char*)vbuf = c;
  802292:	8b 55 0c             	mov    0xc(%ebp),%edx
  802295:	88 02                	mov    %al,(%edx)
	return 1;
  802297:	b8 01 00 00 00       	mov    $0x1,%eax
  80229c:	eb 05                	jmp    8022a3 <devcons_read+0x3b>
	while ((c = sys_cgetc()) == 0)
		sys_yield();
	if (c < 0)
		return c;
	if (c == 0x04)	// ctl-d is eof
		return 0;
  80229e:	b8 00 00 00 00       	mov    $0x0,%eax
	*(char*)vbuf = c;
	return 1;
}
  8022a3:	c9                   	leave  
  8022a4:	c3                   	ret    

008022a5 <cputchar>:
#include <inc/string.h>
#include <inc/lib.h>

void
cputchar(int ch)
{
  8022a5:	55                   	push   %ebp
  8022a6:	89 e5                	mov    %esp,%ebp
  8022a8:	83 ec 20             	sub    $0x20,%esp
	char c = ch;
  8022ab:	8b 45 08             	mov    0x8(%ebp),%eax
  8022ae:	88 45 f7             	mov    %al,-0x9(%ebp)

	// Unlike standard Unix's putchar,
	// the cputchar function _always_ outputs to the system console.
	sys_cputs(&c, 1);
  8022b1:	6a 01                	push   $0x1
  8022b3:	8d 45 f7             	lea    -0x9(%ebp),%eax
  8022b6:	50                   	push   %eax
  8022b7:	e8 e5 eb ff ff       	call   800ea1 <sys_cputs>
  8022bc:	83 c4 10             	add    $0x10,%esp
}
  8022bf:	c9                   	leave  
  8022c0:	c3                   	ret    

008022c1 <getchar>:

int
getchar(void)
{
  8022c1:	55                   	push   %ebp
  8022c2:	89 e5                	mov    %esp,%ebp
  8022c4:	83 ec 1c             	sub    $0x1c,%esp
	int r;

	// JOS does, however, support standard _input_ redirection,
	// allowing the user to redirect script files to the shell and such.
	// getchar() reads a character from file descriptor 0.
	r = read(0, &c, 1);
  8022c7:	6a 01                	push   $0x1
  8022c9:	8d 45 f7             	lea    -0x9(%ebp),%eax
  8022cc:	50                   	push   %eax
  8022cd:	6a 00                	push   $0x0
  8022cf:	e8 00 f2 ff ff       	call   8014d4 <read>
	if (r < 0)
  8022d4:	83 c4 10             	add    $0x10,%esp
  8022d7:	85 c0                	test   %eax,%eax
  8022d9:	78 0f                	js     8022ea <getchar+0x29>
		return r;
	if (r < 1)
  8022db:	85 c0                	test   %eax,%eax
  8022dd:	7e 06                	jle    8022e5 <getchar+0x24>
		return -E_EOF;
	return c;
  8022df:	0f b6 45 f7          	movzbl -0x9(%ebp),%eax
  8022e3:	eb 05                	jmp    8022ea <getchar+0x29>
	// getchar() reads a character from file descriptor 0.
	r = read(0, &c, 1);
	if (r < 0)
		return r;
	if (r < 1)
		return -E_EOF;
  8022e5:	b8 f8 ff ff ff       	mov    $0xfffffff8,%eax
	return c;
}
  8022ea:	c9                   	leave  
  8022eb:	c3                   	ret    

008022ec <iscons>:
	.dev_stat =	devcons_stat
};

int
iscons(int fdnum)
{
  8022ec:	55                   	push   %ebp
  8022ed:	89 e5                	mov    %esp,%ebp
  8022ef:	83 ec 20             	sub    $0x20,%esp
	int r;
	struct Fd *fd;

	if ((r = fd_lookup(fdnum, &fd)) < 0)
  8022f2:	8d 45 f4             	lea    -0xc(%ebp),%eax
  8022f5:	50                   	push   %eax
  8022f6:	ff 75 08             	pushl  0x8(%ebp)
  8022f9:	e8 67 ef ff ff       	call   801265 <fd_lookup>
  8022fe:	83 c4 10             	add    $0x10,%esp
  802301:	85 c0                	test   %eax,%eax
  802303:	78 11                	js     802316 <iscons+0x2a>
		return r;
	return fd->fd_dev_id == devcons.dev_id;
  802305:	8b 45 f4             	mov    -0xc(%ebp),%eax
  802308:	8b 15 58 30 80 00    	mov    0x803058,%edx
  80230e:	39 10                	cmp    %edx,(%eax)
  802310:	0f 94 c0             	sete   %al
  802313:	0f b6 c0             	movzbl %al,%eax
}
  802316:	c9                   	leave  
  802317:	c3                   	ret    

00802318 <opencons>:

int
opencons(void)
{
  802318:	55                   	push   %ebp
  802319:	89 e5                	mov    %esp,%ebp
  80231b:	83 ec 24             	sub    $0x24,%esp
	int r;
	struct Fd* fd;

	if ((r = fd_alloc(&fd)) < 0)
  80231e:	8d 45 f4             	lea    -0xc(%ebp),%eax
  802321:	50                   	push   %eax
  802322:	e8 ef ee ff ff       	call   801216 <fd_alloc>
  802327:	83 c4 10             	add    $0x10,%esp
		return r;
  80232a:	89 c2                	mov    %eax,%edx
opencons(void)
{
	int r;
	struct Fd* fd;

	if ((r = fd_alloc(&fd)) < 0)
  80232c:	85 c0                	test   %eax,%eax
  80232e:	78 3e                	js     80236e <opencons+0x56>
		return r;
	if ((r = sys_page_alloc(0, fd, PTE_P|PTE_U|PTE_W|PTE_SHARE)) < 0)
  802330:	83 ec 04             	sub    $0x4,%esp
  802333:	68 07 04 00 00       	push   $0x407
  802338:	ff 75 f4             	pushl  -0xc(%ebp)
  80233b:	6a 00                	push   $0x0
  80233d:	e8 1b ec ff ff       	call   800f5d <sys_page_alloc>
  802342:	83 c4 10             	add    $0x10,%esp
		return r;
  802345:	89 c2                	mov    %eax,%edx
	int r;
	struct Fd* fd;

	if ((r = fd_alloc(&fd)) < 0)
		return r;
	if ((r = sys_page_alloc(0, fd, PTE_P|PTE_U|PTE_W|PTE_SHARE)) < 0)
  802347:	85 c0                	test   %eax,%eax
  802349:	78 23                	js     80236e <opencons+0x56>
		return r;
	fd->fd_dev_id = devcons.dev_id;
  80234b:	8b 15 58 30 80 00    	mov    0x803058,%edx
  802351:	8b 45 f4             	mov    -0xc(%ebp),%eax
  802354:	89 10                	mov    %edx,(%eax)
	fd->fd_omode = O_RDWR;
  802356:	8b 45 f4             	mov    -0xc(%ebp),%eax
  802359:	c7 40 08 02 00 00 00 	movl   $0x2,0x8(%eax)
	return fd2num(fd);
  802360:	83 ec 0c             	sub    $0xc,%esp
  802363:	50                   	push   %eax
  802364:	e8 86 ee ff ff       	call   8011ef <fd2num>
  802369:	89 c2                	mov    %eax,%edx
  80236b:	83 c4 10             	add    $0x10,%esp
}
  80236e:	89 d0                	mov    %edx,%eax
  802370:	c9                   	leave  
  802371:	c3                   	ret    

00802372 <_panic>:
 * It prints "panic: <message>", then causes a breakpoint exception,
 * which causes JOS to enter the JOS kernel monitor.
 */
void
_panic(const char *file, int line, const char *fmt, ...)
{
  802372:	55                   	push   %ebp
  802373:	89 e5                	mov    %esp,%ebp
  802375:	56                   	push   %esi
  802376:	53                   	push   %ebx
	va_list ap;

	va_start(ap, fmt);
  802377:	8d 5d 14             	lea    0x14(%ebp),%ebx

	// Print the panic message
	cprintf("[%08x] user panic in %s at %s:%d: ",
  80237a:	8b 35 00 30 80 00    	mov    0x803000,%esi
  802380:	e8 9a eb ff ff       	call   800f1f <sys_getenvid>
  802385:	83 ec 0c             	sub    $0xc,%esp
  802388:	ff 75 0c             	pushl  0xc(%ebp)
  80238b:	ff 75 08             	pushl  0x8(%ebp)
  80238e:	56                   	push   %esi
  80238f:	50                   	push   %eax
  802390:	68 74 2d 80 00       	push   $0x802d74
  802395:	e8 33 e2 ff ff       	call   8005cd <cprintf>
		sys_getenvid(), binaryname, file, line);
	vcprintf(fmt, ap);
  80239a:	83 c4 18             	add    $0x18,%esp
  80239d:	53                   	push   %ebx
  80239e:	ff 75 10             	pushl  0x10(%ebp)
  8023a1:	e8 d6 e1 ff ff       	call   80057c <vcprintf>
	cprintf("\n");
  8023a6:	c7 04 24 60 2d 80 00 	movl   $0x802d60,(%esp)
  8023ad:	e8 1b e2 ff ff       	call   8005cd <cprintf>
  8023b2:	83 c4 10             	add    $0x10,%esp

	// Cause a breakpoint exception
	while (1)
		asm volatile("int3");
  8023b5:	cc                   	int3   
  8023b6:	eb fd                	jmp    8023b5 <_panic+0x43>

008023b8 <ipc_recv>:
//   If 'pg' is null, pass sys_ipc_recv a value that it will understand
//   as meaning "no page".  (Zero is not the right value, since that's
//   a perfectly valid place to map a page.)
int32_t
ipc_recv(envid_t *from_env_store, void *pg, int *perm_store)
{
  8023b8:	55                   	push   %ebp
  8023b9:	89 e5                	mov    %esp,%ebp
  8023bb:	56                   	push   %esi
  8023bc:	53                   	push   %ebx
  8023bd:	8b 75 08             	mov    0x8(%ebp),%esi
  8023c0:	8b 45 0c             	mov    0xc(%ebp),%eax
  8023c3:	8b 5d 10             	mov    0x10(%ebp),%ebx
	// LAB 4: Your code here.
	//panic("ipc_recv not implemented");
        if(!pg) pg = (void *)UTOP;
  8023c6:	85 c0                	test   %eax,%eax
  8023c8:	ba 00 00 c0 ee       	mov    $0xeec00000,%edx
  8023cd:	0f 44 c2             	cmove  %edx,%eax
        int r;
        if((r = sys_ipc_recv(pg)) < 0) {
  8023d0:	83 ec 0c             	sub    $0xc,%esp
  8023d3:	50                   	push   %eax
  8023d4:	e8 34 ed ff ff       	call   80110d <sys_ipc_recv>
  8023d9:	83 c4 10             	add    $0x10,%esp
  8023dc:	85 c0                	test   %eax,%eax
  8023de:	79 16                	jns    8023f6 <ipc_recv+0x3e>
                if(from_env_store) *from_env_store = 0;
  8023e0:	85 f6                	test   %esi,%esi
  8023e2:	74 06                	je     8023ea <ipc_recv+0x32>
  8023e4:	c7 06 00 00 00 00    	movl   $0x0,(%esi)
                if(perm_store) *perm_store = 0;
  8023ea:	85 db                	test   %ebx,%ebx
  8023ec:	74 2c                	je     80241a <ipc_recv+0x62>
  8023ee:	c7 03 00 00 00 00    	movl   $0x0,(%ebx)
  8023f4:	eb 24                	jmp    80241a <ipc_recv+0x62>
                return r;
        }
        if(from_env_store) *from_env_store = thisenv->env_ipc_from;
  8023f6:	85 f6                	test   %esi,%esi
  8023f8:	74 0a                	je     802404 <ipc_recv+0x4c>
  8023fa:	a1 18 40 80 00       	mov    0x804018,%eax
  8023ff:	8b 40 74             	mov    0x74(%eax),%eax
  802402:	89 06                	mov    %eax,(%esi)
        if(perm_store) *perm_store = thisenv->env_ipc_perm;
  802404:	85 db                	test   %ebx,%ebx
  802406:	74 0a                	je     802412 <ipc_recv+0x5a>
  802408:	a1 18 40 80 00       	mov    0x804018,%eax
  80240d:	8b 40 78             	mov    0x78(%eax),%eax
  802410:	89 03                	mov    %eax,(%ebx)
         
	return thisenv->env_ipc_value;
  802412:	a1 18 40 80 00       	mov    0x804018,%eax
  802417:	8b 40 70             	mov    0x70(%eax),%eax
}
  80241a:	8d 65 f8             	lea    -0x8(%ebp),%esp
  80241d:	5b                   	pop    %ebx
  80241e:	5e                   	pop    %esi
  80241f:	5d                   	pop    %ebp
  802420:	c3                   	ret    

00802421 <ipc_send>:
//   Use sys_yield() to be CPU-friendly.
//   If 'pg' is null, pass sys_ipc_try_send a value that it will understand
//   as meaning "no page".  (Zero is not the right value.)
void
ipc_send(envid_t to_env, uint32_t val, void *pg, int perm)
{
  802421:	55                   	push   %ebp
  802422:	89 e5                	mov    %esp,%ebp
  802424:	57                   	push   %edi
  802425:	56                   	push   %esi
  802426:	53                   	push   %ebx
  802427:	83 ec 0c             	sub    $0xc,%esp
  80242a:	8b 7d 08             	mov    0x8(%ebp),%edi
  80242d:	8b 75 0c             	mov    0xc(%ebp),%esi
  802430:	8b 5d 10             	mov    0x10(%ebp),%ebx
	// LAB 4: Your code here.
	//panic("ipc_send not implemented");
        if(!pg) pg = (void *)UTOP;
  802433:	85 db                	test   %ebx,%ebx
  802435:	b8 00 00 c0 ee       	mov    $0xeec00000,%eax
  80243a:	0f 44 d8             	cmove  %eax,%ebx
  80243d:	eb 1c                	jmp    80245b <ipc_send+0x3a>
        int r;
        while((r = sys_ipc_try_send(to_env, val, pg, perm)) < 0) {
                 if(r != -E_IPC_NOT_RECV)
  80243f:	83 f8 f9             	cmp    $0xfffffff9,%eax
  802442:	74 12                	je     802456 <ipc_send+0x35>
                         panic("sys_ipc_try_send fails %e\n", r);
  802444:	50                   	push   %eax
  802445:	68 98 2d 80 00       	push   $0x802d98
  80244a:	6a 39                	push   $0x39
  80244c:	68 b3 2d 80 00       	push   $0x802db3
  802451:	e8 1c ff ff ff       	call   802372 <_panic>
                 sys_yield();
  802456:	e8 e3 ea ff ff       	call   800f3e <sys_yield>
{
	// LAB 4: Your code here.
	//panic("ipc_send not implemented");
        if(!pg) pg = (void *)UTOP;
        int r;
        while((r = sys_ipc_try_send(to_env, val, pg, perm)) < 0) {
  80245b:	ff 75 14             	pushl  0x14(%ebp)
  80245e:	53                   	push   %ebx
  80245f:	56                   	push   %esi
  802460:	57                   	push   %edi
  802461:	e8 84 ec ff ff       	call   8010ea <sys_ipc_try_send>
  802466:	83 c4 10             	add    $0x10,%esp
  802469:	85 c0                	test   %eax,%eax
  80246b:	78 d2                	js     80243f <ipc_send+0x1e>
                 if(r != -E_IPC_NOT_RECV)
                         panic("sys_ipc_try_send fails %e\n", r);
                 sys_yield();
        }
                
}
  80246d:	8d 65 f4             	lea    -0xc(%ebp),%esp
  802470:	5b                   	pop    %ebx
  802471:	5e                   	pop    %esi
  802472:	5f                   	pop    %edi
  802473:	5d                   	pop    %ebp
  802474:	c3                   	ret    

00802475 <ipc_find_env>:
// Find the first environment of the given type.  We'll use this to
// find special environments.
// Returns 0 if no such environment exists.
envid_t
ipc_find_env(enum EnvType type)
{
  802475:	55                   	push   %ebp
  802476:	89 e5                	mov    %esp,%ebp
  802478:	8b 4d 08             	mov    0x8(%ebp),%ecx
	int i;
	for (i = 0; i < NENV; i++)
  80247b:	b8 00 00 00 00       	mov    $0x0,%eax
		if (envs[i].env_type == type)
  802480:	6b d0 7c             	imul   $0x7c,%eax,%edx
  802483:	81 c2 00 00 c0 ee    	add    $0xeec00000,%edx
  802489:	8b 52 50             	mov    0x50(%edx),%edx
  80248c:	39 ca                	cmp    %ecx,%edx
  80248e:	75 0d                	jne    80249d <ipc_find_env+0x28>
			return envs[i].env_id;
  802490:	6b c0 7c             	imul   $0x7c,%eax,%eax
  802493:	05 40 00 c0 ee       	add    $0xeec00040,%eax
  802498:	8b 40 08             	mov    0x8(%eax),%eax
  80249b:	eb 0e                	jmp    8024ab <ipc_find_env+0x36>
// Returns 0 if no such environment exists.
envid_t
ipc_find_env(enum EnvType type)
{
	int i;
	for (i = 0; i < NENV; i++)
  80249d:	83 c0 01             	add    $0x1,%eax
  8024a0:	3d 00 04 00 00       	cmp    $0x400,%eax
  8024a5:	75 d9                	jne    802480 <ipc_find_env+0xb>
		if (envs[i].env_type == type)
			return envs[i].env_id;
	return 0;
  8024a7:	66 b8 00 00          	mov    $0x0,%ax
}
  8024ab:	5d                   	pop    %ebp
  8024ac:	c3                   	ret    

008024ad <pageref>:
#include <inc/lib.h>

int
pageref(void *v)
{
  8024ad:	55                   	push   %ebp
  8024ae:	89 e5                	mov    %esp,%ebp
  8024b0:	8b 55 08             	mov    0x8(%ebp),%edx
	pte_t pte;

	if (!(uvpd[PDX(v)] & PTE_P))
  8024b3:	89 d0                	mov    %edx,%eax
  8024b5:	c1 e8 16             	shr    $0x16,%eax
  8024b8:	8b 0c 85 00 d0 7b ef 	mov    -0x10843000(,%eax,4),%ecx
		return 0;
  8024bf:	b8 00 00 00 00       	mov    $0x0,%eax
int
pageref(void *v)
{
	pte_t pte;

	if (!(uvpd[PDX(v)] & PTE_P))
  8024c4:	f6 c1 01             	test   $0x1,%cl
  8024c7:	74 1d                	je     8024e6 <pageref+0x39>
		return 0;
	pte = uvpt[PGNUM(v)];
  8024c9:	c1 ea 0c             	shr    $0xc,%edx
  8024cc:	8b 14 95 00 00 40 ef 	mov    -0x10c00000(,%edx,4),%edx
	if (!(pte & PTE_P))
  8024d3:	f6 c2 01             	test   $0x1,%dl
  8024d6:	74 0e                	je     8024e6 <pageref+0x39>
		return 0;
	return pages[PGNUM(pte)].pp_ref;
  8024d8:	c1 ea 0c             	shr    $0xc,%edx
  8024db:	0f b7 04 d5 04 00 00 	movzwl -0x10fffffc(,%edx,8),%eax
  8024e2:	ef 
  8024e3:	0f b7 c0             	movzwl %ax,%eax
}
  8024e6:	5d                   	pop    %ebp
  8024e7:	c3                   	ret    
  8024e8:	66 90                	xchg   %ax,%ax
  8024ea:	66 90                	xchg   %ax,%ax
  8024ec:	66 90                	xchg   %ax,%ax
  8024ee:	66 90                	xchg   %ax,%ax

008024f0 <__udivdi3>:
  8024f0:	55                   	push   %ebp
  8024f1:	57                   	push   %edi
  8024f2:	56                   	push   %esi
  8024f3:	83 ec 10             	sub    $0x10,%esp
  8024f6:	8b 54 24 2c          	mov    0x2c(%esp),%edx
  8024fa:	8b 7c 24 20          	mov    0x20(%esp),%edi
  8024fe:	8b 74 24 24          	mov    0x24(%esp),%esi
  802502:	8b 4c 24 28          	mov    0x28(%esp),%ecx
  802506:	85 d2                	test   %edx,%edx
  802508:	89 7c 24 04          	mov    %edi,0x4(%esp)
  80250c:	89 34 24             	mov    %esi,(%esp)
  80250f:	89 c8                	mov    %ecx,%eax
  802511:	75 35                	jne    802548 <__udivdi3+0x58>
  802513:	39 f1                	cmp    %esi,%ecx
  802515:	0f 87 bd 00 00 00    	ja     8025d8 <__udivdi3+0xe8>
  80251b:	85 c9                	test   %ecx,%ecx
  80251d:	89 cd                	mov    %ecx,%ebp
  80251f:	75 0b                	jne    80252c <__udivdi3+0x3c>
  802521:	b8 01 00 00 00       	mov    $0x1,%eax
  802526:	31 d2                	xor    %edx,%edx
  802528:	f7 f1                	div    %ecx
  80252a:	89 c5                	mov    %eax,%ebp
  80252c:	89 f0                	mov    %esi,%eax
  80252e:	31 d2                	xor    %edx,%edx
  802530:	f7 f5                	div    %ebp
  802532:	89 c6                	mov    %eax,%esi
  802534:	89 f8                	mov    %edi,%eax
  802536:	f7 f5                	div    %ebp
  802538:	89 f2                	mov    %esi,%edx
  80253a:	83 c4 10             	add    $0x10,%esp
  80253d:	5e                   	pop    %esi
  80253e:	5f                   	pop    %edi
  80253f:	5d                   	pop    %ebp
  802540:	c3                   	ret    
  802541:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
  802548:	3b 14 24             	cmp    (%esp),%edx
  80254b:	77 7b                	ja     8025c8 <__udivdi3+0xd8>
  80254d:	0f bd f2             	bsr    %edx,%esi
  802550:	83 f6 1f             	xor    $0x1f,%esi
  802553:	0f 84 97 00 00 00    	je     8025f0 <__udivdi3+0x100>
  802559:	bd 20 00 00 00       	mov    $0x20,%ebp
  80255e:	89 d7                	mov    %edx,%edi
  802560:	89 f1                	mov    %esi,%ecx
  802562:	29 f5                	sub    %esi,%ebp
  802564:	d3 e7                	shl    %cl,%edi
  802566:	89 c2                	mov    %eax,%edx
  802568:	89 e9                	mov    %ebp,%ecx
  80256a:	d3 ea                	shr    %cl,%edx
  80256c:	89 f1                	mov    %esi,%ecx
  80256e:	09 fa                	or     %edi,%edx
  802570:	8b 3c 24             	mov    (%esp),%edi
  802573:	d3 e0                	shl    %cl,%eax
  802575:	89 54 24 08          	mov    %edx,0x8(%esp)
  802579:	89 e9                	mov    %ebp,%ecx
  80257b:	89 44 24 0c          	mov    %eax,0xc(%esp)
  80257f:	8b 44 24 04          	mov    0x4(%esp),%eax
  802583:	89 fa                	mov    %edi,%edx
  802585:	d3 ea                	shr    %cl,%edx
  802587:	89 f1                	mov    %esi,%ecx
  802589:	d3 e7                	shl    %cl,%edi
  80258b:	89 e9                	mov    %ebp,%ecx
  80258d:	d3 e8                	shr    %cl,%eax
  80258f:	09 c7                	or     %eax,%edi
  802591:	89 f8                	mov    %edi,%eax
  802593:	f7 74 24 08          	divl   0x8(%esp)
  802597:	89 d5                	mov    %edx,%ebp
  802599:	89 c7                	mov    %eax,%edi
  80259b:	f7 64 24 0c          	mull   0xc(%esp)
  80259f:	39 d5                	cmp    %edx,%ebp
  8025a1:	89 14 24             	mov    %edx,(%esp)
  8025a4:	72 11                	jb     8025b7 <__udivdi3+0xc7>
  8025a6:	8b 54 24 04          	mov    0x4(%esp),%edx
  8025aa:	89 f1                	mov    %esi,%ecx
  8025ac:	d3 e2                	shl    %cl,%edx
  8025ae:	39 c2                	cmp    %eax,%edx
  8025b0:	73 5e                	jae    802610 <__udivdi3+0x120>
  8025b2:	3b 2c 24             	cmp    (%esp),%ebp
  8025b5:	75 59                	jne    802610 <__udivdi3+0x120>
  8025b7:	8d 47 ff             	lea    -0x1(%edi),%eax
  8025ba:	31 f6                	xor    %esi,%esi
  8025bc:	89 f2                	mov    %esi,%edx
  8025be:	83 c4 10             	add    $0x10,%esp
  8025c1:	5e                   	pop    %esi
  8025c2:	5f                   	pop    %edi
  8025c3:	5d                   	pop    %ebp
  8025c4:	c3                   	ret    
  8025c5:	8d 76 00             	lea    0x0(%esi),%esi
  8025c8:	31 f6                	xor    %esi,%esi
  8025ca:	31 c0                	xor    %eax,%eax
  8025cc:	89 f2                	mov    %esi,%edx
  8025ce:	83 c4 10             	add    $0x10,%esp
  8025d1:	5e                   	pop    %esi
  8025d2:	5f                   	pop    %edi
  8025d3:	5d                   	pop    %ebp
  8025d4:	c3                   	ret    
  8025d5:	8d 76 00             	lea    0x0(%esi),%esi
  8025d8:	89 f2                	mov    %esi,%edx
  8025da:	31 f6                	xor    %esi,%esi
  8025dc:	89 f8                	mov    %edi,%eax
  8025de:	f7 f1                	div    %ecx
  8025e0:	89 f2                	mov    %esi,%edx
  8025e2:	83 c4 10             	add    $0x10,%esp
  8025e5:	5e                   	pop    %esi
  8025e6:	5f                   	pop    %edi
  8025e7:	5d                   	pop    %ebp
  8025e8:	c3                   	ret    
  8025e9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
  8025f0:	3b 4c 24 04          	cmp    0x4(%esp),%ecx
  8025f4:	76 0b                	jbe    802601 <__udivdi3+0x111>
  8025f6:	31 c0                	xor    %eax,%eax
  8025f8:	3b 14 24             	cmp    (%esp),%edx
  8025fb:	0f 83 37 ff ff ff    	jae    802538 <__udivdi3+0x48>
  802601:	b8 01 00 00 00       	mov    $0x1,%eax
  802606:	e9 2d ff ff ff       	jmp    802538 <__udivdi3+0x48>
  80260b:	90                   	nop
  80260c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
  802610:	89 f8                	mov    %edi,%eax
  802612:	31 f6                	xor    %esi,%esi
  802614:	e9 1f ff ff ff       	jmp    802538 <__udivdi3+0x48>
  802619:	66 90                	xchg   %ax,%ax
  80261b:	66 90                	xchg   %ax,%ax
  80261d:	66 90                	xchg   %ax,%ax
  80261f:	90                   	nop

00802620 <__umoddi3>:
  802620:	55                   	push   %ebp
  802621:	57                   	push   %edi
  802622:	56                   	push   %esi
  802623:	83 ec 20             	sub    $0x20,%esp
  802626:	8b 44 24 34          	mov    0x34(%esp),%eax
  80262a:	8b 4c 24 30          	mov    0x30(%esp),%ecx
  80262e:	8b 7c 24 38          	mov    0x38(%esp),%edi
  802632:	89 c6                	mov    %eax,%esi
  802634:	89 44 24 10          	mov    %eax,0x10(%esp)
  802638:	8b 44 24 3c          	mov    0x3c(%esp),%eax
  80263c:	89 4c 24 1c          	mov    %ecx,0x1c(%esp)
  802640:	89 7c 24 0c          	mov    %edi,0xc(%esp)
  802644:	89 4c 24 14          	mov    %ecx,0x14(%esp)
  802648:	89 74 24 18          	mov    %esi,0x18(%esp)
  80264c:	85 c0                	test   %eax,%eax
  80264e:	89 c2                	mov    %eax,%edx
  802650:	75 1e                	jne    802670 <__umoddi3+0x50>
  802652:	39 f7                	cmp    %esi,%edi
  802654:	76 52                	jbe    8026a8 <__umoddi3+0x88>
  802656:	89 c8                	mov    %ecx,%eax
  802658:	89 f2                	mov    %esi,%edx
  80265a:	f7 f7                	div    %edi
  80265c:	89 d0                	mov    %edx,%eax
  80265e:	31 d2                	xor    %edx,%edx
  802660:	83 c4 20             	add    $0x20,%esp
  802663:	5e                   	pop    %esi
  802664:	5f                   	pop    %edi
  802665:	5d                   	pop    %ebp
  802666:	c3                   	ret    
  802667:	89 f6                	mov    %esi,%esi
  802669:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi
  802670:	39 f0                	cmp    %esi,%eax
  802672:	77 5c                	ja     8026d0 <__umoddi3+0xb0>
  802674:	0f bd e8             	bsr    %eax,%ebp
  802677:	83 f5 1f             	xor    $0x1f,%ebp
  80267a:	75 64                	jne    8026e0 <__umoddi3+0xc0>
  80267c:	8b 6c 24 14          	mov    0x14(%esp),%ebp
  802680:	39 6c 24 0c          	cmp    %ebp,0xc(%esp)
  802684:	0f 86 f6 00 00 00    	jbe    802780 <__umoddi3+0x160>
  80268a:	3b 44 24 18          	cmp    0x18(%esp),%eax
  80268e:	0f 82 ec 00 00 00    	jb     802780 <__umoddi3+0x160>
  802694:	8b 44 24 14          	mov    0x14(%esp),%eax
  802698:	8b 54 24 18          	mov    0x18(%esp),%edx
  80269c:	83 c4 20             	add    $0x20,%esp
  80269f:	5e                   	pop    %esi
  8026a0:	5f                   	pop    %edi
  8026a1:	5d                   	pop    %ebp
  8026a2:	c3                   	ret    
  8026a3:	90                   	nop
  8026a4:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
  8026a8:	85 ff                	test   %edi,%edi
  8026aa:	89 fd                	mov    %edi,%ebp
  8026ac:	75 0b                	jne    8026b9 <__umoddi3+0x99>
  8026ae:	b8 01 00 00 00       	mov    $0x1,%eax
  8026b3:	31 d2                	xor    %edx,%edx
  8026b5:	f7 f7                	div    %edi
  8026b7:	89 c5                	mov    %eax,%ebp
  8026b9:	8b 44 24 10          	mov    0x10(%esp),%eax
  8026bd:	31 d2                	xor    %edx,%edx
  8026bf:	f7 f5                	div    %ebp
  8026c1:	89 c8                	mov    %ecx,%eax
  8026c3:	f7 f5                	div    %ebp
  8026c5:	eb 95                	jmp    80265c <__umoddi3+0x3c>
  8026c7:	89 f6                	mov    %esi,%esi
  8026c9:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi
  8026d0:	89 c8                	mov    %ecx,%eax
  8026d2:	89 f2                	mov    %esi,%edx
  8026d4:	83 c4 20             	add    $0x20,%esp
  8026d7:	5e                   	pop    %esi
  8026d8:	5f                   	pop    %edi
  8026d9:	5d                   	pop    %ebp
  8026da:	c3                   	ret    
  8026db:	90                   	nop
  8026dc:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
  8026e0:	b8 20 00 00 00       	mov    $0x20,%eax
  8026e5:	89 e9                	mov    %ebp,%ecx
  8026e7:	29 e8                	sub    %ebp,%eax
  8026e9:	d3 e2                	shl    %cl,%edx
  8026eb:	89 c7                	mov    %eax,%edi
  8026ed:	89 44 24 18          	mov    %eax,0x18(%esp)
  8026f1:	8b 44 24 0c          	mov    0xc(%esp),%eax
  8026f5:	89 f9                	mov    %edi,%ecx
  8026f7:	d3 e8                	shr    %cl,%eax
  8026f9:	89 c1                	mov    %eax,%ecx
  8026fb:	8b 44 24 0c          	mov    0xc(%esp),%eax
  8026ff:	09 d1                	or     %edx,%ecx
  802701:	89 fa                	mov    %edi,%edx
  802703:	89 4c 24 10          	mov    %ecx,0x10(%esp)
  802707:	89 e9                	mov    %ebp,%ecx
  802709:	d3 e0                	shl    %cl,%eax
  80270b:	89 f9                	mov    %edi,%ecx
  80270d:	89 44 24 0c          	mov    %eax,0xc(%esp)
  802711:	89 f0                	mov    %esi,%eax
  802713:	d3 e8                	shr    %cl,%eax
  802715:	89 e9                	mov    %ebp,%ecx
  802717:	89 c7                	mov    %eax,%edi
  802719:	8b 44 24 1c          	mov    0x1c(%esp),%eax
  80271d:	d3 e6                	shl    %cl,%esi
  80271f:	89 d1                	mov    %edx,%ecx
  802721:	89 fa                	mov    %edi,%edx
  802723:	d3 e8                	shr    %cl,%eax
  802725:	89 e9                	mov    %ebp,%ecx
  802727:	09 f0                	or     %esi,%eax
  802729:	8b 74 24 1c          	mov    0x1c(%esp),%esi
  80272d:	f7 74 24 10          	divl   0x10(%esp)
  802731:	d3 e6                	shl    %cl,%esi
  802733:	89 d1                	mov    %edx,%ecx
  802735:	f7 64 24 0c          	mull   0xc(%esp)
  802739:	39 d1                	cmp    %edx,%ecx
  80273b:	89 74 24 14          	mov    %esi,0x14(%esp)
  80273f:	89 d7                	mov    %edx,%edi
  802741:	89 c6                	mov    %eax,%esi
  802743:	72 0a                	jb     80274f <__umoddi3+0x12f>
  802745:	39 44 24 14          	cmp    %eax,0x14(%esp)
  802749:	73 10                	jae    80275b <__umoddi3+0x13b>
  80274b:	39 d1                	cmp    %edx,%ecx
  80274d:	75 0c                	jne    80275b <__umoddi3+0x13b>
  80274f:	89 d7                	mov    %edx,%edi
  802751:	89 c6                	mov    %eax,%esi
  802753:	2b 74 24 0c          	sub    0xc(%esp),%esi
  802757:	1b 7c 24 10          	sbb    0x10(%esp),%edi
  80275b:	89 ca                	mov    %ecx,%edx
  80275d:	89 e9                	mov    %ebp,%ecx
  80275f:	8b 44 24 14          	mov    0x14(%esp),%eax
  802763:	29 f0                	sub    %esi,%eax
  802765:	19 fa                	sbb    %edi,%edx
  802767:	d3 e8                	shr    %cl,%eax
  802769:	0f b6 4c 24 18       	movzbl 0x18(%esp),%ecx
  80276e:	89 d7                	mov    %edx,%edi
  802770:	d3 e7                	shl    %cl,%edi
  802772:	89 e9                	mov    %ebp,%ecx
  802774:	09 f8                	or     %edi,%eax
  802776:	d3 ea                	shr    %cl,%edx
  802778:	83 c4 20             	add    $0x20,%esp
  80277b:	5e                   	pop    %esi
  80277c:	5f                   	pop    %edi
  80277d:	5d                   	pop    %ebp
  80277e:	c3                   	ret    
  80277f:	90                   	nop
  802780:	8b 74 24 10          	mov    0x10(%esp),%esi
  802784:	29 f9                	sub    %edi,%ecx
  802786:	19 c6                	sbb    %eax,%esi
  802788:	89 4c 24 14          	mov    %ecx,0x14(%esp)
  80278c:	89 74 24 18          	mov    %esi,0x18(%esp)
  802790:	e9 ff fe ff ff       	jmp    802694 <__umoddi3+0x74>
